...
void Error::CheckError(cl_int err)
{
	if (err != CL_SUCCESS)
	{
		printf("Error : %s happened in file %s at line %s /n", CConvertUtility::ConvertToUTF8(opencl_error_to_str(err)),
		       CConvertUtility::ConvertToUTF8(to_str(__FILE__)), CConvertUtility::ConvertToUTF8(to_str(__LINE__)));
