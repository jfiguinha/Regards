...
wxString Error::to_str(const cl_int& x, std::streamsize width, char fill)
{
	using namespace std;
	ostringstream os;
	os << setw(static_cast<int>(width)) << setfill(fill) << x;
	if (!os)
	{
...
void Error::CheckError(cl_int err)
{
	if (err != CL_SUCCESS)
	{
		printf("Error : %s happened in file %s at line %s /n", CConvertUtility::ConvertToStdString(opencl_error_to_str(err)).c_str(),
		       CConvertUtility::ConvertToStdString(to_str(__FILE__)).c_str(), CConvertUtility::ConvertToStdString(to_str(__LINE__)).c_str());
