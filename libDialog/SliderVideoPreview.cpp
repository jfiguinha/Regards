...
CSliderVideoPreview::CSliderVideoPreview(wxWindow* parent, wxWindowID id, CSliderInterface* sliderEvent,
	...
	: CWindowMain("CSliderVideoPreview", parent, id)
{
	...
	Connect(wxEVT_PAINT, wxPaintEventHandler(CSliderVideoPreview::on_paint));
	...
	m_animation->SetSize(wxSize(this->CSliderVideoPreview::GetHeight(), this->CSliderVideoPreview::GetHeight()));
...
void CSliderVideoPreview::Draw(wxDC* context)
{
	if (GetWindowWidth() > 0 && GetWindowHeight() > 0)
	{
		...
		int totalTimeSize = DrawTotalTimeLibelle(&pimpl->sourceDCContext, totalTime, volumeSize);
		...
		InsertPreviousButton(xButtonPos, &pimpl->sourceDCContext);

		xButtonPos += themeSlider.GetButtonSpeakerWidth() + 5;
		InsertNextButton(xButtonPos, &pimpl->sourceDCContext);
...
void CSliderVideoPreview::InsertPreviousButton(const int& xStart, wxDC* context)
{
	...
	if (!buttonImagePrevious.IsOk() || (buttonImagePrevious.GetWidth() != themeSlider.GetButtonVolumeDownWidth() ||
		buttonImagePrevious.GetHeight() != themeSlider.GetButtonVolumeDownHeight()))
	{
		buttonImagePrevious = CLibResource::CreatePictureFromSVG("IDB_ARROWLPNG",
		                                                         themeSlider.GetButtonVolumeDownWidth(),
...
void CSliderVideoPreview::InsertNextButton(const int& xStart, wxDC* context)
{
	...
	if (!buttonImageNext.IsOk() || (buttonImageNext.GetWidth() != themeSlider.GetButtonVolumeDownWidth() ||
		buttonImageNext.GetHeight() != themeSlider.GetButtonVolumeDownHeight()))
	{
		buttonImageNext = CLibResource::CreatePictureFromSVG("IDB_ARROWRPNG", themeSlider.GetButtonVolumeDownWidth(),
		                                                     themeSlider.GetButtonVolumeDownHeight());
	}
	bmp = buttonImageNext;
...
void CSliderVideoPreview::on_paint(wxPaintEvent& event)
{
