...
void ThrowIfError(int code, std::string_view context) {
    if (code < 0) {
        ...
        oss << "[FFmpeg] " << context << " : " << AvError(code);
    ...
    namespace Media {
        ...
        Timecode Timecode::FromString(std::string_view s) {
            ...
            if (s.size() < 8 || s[2] != ':' || s[5] != ':') {
                ...
                oss << "Timecode invalide '" << s << "' : format attendu "
            ...
            if (!ParseInt(p, p + 2, tc.hours) || !ParseInt(p + 3, p + 5, tc.minutes) ||
                !ParseInt(p + 6, p + 8, tc.seconds)) {
                std::ostringstream oss;

                oss << "Timecode invalide '" << s << "' : champs non numériques";
            ...
            if (s.size() > 8) {
                if (s[8] != '.') {
                    ...
                    oss << "Timecode invalide '" << s << "' : séparateur de ms attendu '.'";

                    throw std::invalid_argument(oss.str());
                }

                const std::string_view msPart = s.substr(9);

                if (msPart.empty() || msPart.size() > 3) {
                    std::ostringstream oss;

                    oss << "Timecode invalide '" << s
                ...
                if (!ParseInt(msPart.data(), msPart.data() + msPart.size(), raw)) {
                    ...
                    oss << "Timecode invalide '" << s << "' : ms non numériques";

                    throw std::invalid_argument(oss.str());
                }

                static const int scale[] = { 100, 10, 1 };
            ...
            if (!tc.IsValid()) {
                ...
                oss << "Timecode invalide '" << s << "' : valeurs hors limites";
        ...
        void MediaExtractor::ProbeInput() {
            ...
            ThrowIfError(avformat_open_input(&raw, m_inputPath.c_str(), nullptr, nullptr),
                "avformat_open_input");

            UniqueInputFmt ctx(raw);

            ThrowIfError(avformat_find_stream_info(ctx.get(), nullptr),
        ...
        namespace {
            int FindBestStream(AVFormatContext* ctx, AVMediaType type,
        ...
        ExtractionResult MediaExtractor::Extract(const std::string& outputPath,
            ...
            try {
                ...
                if (vIdx < 0 && aIdx < 0) {
                    throw std::runtime_error("Aucun flux vidéo/audio trouvé.");
                }
                ...
                ThrowIfError(avformat_alloc_output_context2(&rawOut, nullptr, nullptr,
                    outputPath.c_str()),
                ...
                for (unsigned i = 0; i < nbStreams; ++i) {
                    const bool keep =
                        static_cast<int>(i) == vIdx || static_cast<int>(i) == aIdx;
                ...
                if (outIndex == 0) {
                    throw std::runtime_error("Aucun flux à copier.");
                }
                ...
                if (!(outCtx->oformat->flags & AVFMT_NOFILE)) {
                    ThrowIfError(avio_open(&outCtx->pb, outputPath.c_str(), AVIO_FLAG_WRITE),
                        "avio_open");
                }
                ...
                const int64_t seekTarget = static_cast<int64_t>(
                    (range.start > 0 ? range.start : 1.0) * AV_TIME_BASE);

                if (opts.accurate) {
                    ThrowIfError(avformat_seek_file(inCtx.get(), -1, INT64_MIN, seekTarget,
                        seekTarget, 0),
                        "avformat_seek_file");
                }
                else {
                ...
                std::vector<int64_t> firstTs(nbStreams, AV_NOPTS_VALUE);
                ...
                while (true) {
                    ...
                    const int dstIdx =
                        srcIdx < static_cast<int>(nbStreams) ? streamMap[srcIdx] : -1;
                    ...
                    const double pktTimeSecs = ts * av_q2d(inStream->time_base);
                    ...
                    if (pktTimeSecs < range.start) {
                        ...
                        if (firstTs[srcIdx] == AV_NOPTS_VALUE && !isKeyframe &&
                    ...
                    av_packet_rescale_ts(pkt.get(), inStream->time_base,
                        outStream->time_base);
                    ...
                    if (opts.progressCallback && totalRange > 0.0) {
                        const double progress =
                            std::clamp((pktTimeSecs - range.start) / totalRange, 0.0, 1.0);
        ...
        namespace {
            bool RunExtraction(const std::string& fileIn, const std::string& timestart,
                const std::string& timestop, const std::string& fileOut,
                ExtractionMode mode) {
                try {
                    ...
                    if (start != 0.0 && stop != 0.0 && stop <= start) {
                        return false;
                    }
        ...
        bool ExecuteFFmpegCutVideo(const std::string& fileIn,
        ...
        bool ExecuteFFmpegExtractVideo(const std::string& fileIn,
            const std::string& fileOut) {
            return RunExtraction(fileIn, "00:00:00.000", "00:00:00.000", fileOut,
        ...
        bool ExecuteFFmpegExtractAudio(const std::string& fileIn,
            const std::string& fileOut) {
            return RunExtraction(fileIn, "00:00:00.000", "00:00:00.000", fileOut,
        ...
        bool ExecuteFFmpegMuxVideoAudio(const std::string& fileVideo,
            ...
            try {
                ...
                const int vIdx = av_find_best_stream(inVideo.get(), AVMEDIA_TYPE_VIDEO, -1,
                    -1, nullptr, 0);
                ...
                const int aIdx = av_find_best_stream(inAudio.get(), AVMEDIA_TYPE_AUDIO, -1,
                    -1, nullptr, 0);
                ...
                if (!(outCtx->oformat->flags & AVFMT_NOFILE)) {
                    ThrowIfError(avio_open(&outCtx->pb, fileOutput.c_str(), AVIO_FLAG_WRITE),
                        "avio_open");
                }
                ...
                auto ReadVideoPacket = [&]() -> bool {
                    av_packet_unref(videoPkt.get());

                    while (true) {
                        const int ret = av_read_frame(inVideo.get(), videoPkt.get());
                ...
                auto ReadAudioPacket = [&]() -> bool {
                    av_packet_unref(audioPkt.get());

                    while (true) {
                        const int ret = av_read_frame(inAudio.get(), audioPkt.get());
                ...
                while (haveVideo || haveAudio) {
                    ...
                    if (!haveAudio) {
                        ...
                        const int64_t vDts =
                            ...
                            ? INT64_MAX
                            : av_rescale_q(videoPkt->dts, srcVideo->time_base,
                                AV_TIME_BASE_Q);

                        const int64_t aDts =
                            audioPkt->dts == AV_NOPTS_VALUE
                            ? INT64_MAX
                            : av_rescale_q(audioPkt->dts, srcAudio->time_base,
                                AV_TIME_BASE_Q);
                    ...
                    if (writeVideo) {
                        if (firstVideoDts == AV_NOPTS_VALUE) {
                            firstVideoDts = videoPkt->dts != AV_NOPTS_VALUE ? videoPkt->dts : 0;
                        }
                        ...
                        av_packet_rescale_ts(videoPkt.get(), srcVideo->time_base,
                            outVideo->time_base);
                        ...
                        ThrowIfError(av_interleaved_write_frame(outCtx.get(), videoPkt.get()),
                            "write video");
                        ...
                        if (firstAudioDts == AV_NOPTS_VALUE) {
                            firstAudioDts = audioPkt->dts != AV_NOPTS_VALUE ? audioPkt->dts : 0;
                        }
                        ...
                        av_packet_rescale_ts(audioPkt.get(), srcAudio->time_base,
                            outAudio->time_base);
                        ...
                        ThrowIfError(av_interleaved_write_frame(outCtx.get(), audioPkt.get()),
                            "write audio");
        ...
        bool CreateLoopedAudio(const std::string& inputAudioFile,
            const std::string& outputFile,
            const std::string& timeVideo) {
            try {
                const double targetDurationSeconds =
                    Timecode::FromString(timeVideo).ToSeconds();
                ...
                ThrowIfError(
                    avformat_open_input(&rawIn, inputAudioFile.c_str(), nullptr, nullptr),
                    "avformat_open_input");
                ...
                const int audioIdx = av_find_best_stream(inCtx.get(), AVMEDIA_TYPE_AUDIO,
                    -1, -1, nullptr, 0);

                if (audioIdx < 0) {
                    throw std::runtime_error("Aucun flux audio trouvé.");
                }

                AVStream* inStream = inCtx->streams[audioIdx];

                // =====================================================
                // Durée audio
                // =====================================================

                const int64_t streamDurationTs = inStream->duration;

                if (streamDurationTs <= 0) {
                    throw std::runtime_error("Durée du flux audio inconnue.");
                }

                const double streamDurationSecs =
                    streamDurationTs * av_q2d(inStream->time_base);

                if (streamDurationSecs <= 0.0) {
                    throw std::runtime_error("Durée audio invalide.");
                }
                ...
                if (!(outCtx->oformat->flags & AVFMT_NOFILE)) {
                    ThrowIfError(avio_open(&outCtx->pb, outputFile.c_str(), AVIO_FLAG_WRITE),
                        "avio_open");
                }
                ...
                while (producedDuration < targetDurationSeconds && !finished) {
                    ThrowIfError(
                        av_seek_frame(inCtx.get(), audioIdx, 0, AVSEEK_FLAG_BACKWARD),
                        "av_seek_frame");
                    ...
                    while (true) {
                        ...
                        const double currentTime =
                            producedDuration + ((ts - firstPts) * av_q2d(inStream->time_base));
                        ...
                        if (pkt->pts != AV_NOPTS_VALUE) {
                            pkt->pts = pkt->pts - firstPts + ptsOffset;
                        }

                        if (pkt->dts != AV_NOPTS_VALUE) {
                            pkt->dts = pkt->dts - firstPts + ptsOffset;
                        }
        ...
        std::vector<wxString> GetHardwareList() {
            ...
            while ((type = av_hwdevice_iterate_types(type)) != AV_HWDEVICE_TYPE_NONE) {
                ...
                const wxString encoder = wxString::FromUTF8(typeName);
