...
uint8_t* CRegardsJpegPicture::CompressRegardsBitmapToJpeg(uint8_t* buffer, int width, int height,
                                                          unsigned long& outputsize, const bool& flip)
{
	...
	if (flip)
		tjCompress2(_jpegCompressor, buffer, width, 0, height, TJPF_BGRX, &data, &outputsize, TJSAMP_444, JPEG_QUALITY,
