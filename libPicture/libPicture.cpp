...
void CLibPicture::CreateJpegExifFile(const uint8_t * memoryBuffer, const int & itemSize, uint8_t* & data)
{
	...
	case HEIC:
		{
			...
			if (pictureMetadata.HasExif())
			{
				...
				metadata.GetMetadataBuffer(dataIn, size);
				if (size > 0)
				{
					dataIn = new uint8_t[size];
					dataOut = new uint8_t[size + 512];
					metadata.GetMetadataBuffer(dataIn, size);
					hasExif = true;


					CreateJpegExifFile(dataIn, size, dataOut);
				}
			}

			CRegardsBitmap* image = bitmap->GetRegardsBitmap();
			CHeic::SavePicture(fileName.ToStdString(), image, dataOut, size, 100 - quality, hasExif);
