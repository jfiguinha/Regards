...
namespace Regards
{
	namespace Picture
	{
		...
		public:
			...
			static void SavePictureToPdf(const wxString& fileName, CImageLoadingFormat* bitmap, const int& option, const int& quality);
			static wxString ExtractPage(const wxString& filename, const vector<int>& listPage);
			static void AddPdfPage(wxPdfDocument * oPdfDocument, CImageLoadingFormat* imageFormat, int option, int quality, int numpage);
			static void AddPage(const wxString& fileToAdd, const wxString& filename, const vector<int>& listPage, int oldAnimationPosition);
			static void RemovePage(const wxString& filename, const vector<int>& listPage);
		private:
			static bool SaveToPDF(wxImage* poImage, const wxString& fileName, const wxString& pictureName, int option, int quality);
