...
CThumbnailVideoOpenCV::CThumbnailVideoOpenCV(const wxString& fileName)
{
	filename = fileName;
...
CThumbnailVideoOpenCV::~CThumbnailVideoOpenCV()
{
	delete capture;
}

int CThumbnailVideoOpenCV::GetVideoOrientation()
{
	if (!isOpen)
		return 0;
...
CRegardsBitmap* CThumbnailVideoOpenCV::GetVideoFrame(const int& timePosition, const int& thumbnailWidth, const int& thumbnailHeight)
{
...
void CThumbnailVideoOpenCV::GetVideoDimensions(int& width, int& height, int& rotation)
{
	if (isOpen)
	{
...
int64_t CThumbnailVideoOpenCV::GetMovieDuration()
{
	double fps = capture->get(CAP_PROP_FPS);
...
vector<CImageVideoThumbnail*> CThumbnailVideoOpenCV::GetVideoListFrame(const int& widthThumbnail, const int& heightThumbnail,
