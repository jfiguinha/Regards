...
void CPiccanteHDR::BestExposure(CRegardsFloatBitmap * & floatBitmap, const float &tmoValue)
{
...
void CPiccanteHDR::FilterBilateral2DS(CRegardsFloatBitmap * & floatBitmap, const float & sigma_s, const float & sigma_r)
{
	printf("Reading an HDR file...");
	pic::Image img(floatBitmap->GetWidth(), floatBitmap->GetHeight(), 4);
	img.data = floatBitmap->GetData();
...
void CPiccanteHDR::FilterKuwahara(CRegardsFloatBitmap * & floatBitmap, const int &kernelSize)
{
	printf("Reading an HDR file...");
	pic::Image img(floatBitmap->GetWidth(), floatBitmap->GetHeight(), 4);
	img.data = floatBitmap->GetData();
...
void CPiccanteHDR::GetPictureDimensions(const wxString & fileName, int & width, int & height)
{
    pic::Image img;
...
void CPiccanteHDR::LoadPicture(const wxString & fileName, const bool &isThumbnail, CImageLoadingFormat * bitmap)
{
    pic::Image img;
    img.Read(CConvertUtility::ConvertToStdString(fileName));
