...
namespace RawSpeed
{
	...
	void SrwDecoder::decodeCompressed(TiffIFD* raw)
	{
		...
		uint32 compressed_offset = raw->getEntry(static_cast<TiffTag>(40976))->getInt();
		ByteStream* b;
		...
		for (uint32 y = 0; y < height; y++)
		{
			...
			auto img_up = (ushort16*)mRaw->getData(0, max(0, (int)y - 1));
			auto img_up2 = (ushort16*)mRaw->getData(0, max(0, (int)y - 2));
			...
			for (uint32 x = 0; x < width; x += 16)
			{
				...
				if (dir)
				{
					...
					{
						int b = len[(c >> 3)];
						int32 adj = (static_cast<int32>(bits.getBits(b)) << (32 - b) >> (32 - b));
						img[c] = adj + img_up[c];
					}
	...
	void SrwDecoder::decodeMetaDataInternal(CameraMetaData* meta)
	{
		...
		if (!this->checkCameraSupported(meta, make, model, "") && !data.empty() && data[0]->hasEntry(
			CFAREPEATPATTERNDIM))
		{
