...
#define CHECKSIZE(A) if (A > size) ThrowIOE("Error decoding DNG Slice (invalid size). File Corrupt")
	...
	void DngDecoderSlices::startDecoding()
	{
		...
		int slicesPerThread = (static_cast<int>(slices.size()) + nThreads - 1) / nThreads;
		//  decodedSlices = 0;
		pthread_attr_t attr;
	...
	void DngDecoderSlices::decodeSlice(DngDecoderThread* t)
	{
		...
		else if (compression == 0x884c)
		{
			...
			while (!t->slices.empty())
			{
				...
				auto buffer = static_cast<JSAMPARRAY>(malloc(sizeof(JSAMPROW)));
				...
				try
				{
					...
					complete_buffer = static_cast<uchar8*>(_aligned_malloc(pic_size, 16));
					while (dinfo.output_scanline < dinfo.output_height)
					{
						buffer[0] = &complete_buffer[dinfo.output_scanline * row_stride];
