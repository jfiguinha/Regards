...
namespace RawSpeed
{
	...
	{
		vector<TiffIFD*> data = mRootIFD->getIFDsWithTag(static_cast<TiffTag>(0xc5d8));
		...
		bool flipDims = false;
		if (raw->hasEntry(static_cast<TiffTag>(0xc6c5)))
		{
			ushort16 ss = raw->getEntry(static_cast<TiffTag>(0xc6c5))->getInt();
			// sRaw
			if (ss == 4)
			{
				mRaw->dim.x /= 3;
	...
	void Cr2Decoder::checkSupportInternal(CameraMetaData* meta)
	{
		...
		data = mRootIFD->getIFDsWithTag(static_cast<TiffTag>(0xc5d8));
	...
	int Cr2Decoder::getHue()
	{
		...
		uint32 model_id = mRootIFD->getEntryRecursive(static_cast<TiffTag>(0x10))->getInt();
	...
	{
		vector<TiffIFD*> data = mRootIFD->getIFDsWithTag(static_cast<TiffTag>(0x4001));
		if (data.empty())
			ThrowRDE("CR2 sRaw: Unable to locate WB info.");

		const ushort16* wb_data = data[0]->getEntry(static_cast<TiffTag>(0x4001))->getShortArray();
		...
		{
			sraw_coeffs[0] = static_cast<int>(1024.0f / (static_cast<float>(sraw_coeffs[0]) / 1024.0f));
			sraw_coeffs[2] = static_cast<int>(1024.0f / (static_cast<float>(sraw_coeffs[2]) / 1024.0f));
		}
