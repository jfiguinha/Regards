...
namespace RawSpeed
{
	...
	void Camera::parseBlackAreas(xmlDocPtr doc, xmlNodePtr cur)
	{
		...
		else if (!xmlStrcmp(cur->name, (const xmlChar*)"Horizontal"))
		{
			...
			if (y < 0)
			{
				ThrowCME("Invalid y coordinate in horizontal BlackArea of in camera %s %s", make.c_str(),
				         model.c_str());
			}
  ...
  if (EOF == sscanf((const char*)in, "%d", &i))
...
#endif
			ThrowCME("Error parsing attribute %s in tag %s, in camera %s %s.", attribute, tag, make.c_str(),
			         model.c_str());
	...
	int Camera::getAttributeAsInt(xmlNodePtr cur, const xmlChar* tag, const char* attribute)
	{
		...
		if (!key)
			ThrowCME("Could not find attribute %s in tag %s, in camera %s %s.", attribute, tag, make.c_str(),
			         model.c_str());
	...
	void Camera::parseHint(xmlDocPtr doc, xmlNodePtr cur)
	{
		if (!xmlStrcmp(cur->name, (const xmlChar*)"Hint"))
		{
			...
			if (key)
			{
				...
				ThrowCME("CameraMetadata: Could not find value for hint %s for %s %s camera.", hint_name.c_str(),
