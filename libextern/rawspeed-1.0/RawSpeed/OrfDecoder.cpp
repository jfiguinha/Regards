...
namespace RawSpeed
{
	...
	RawImage OrfDecoder::decodeRawInternal()
	{
		...
		{
			ThrowRDE("ORF Decoder: Byte count number does not match strip size: count:%u, strips:%u ", counts->count,
			         offsets->count);
		}
		...
		try
		{
			...
			makertiff.parseData();
			data = makertiff.RootIFD()->getIFDsWithTag(static_cast<TiffTag>(0x2010));
			if (data.empty())
				ThrowRDE("ORF Decoder: Unsupported compression");
			TiffEntry* oly = data[0]->getEntry(static_cast<TiffTag>(0x2010));
	...
	void OrfDecoder::decodeCompressed(ByteStream& s, uint32 w, uint32 h)
	{
		...
		for (uint32 y = 0; y < h; y++)
		{
			...
			for (uint32 x = 0; x < w; x++)
			{
				...
				bits.fill();
				i = 2 * (acarry0[2] < 3);
				for (nbits = 2 + i; static_cast<ushort16>(acarry0[0]) >> (nbits + i); nbits++);
				...
				{
					up = dest[-pitch + static_cast<int>(x)];
				...
				bits.fill();
				i = 2 * (acarry1[2] < 3);
				for (nbits = 2 + i; static_cast<ushort16>(acarry1[0]) >> (nbits + i); nbits++);
	...
	void OrfDecoder::decodeMetaDataInternal(CameraMetaData* meta)
	{
		...
		TiffIFD* ImageProcessing = nullptr;
		try
		{
			data = mRootIFD->getIFDsWithTag(MAKERNOTE);
			...
			if (!data.empty())
			{
				...
				TiffParserOlympus makertiff(&makermap);
				makertiff.parseData();
				TiffEntry* blackEntry = nullptr;

				// Try reading black level from tag 0x2040 (Olympus Imageprocessing)
				if (makertiff.RootIFD()->hasEntryRecursive(static_cast<TiffTag>(0x2040)))
				{
					try
					{
						TiffEntry* imagep = makertiff.RootIFD()->getEntryRecursive(static_cast<TiffTag>(0x2040));
						...
						if (makertiff.tiff_endian == makertiff.getHostEndian())
							...
							offset = static_cast<unsigned int>(data[0]) << 24 | static_cast<unsigned int>(data[1]) << 16
						...
						blackEntry = ImageProcessing->getEntry(static_cast<TiffTag>(0x600));
					}
					catch (TiffParserException)
					{
					}
				...
				if (!blackEntry && makertiff.RootIFD()->hasEntryRecursive(static_cast<TiffTag>(0x1012)))
				{
					blackEntry = makertiff.RootIFD()->getEntryRecursive(static_cast<TiffTag>(0x1012));
				}
