...
namespace RawSpeed
{
	...
	ushort16 ByteStreamSwap::getShort()
	{
		...
		uint32 a = buffer[off++];
		uint32 b = buffer[off++];
		return static_cast<ushort16>((a << 8) | b);
	}
	...
	int ByteStreamSwap::getInt()
	{
		...
		int r = static_cast<int>(buffer[off]) << 24 | static_cast<int>(buffer[off + 1]) << 16 | static_cast<int>(buffer[
			off + 2]) << 8 | static_cast<int>(buffer[off + 3]);
		off += 4;
		return r;
	}
	...
	uint32 ByteStreamSwap::getUInt()
	{
		...
		uint32 r = static_cast<uint32>(buffer[off]) << 24 | static_cast<uint32>(buffer[off + 1]) << 16 | static_cast<
			uint32>(buffer[off + 2]) << 8 | static_cast<uint32>(buffer[off + 3]);
		off += 4;
		return r;
	}
} // namespace RawSpeed
