...
namespace RawSpeed
{
	...
	{
		auto histogram = static_cast<int*>(malloc(4 * 65536 * sizeof(int)));
		...
		for (uint32 i = 0; i < blackAreas.size(); i++)
		{
			...
			if (!area.isVertical)
			{
				if (static_cast<int>(area.offset) + static_cast<int>(area.size) > uncropped_dim.y)
			...
			if (area.isVertical)
			{
				if (static_cast<int>(area.offset) + static_cast<int>(area.size) > uncropped_dim.x)
	...
	void RawImageDataU16::scaleBlackWhite()
	{
		...
		/* Skip, if not needed */
		if ((blackAreas.size() == 0 && blackLevel == 0 && whitePoint == 65535 && blackLevelSeparate[0] < 0) || dim.
			area() <= 0)
			return;
  ...
  use_sse2 = TRUE;
		...
		// Scale in 30.2 fp
		int full_scale_fp = static_cast<int>(app_scale * 4.0f);
		// Half Scale in 18.14 fp
		int half_scale_fp = static_cast<int>(app_scale * 4095.0f);
		...
		if (use_sse2 && app_scale < 63)
		{
			...
			auto sub_mul = static_cast<uint32*>(_aligned_malloc(16 * 4 * 2, 16));
			if (!sub_mul)
				ThrowRDE("Out of memory, failed to allocate 128 bytes");
			uint32 gw = pitch / 16;
			// 10 bit fraction
			uint32 mul = static_cast<int>(1024.0f * 65535.0f / static_cast<float>(whitePoint - blackLevelSeparate[
				mOffset.x & 1]));
			mul |= static_cast<int>(1024.0f * 65535.0f / static_cast<float>(whitePoint - blackLevelSeparate[(mOffset.x +
					1) & 1]))
				<< 16;
			...
			mul = static_cast<int>(1024.0f * 65535.0f / static_cast<float>(whitePoint - blackLevelSeparate[2 + (mOffset.
				x & 1)]));
			mul |= static_cast<int>(1024.0f * 65535.0f / static_cast<float>(whitePoint - blackLevelSeparate[2 + ((
					mOffset.x + 1) &
				1)])) << 16;
			...
			{
				__m128i sserandom = _mm_set_epi32(dim.x * 1676 + y * 18000, dim.x * 2342 + y * 34311,
		...
		else
		{
			...
			for (int i = 0; i < 4; i++)
			{
				...
				if ((mOffset.y & 1) != 0)
					v ^= 2;
				mul[i] = static_cast<int>(16384.0f * 65535.0f / static_cast<float>(whitePoint - blackLevelSeparate[v]));
