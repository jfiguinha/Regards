...
namespace RawSpeed
{
	DngDecoder::DngDecoder(TiffIFD* rootIFD, FileMap* file) : RawDecoder(file), mRootIFD(rootIFD)
	{
		...
		if (v[0] != 1)
			ThrowRDE("Not a supported DNG image format: v%u.%u.%u.%u", static_cast<int>(v[0]), static_cast<int>(v[1]),
	...
	RawImage DngDecoder::decodeRawInternal()
	{
		...
		try
		{
			...
			if (mRaw->isCFA)
			{
				...
				const uchar8* cPat = raw->getEntry(CFAPATTERN)->getData();
			...
			if (compression == 1)
			{
				...
				try
				{
					...
					{
						ThrowRDE("DNG Decoder: Byte count number does not match strip size: count:%u, strips:%u ",
						         TEcounts->count, TEoffsets->count);
					}
					...
					for (uint32 i = 0; i < slices.size(); i++)
					{
						...
						try
						{
							readUncompressedRaw(in, size, pos, mRaw->getCpp() * width * bps / 8, bps,
						...
						catch (IOException& ex)
						{
							if (i > 0)
								...
								ThrowRDE(
									"DNG decoder: IO error occurred in first slice, unable to decode more. Error is: %s",
									ex.what());
						}
					}
				}
				...
				try
				{
					...
					if (raw->hasEntry(TILEOFFSETS))
					{
						...
						if (TEoffsets->count != TEcounts->count || TEoffsets->count != nTiles)
							ThrowRDE("DNG Decoder: Tile count mismatch: offsets:%u count:%u, calculated:%u",
							         TEoffsets->count, TEcounts->count, nTiles);
						...
						for (uint32 y = 0; y < tilesY; y++)
						{
							...
							{
								DngSliceElement e(offsets[x + y * tilesX], counts[x + y * tilesX], tilew * x,
								                  tileh * y);
					...
					else
					{
						...
						if (TEcounts->count != TEoffsets->count)
						{
							ThrowRDE("DNG Decoder: Byte count number does not match strip size: count:%u, stips:%u ",
							         TEcounts->count, TEoffsets->count);
						}
		...
		if (raw->hasEntry(LINEARIZATIONTABLE) && !uncorrectedRawValues)
		{
			...
			for (int y = 0; y < mRaw->dim.y; y++)
			{
				...
				auto pixels = (ushort16*)mRaw->getData(0, y);
		...
		if (compression == 0x884c && !uncorrectedRawValues)
		{
			if (raw->hasEntry(OPCODELIST2))
			{
				...
				mRaw->blackLevel = 0;
				mRaw->blackLevelSeparate[0] = mRaw->blackLevelSeparate[1] = mRaw->blackLevelSeparate[2] = mRaw->
					blackLevelSeparate[3] = 0;
	...
	bool DngDecoder::decodeMaskedAreas(TiffIFD* raw)
	{
		...
		{
			auto topleft = iPoint2D(rects[i * 4 + 1], rects[i * 4]);
			auto bottomright = iPoint2D(rects[i * 4 + 3], rects[i * 4 + 2]);
	...
	bool DngDecoder::decodeBlackLevels(TiffIFD* raw)
	{
		...
		const uint32* iblackarray = nullptr;
		const ushort16* sblackarray = nullptr;
		...
		{
			auto blackarrayv = (const int*)raw->getEntry(BLACKLEVELDELTAV)->getIntArray();
			...
			for (int i = 0; i < 4; i++)
				mRaw->blackLevelSeparate[i] += static_cast<int>(black_sum[i >> 1] / static_cast<float>(mRaw->dim.y) *
					2.0f);
		}

		if (raw->hasEntry(BLACKLEVELDELTAH))
		{
			auto blackarrayh = (const int*)raw->getEntry(BLACKLEVELDELTAH)->getIntArray();
			float black_sum[2] = {0.0f, 0.0f};
			...
			for (int i = 0; i < 4; i++)
				mRaw->blackLevelSeparate[i] += static_cast<int>(black_sum[i & 1] / static_cast<float>(mRaw->dim.x) *
					2.0f);
		}
		return TRUE;
	}
