...
namespace RawSpeed
{
	DngOpcodes::DngOpcodes(TiffEntry* entry)
	{
		...
		for (uint32 i = 0; i < opcode_count; i++)
		{
			...
			case 4:
				mOpcodes.push_back(
					new OpcodeFixBadPixelsConstant(&data[bytes_used], entry_size - bytes_used, &opcode_used));
				break;
			case 5:
				mOpcodes.push_back(
					new OpcodeFixBadPixelsList(&data[bytes_used], entry_size - bytes_used, &opcode_used));
				break;
			case 6:
			...
			if (bytes_used > static_cast<int>(entry_size))
	...
	OpcodeFixBadPixelsConstant::OpcodeFixBadPixelsConstant(const uchar8* parameters, int param_max_bytes,
	                                                       uint32* bytes_used)
	{
		if (param_max_bytes < 8)
			ThrowRDE("OpcodeFixBadPixelsConstant: Not enough data to read parameters, only %d bytes left.",
			         param_max_bytes);
	...
	void OpcodeFixBadPixelsConstant::apply(RawImage& in, RawImage& out, int startY, int endY)
	{
		...
		for (int y = startY; y < endY; y ++)
		{
			...
			for (int x = 0; x < in->dim.x; x++)
			{
				if (src[x] == mValue)
				{
					bad_pos.push_back(offset + (static_cast<uint32>(x) | static_cast<uint32>(y) << 16));
				}
			}
		}
		if (!bad_pos.empty())
	...
	OpcodeFixBadPixelsList::OpcodeFixBadPixelsList(const uchar8* parameters, int param_max_bytes, uint32* bytes_used)
	{
		if (param_max_bytes < 12)
			ThrowRDE("OpcodeFixBadPixelsList: Not enough data to read parameters, only %d bytes left.",
			         param_max_bytes);
		...
		{
			uint32 BadPointRow = static_cast<uint32>(getLong(&parameters[bytes_used[0]]));
			uint32 BadPointCol = static_cast<uint32>(getLong(&parameters[bytes_used[0] + 4]));
			bytes_used[0] += 8;
		...
		{
			uint32 BadRectTop = static_cast<uint32>(getLong(&parameters[bytes_used[0]]));
			uint32 BadRectLeft = static_cast<uint32>(getLong(&parameters[bytes_used[0] + 4]));
			uint32 BadRectBottom = static_cast<uint32>(getLong(&parameters[bytes_used[0]]));
			uint32 BadRectRight = static_cast<uint32>(getLong(&parameters[bytes_used[0] + 4]));
			bytes_used[0] += 16;
	...
	void OpcodeFixBadPixelsList::apply(RawImage& in, RawImage& out, int startY, int endY)
	{
		iPoint2D crop = in->getCropOffset();
		uint32 offset = crop.x | (crop.y << 16);
		for (auto i = bad_pos.begin(); i != bad_pos.end(); ++i)
		{
			uint32 pos = offset + (*i);
	...
	OpcodeMapTable::OpcodeMapTable(const uchar8* parameters, int param_max_bytes, uint32* bytes_used)
	{
		...
		mAoi.setAbsolute(getLong(&parameters[4]), getLong(&parameters[0]), getLong(&parameters[12]),
		                 getLong(&parameters[8]));
	...
	RawImage& OpcodeMapTable::createOutput(RawImage& in)
	{
		...
		if (mFirstPlane > static_cast<int>(in->getCpp()))
			ThrowRDE("OpcodeMapTable: Not that many planes in actual image");

		if (mFirstPlane + mPlanes > static_cast<int>(in->getCpp()))
			ThrowRDE("OpcodeMapTable: Not that many planes in actual image");
	...
	RawImage& OpcodeMapPolynomial::createOutput(RawImage& in)
	{
		...
		{
			double in_val = static_cast<double>(i) / 65536.0;
			double val = mCoefficient[0];
			for (int j = 1; j <= mDegree; j++)
				val += mCoefficient[j] * pow(in_val, static_cast<double>(j));
			mLookup[i] = clampbits(static_cast<int>(val * 65535.5), 16);
		}
		return in;
	}
	...
	OpcodeDeltaPerRow::OpcodeDeltaPerRow(const uchar8* parameters, int param_max_bytes, uint32* bytes_used)
	{
		...
		*bytes_used = 36;
		...
		if (mAoi.getHeight() != mCount)
			ThrowRDE("OpcodeDeltaPerRow: Element count (%d) does not match height of area (%d).", mCount,
			         mAoi.getHeight());
	...
	void OpcodeDeltaPerRow::apply(RawImage& in, RawImage& out, int startY, int endY)
	{
		if (in->getDataType() == TYPE_USHORT16)
		{
			...
			for (int y = startY; y < endY; y += mRowPitch)
			{
				...
				// Add offset, so this is always first plane
				src += mFirstPlane;
				int delta = static_cast<int>(65535.0f * mDelta[y]);
				for (int x = 0; x < mAoi.getWidth(); x += mColPitch)
				{
					for (int p = 0; p < mPlanes; p++)
					{
	...
	OpcodeDeltaPerCol::OpcodeDeltaPerCol(const uchar8* parameters, int param_max_bytes, uint32* bytes_used)
	{
		...
		*bytes_used = 36;
		...
		if (mAoi.getWidth() != mCount)
			ThrowRDE("OpcodeDeltaPerRow: Element count (%d) does not match width of area (%d).", mCount,
			         mAoi.getWidth());
	...
	RawImage& OpcodeDeltaPerCol::createOutput(RawImage& in)
	{
		...
		if (in->getDataType() == TYPE_USHORT16)
		{
			...
			mDeltaX = new int[w];
			for (int i = 0; i < w; i++)
				mDeltaX[i] = static_cast<int>(65535.0f * mDelta[i] + 0.5f);
		}
		return in;
	}
	...
	OpcodeScalePerRow::OpcodeScalePerRow(const uchar8* parameters, int param_max_bytes, uint32* bytes_used)
	{
		...
		*bytes_used = 36;
		if (param_max_bytes < 36 + (mCount * 4))
			ThrowRDE("OpcodeScalePerRow: Not enough data to read parameters, only %d bytes left.", param_max_bytes);
		if (mAoi.getHeight() != mCount)
			ThrowRDE("OpcodeScalePerRow: Element count (%d) does not match height of area (%d).", mCount,
			         mAoi.getHeight());
	...
	void OpcodeScalePerRow::apply(RawImage& in, RawImage& out, int startY, int endY)
	{
		if (in->getDataType() == TYPE_USHORT16)
		{
			...
			for (int y = startY; y < endY; y += mRowPitch)
			{
				...
				// Add offset, so this is always first plane
				src += mFirstPlane;
				int delta = static_cast<int>(1024.0f * mDelta[y]);
				for (int x = 0; x < mAoi.getWidth(); x += mColPitch)
				{
					for (int p = 0; p < mPlanes; p++)
					{
	...
	OpcodeScalePerCol::OpcodeScalePerCol(const uchar8* parameters, int param_max_bytes, uint32* bytes_used)
	{
		...
		*bytes_used = 36;
		if (param_max_bytes < 36 + (mCount * 4))
			ThrowRDE("OpcodeScalePerCol: Not enough data to read parameters, only %d bytes left.", param_max_bytes);
		if (mAoi.getWidth() != mCount)
			ThrowRDE("OpcodeScalePerCol: Element count (%d) does not match width of area (%d).", mCount,
			         mAoi.getWidth());
	...
	RawImage& OpcodeScalePerCol::createOutput(RawImage& in)
	{
		...
		if (in->getDataType() == TYPE_USHORT16)
		{
			...
			mDeltaX = new int[w];
			for (int i = 0; i < w; i++)
				mDeltaX[i] = static_cast<int>(1024.0f * mDelta[i]);
		}
		return in;
	}
