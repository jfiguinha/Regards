...
namespace RawSpeed
{
	...
	void NikonDecompressor::DecompressNikon(ByteStream* metadata, uint32 w, uint32 h, uint32 bitsPS, uint32 offset,
	                                        uint32 size)
	{
		...
		for (y = 0; y < h; y++)
		{
			...
			pLeft1 = pUp1[y & 1];
			pLeft2 = pUp2[y & 1];
			dest[0] = curve[clampbits(pLeft1, 15)] | (static_cast<uint32>(curve[clampbits(pLeft2, 15)]) << 16);
			for (x = 1; x < cw; x++)
			{
				bits.checkPos();
				pLeft1 += HuffDecodeNikon(bits);
				pLeft2 += HuffDecodeNikon(bits);
				dest[x] = curve[clampbits(pLeft1, 15)] | (static_cast<uint32>(curve[clampbits(pLeft2, 15)]) << 16);
			}
		}
	}
