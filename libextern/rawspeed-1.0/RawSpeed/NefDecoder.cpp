...
namespace RawSpeed
{
	...
	RawImage NefDecoder::decodeRawInternal()
	{
		...
		{
			ThrowRDE("NEF Decoder: Byte count number does not match strip size: count:%u, strips:%u ", counts->count,
			         offsets->count);
		}
		...
		data = makertiff.RootIFD()->getIFDsWithTag(static_cast<TiffTag>(0x8c));
		...
		try
		{
			meta = data[0]->getEntry(static_cast<TiffTag>(0x96));
		}
		catch (TiffParserException)
		{
			meta = data[0]->getEntry(static_cast<TiffTag>(0x8c)); // Fall back
		}
	...
	*/
		...
		for (uint32 i = 0; i < slices.size(); i++)
		{
			...
			} catch (IOException e)
			{
				if (i > 0)
					...
					ThrowRDE("NEF decoder: IO error occurred in first slice, unable to decode more. Error is: %s",
					         e.what());
			}
			offY += slice.h;
		}
	...
	void NefDecoder::readCoolpixMangledRaw(ByteStream& input, iPoint2D& size, iPoint2D& offset, int inputPitch)
	{
		...
		w *= cpp;
		auto in = new BitPumpMSB32(&input);
		for (; y < h; y++)
		{
			auto dest = (ushort16*)&data[offset.x * sizeof(ushort16) * cpp + y * outPitch];
	...
	void NefDecoder::readCoolpixSplitRaw(ByteStream& input, iPoint2D& size, iPoint2D& offset, int inputPitch)
	{
		...
		w *= cpp;
		h /= 2;
		auto in = new BitPumpMSB(&input);
		for (; y < h; y++)
		{
			auto dest = (ushort16*)&data[offset.x * sizeof(ushort16) * cpp + y * 2 * outPitch];
			for (uint32 x = 0; x < w; x++)
			{
				dest[x] = in->getBits(12);
			}
		...
		{
			auto dest = (ushort16*)&data[offset.x * sizeof(ushort16) * cpp + (y * 2 + 1) * outPitch];
			for (uint32 x = 0; x < w; x++)
			{
				dest[x] = in->getBits(12);
			}
