...
namespace RawSpeed
{
	...
	{
		for (auto i = ownedObjects.begin(); i != ownedObjects.end(); ++i)
		{
			delete(*i);
		}
		ownedObjects.clear();
	...
	void RawDecoder::decodeUncompressed(TiffIFD* rawIFD, BitOrder order)
	{
		...
		for (uint32 i = 0; i < slices.size(); i++)
		{
			...
			iPoint2D pos(0, offY);
			bitPerPixel = static_cast<int>(static_cast<uint64>(slice.count * 8) / (slice.h * width));
			try
			{
			...
			} catch (IOException& e)
			{
				if (i > 0)
					...
					ThrowRDE("RAW decoder: IO error occurred in first slice, unable to decode more. Error is: %s",
					         e.what());
			}
			offY += slice.h;
		}
	...
	void RawDecoder::readUncompressedRaw(ByteStream& input, iPoint2D& size, iPoint2D& offset, int inputPitch,
	                                     int bitPerPixel, BitOrder order)
	{
		...
		else
		{
			...
			if (bitPerPixel == 12 && static_cast<int>(w) == inputPitch * 8 / 12 && getHostEndianness() == little)
			{
