...
#define CHECKSIZE(A) if (A > size) ThrowTPE("Error reading TIFF structure (invalid size). File Corrupt")
	...
	TiffIFD::TiffIFD(FileMap* f, uint32 offset)
	{
		...
		{
			auto t = new TiffEntry(f, offset + 2 + i * 12);

			if (t->tag == SUBIFDS || t->tag == EXIFIFDPOINTER || t->tag == DNGPRIVATEDATA || t->tag == MAKERNOTE || t->
				tag == 0x2e)
			{
				// subIFD tag
	...
	TiffIFD* TiffIFD::parseDngPrivateData(TiffEntry* t)
	{
		...
		*/
		...
		if (big == getHostEndianness())
			...
			count = static_cast<unsigned int>(data[0]) << 24 | static_cast<unsigned int>(data[1]) << 16 | static_cast<
		...
		if (big == getHostEndianness())
			...
			org_offset = static_cast<unsigned int>(data[0]) << 24 | static_cast<unsigned int>(data[1]) << 16 |
		...
		auto maker_data = new uchar8[org_offset + count];
		memcpy(&maker_data[org_offset], data, count);
		auto maker_map = new FileMap(maker_data, org_offset + count);
	...
	{
		for (auto i = mEntry.begin(); i != mEntry.end(); ++i)
		{
			delete((*i).second);
		}
		mEntry.clear();
		for (auto i = mSubIFD.begin(); i != mSubIFD.end(); ++i)
		{
			delete(*i);
		}
		mSubIFD.clear();
