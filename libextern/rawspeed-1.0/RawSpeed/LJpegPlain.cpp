...
namespace RawSpeed
{
	...
	void LJpegPlain::decodeScan()
	{
		...
		if (frame.h + offY > static_cast<uint32>(mRaw->dim.y))
		...
		for (uint32 i = 0; i < frame.cps; i++)
		{
			if (frame.compInfo[i].superH != 1 || frame.compInfo[i].superV != 1)
			{
				...
				if (pred == 1)
				{
					...
					if (frame.compInfo[0].superH == 2 && frame.compInfo[0].superV == 1 &&
						frame.compInfo[1].superH == 1 && frame.compInfo[1].superV == 1 &&
						frame.compInfo[2].superH == 1 && frame.compInfo[2].superV == 1)
					{
	...
	**/
	...
	void LJpegPlain::decodeScanLeftGeneric()
	{
		...
		*/
		...
		uint32 slices = static_cast<uint32>(slicesW.size()) * (frame.h - skipY) / maxSuperV;
		...
		int p[4];
		auto dest = (ushort16*)&draw[offset[0] & 0x0fffffff];
...
#define COMPS 3
	...
	void LJpegPlain::decodeScanLeft4_2_0()
	{
		...
		//Prepare slices (for CR2)
		uint32 slices = static_cast<uint32>(slicesW.size()) * (real_h - skipY) / 2;
		offset = new uint32[slices + 1];
	...
	void LJpegPlain::decodeScanLeft4_2_2()
	{
		...
		//Prepare slices (for CR2)
		uint32 slices = static_cast<uint32>(slicesW.size()) * (frame.h - skipY);
		offset = new uint32[slices + 1];
		...
		*dest++ = p2 = (1 << (frame.prec - Pt - 1)) + HuffDecode(dctbl2);
		...
		for (uint32 y = 0; y < (frame.h - skipY); y++)
		{
			for (; x < cw; x++)
			{
				...
				p1 += diff;
				*dest++ = static_cast<ushort16>(p1);
				//    _ASSERTE(p1 >= 0 && p1 < 65536);

				diff = HuffDecode(dctbl2);
				p2 += diff;
				*dest++ = static_cast<ushort16>(p2);
