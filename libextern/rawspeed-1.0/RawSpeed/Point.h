...
#include <stdlib.h>
	...
	public:
		...
		iPoint2D(const iPoint2D& pt)
		{
			x = pt.x;
			y = pt.y;
		}

		iPoint2D operator +=(const iPoint2D& other)
		{
			x += other.x;
			y += other.y;
			return *this;
		...
		{
			return iPoint2D(min(x, otherPoint.x), min(y, otherPoint.y));
		};
		int x, y;
	};
	...
	public:
		...
		iRectangle2D(const iRectangle2D& r)
		{
			dim = iPoint2D(r.dim);
			pos = iPoint2D(r.pos);
		}

		iRectangle2D(const iPoint2D& _pos, const iPoint2D& size)
		{
			dim = size;
			pos = _pos;
		}
		...
		iRectangle2D combine(const iRectangle2D& other) const
		{
			...
			iPoint2D br1 = getBottomRight();
			iPoint2D br2 = other.getBottomRight();
			combined.setAbsolute(min(pos.x, other.pos.x), min(pos.y, other.pos.y), max(br1.x, br2.x),
			                     max(br2.y, br2.y));
			return combined;
		};
		iPoint2D pos;
