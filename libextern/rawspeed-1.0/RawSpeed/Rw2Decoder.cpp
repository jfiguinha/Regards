...
namespace RawSpeed
{
	...
	RawImage Rw2Decoder::decodeRawInternal()
	{
		...
		TiffIFD* raw = data[0];
		uint32 height = raw->getEntry(static_cast<TiffTag>(3))->getShort();
		uint32 width = raw->getEntry(static_cast<TiffTag>(2))->getShort();
		...
		if (isOldPanasonic)
		{
			...
			if (count != static_cast<int>(width * height * 2))
		...
		// Read blacklevels
		if (raw->hasEntry(static_cast<TiffTag>(0x1c)) && raw->hasEntry(static_cast<TiffTag>(0x1d)) && raw->hasEntry(
			static_cast<TiffTag>(0x1e)))
		{
			mRaw->blackLevelSeparate[0] = raw->getEntry(static_cast<TiffTag>(0x1c))->getInt() + 15;
			mRaw->blackLevelSeparate[1] = mRaw->blackLevelSeparate[2] = raw->getEntry(static_cast<TiffTag>(0x1d))->
			                                                                 getInt() + 15;
			mRaw->blackLevelSeparate[3] = raw->getEntry(static_cast<TiffTag>(0x1e))->getInt() + 15;
		}
		return mRaw;
	}
	...
	void Rw2Decoder::decodeThreaded(RawDecoderThread* t)
	{
		...
		auto zero_hint = hints.find("zero_is_bad");
