...
namespace RawSpeed
{
	...
	{
		auto p = (unsigned short*)f->getData(offset);
		tag = static_cast<TiffTag>(p[0]);
		type = static_cast<TiffDataType>(p[1]);
  ...
  if (type == TIFF_FLOAT || type == TIFF_DOUBLE)
	...
	{
		return (type == TIFF_FLOAT || type == TIFF_DOUBLE || type == TIFF_RATIONAL || type == TIFF_SRATIONAL || type ==
			TIFF_LONG || type == TIFF_SHORT);
	}

	float TiffEntry::getFloat()
	{
		if (!(type == TIFF_FLOAT || type == TIFF_DOUBLE || type == TIFF_RATIONAL || type == TIFF_SRATIONAL || type ==
			TIFF_LONG || type == TIFF_SHORT))
			ThrowTPE("TIFF, getFloat: Wrong type 0x%x encountered. Expected Float", type);
		if (type == TIFF_DOUBLE)
		{
			return static_cast<float>(*(double*)&data[0]);
		}
		if (type == TIFF_FLOAT)
		{
			return *(float*)&data[0];
		...
		if (type == TIFF_RATIONAL)
		{
			...
			if (t[1])
				return static_cast<float>(t[0]) / t[1];
		}
		else if (type == TIFF_SRATIONAL)
		{
			auto t = (const int*)getIntArray();
			if (t[1])
				return static_cast<float>(t[0]) / t[1];
		}
		return 0.0f;
