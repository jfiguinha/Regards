...
int LibRaw::open_file(const char *fname)
{
  ...
  if ((stream->size() > static_cast<INT64>(LIBRAW_MAX_NONDNG_RAW_FILE_SIZE)) && (
        stream->size() > static_cast<INT64>(LIBRAW_MAX_DNG_RAW_FILE_SIZE)))
  {
    delete stream;
...
int LibRaw::open_file(const wchar_t *fname)
{
  ...
  if ((stream->size() > static_cast<INT64>(LIBRAW_MAX_DNG_RAW_FILE_SIZE)) && (
        stream->size() > static_cast<INT64>(LIBRAW_MAX_NONDNG_RAW_FILE_SIZE)))
  {
    delete stream;
    return LIBRAW_TOO_BIG;
  }
...
int LibRaw::open_buffer(const void *buffer, size_t size)
{
  ...
  if ((size > static_cast<INT64>(LIBRAW_MAX_DNG_RAW_FILE_SIZE)) && (
        size > static_cast<INT64>(LIBRAW_MAX_NONDNG_RAW_FILE_SIZE)))
    return LIBRAW_TOO_BIG;
                       ...
                       unsigned black_level)
{
  ...
  case 10:
  {
    ...
    if (libraw_internal_data.unpacker_data.load_flags & 1)
    {
...
#define isRIC imgdata.sizes.raw_inset_crops[0]
    ...
    if (load_raw == &LibRaw::panasonic_load_raw)
    {
      ...
      if (libraw_internal_data.unpacker_data.pana_encoding == 6)
      {
        ...
        INT64 ds = libraw_internal_data.unpacker_data.data_size;
        if (!ds)
        ...
        if ((imgdata.sizes.raw_width % 11) == 0 &&
            (static_cast<INT64>(imgdata.sizes.raw_height) * rowbytes11 == ds))
          load_raw = &LibRaw::panasonicC6_load_raw;
        else if ((imgdata.sizes.raw_width % 14) == 0 &&
                 (static_cast<INT64>(imgdata.sizes.raw_height) * rowbytes14 == ds))
          load_raw = &LibRaw::panasonicC6_load_raw;
        else
      ...
      else if (libraw_internal_data.unpacker_data.pana_encoding == 7)
      {
        ...
        if ((imgdata.sizes.raw_width % pixperblock) == 0 &&
            (static_cast<INT64>(imgdata.sizes.raw_height) * rowbytes ==
             static_cast<INT64>(libraw_internal_data.unpacker_data.data_size)))
...
#undef NIKON_14BIT_SIZE
    ...
    if (makeIs(LIBRAW_CAMERAMAKER_Sony) &&
        ...
        imgdata.color.linear_max[0] > static_cast<long>(imgdata.color.maximum) &&
        imgdata.color.linear_max[0] <= static_cast<long>(imgdata.color.maximum) * 4)
      for (int c = 0; c < 4; c++)
...
#undef isRIC
      if (imgdata.color.raw_bps < 14 && !imgdata.idata.dng_version && load_raw != &LibRaw::canon_sraw_load_raw)
      {
        ...
        if (MN.canon.SpecularWhiteLevel > xmax) // Adjust 14-bit metadata to real bps
        {
          ...
          for (int c = 0; c < 4; c++)
            imgdata.color.linear_max[c] /= div;
          for (int c = 0; c < 4; c++)
            MN.canon.ChannelBlackLevel[c] /= div;
    ...
    {
      float ratio =
          static_cast<float>(imgdata.sizes.raw_height) / static_cast<float>(imgdata.sizes.raw_width);
    ...
    if (imgdata.sizes.raw_aspect >= LIBRAW_IMAGE_ASPECT_MINIMAL_REAL_ASPECT_VALUE
        ...
        && (imgdata.sizes.raw_inset_crops[0].ctop + imgdata.sizes.raw_inset_crops[0].cheight <= imgdata.sizes.
            raw_height)
    ...
    {
      float c0_ratio = static_cast<float>(imgdata.sizes.raw_inset_crops[0].cwidth) / static_cast<float>(imgdata.sizes.
                         raw_inset_crops[0].cheight);
      float c1_ratio = static_cast<float>(imgdata.sizes.raw_aspect) / 1000.f;
      if (c0_ratio / c1_ratio < 0.98 || c0_ratio / c1_ratio > 1.02) // set crops[1]
      {
        ...
        {
          int newheight = static_cast<int>(imgdata.sizes.raw_inset_crops[0].cwidth / c1_ratio);
        ...
        {
          int newwidth = static_cast<int>(imgdata.sizes.raw_inset_crops[0].cheight * c1_ratio);
    ...
    if (load_raw == &LibRaw::nikon_yuv_load_raw) // Is it Nikon sRAW?
    {
      ...
      {
        double x = static_cast<double>(i) / 3072.;
      ...
      for (int i = 0; i < 3; i++)
        for (int j = 0; j < 4; j++)
          imgdata.color.rgb_cam[i][j] = static_cast<float>(i == j);
    }
        ...
        (!C.linear_max[0]) && (C.maximum > 1024) && (load_raw != &LibRaw::nikon_load_sraw))
    {
      C.linear_max[0] = C.linear_max[1] = C.linear_max[2] = C.linear_max[3] =
                                                            static_cast<long>(static_cast<float>((C.maximum)) / 1.07f);
    }
    ...
    )
    {
      for (int cnt = LIBRAW_WBI_Unknown; cnt <= LIBRAW_WBI_StudioTungsten; cnt++)
      {
        ...
        {
          C.WB_Coeffs[cnt][0] = static_cast<int>(static_cast<float>((C.WB_Coeffs[cnt][0])) * 1.0503f);
          C.WB_Coeffs[cnt][2] = static_cast<int>(static_cast<float>((C.WB_Coeffs[cnt][2])) * 2.2867f);
        }
      }
