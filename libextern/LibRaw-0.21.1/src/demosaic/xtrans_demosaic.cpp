...
void LibRaw::xtrans_interpolate(int passes)
{
  ...
  for (int row = 0; row < 6; row++)
    for (int col = 0; col < 6; col++)
      cstat[static_cast<unsigned>(fcol(row, col))]++;
  ...
  ndir = 4 << static_cast<int>(passes > 1);
  ...
  for (int row = 2; row < height - 2; row++)
  {
    int col;
    ushort min, max;
    for (col = 2, max = 0u, min = 0xffffu; col < static_cast<int>(width) - 2; col++)
    {
