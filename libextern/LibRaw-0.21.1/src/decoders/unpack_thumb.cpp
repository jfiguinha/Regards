...
{
  auto myerr = (jpegErrorManager *)cinfo->err;
...
#define Tformat libraw_internal_data.unpacker_data.thumb_format
  ...
  try
  {
    ...
    if ((Tformat >= LIBRAW_INTERNAL_THUMBNAIL_KODAK_THUMB)
...
#endif
    if (ID.toffset + static_cast<INT64>(T.tlength) < 1)
      throw LIBRAW_EXCEPTION_IO_CORRUPT;

    if (ID.toffset + static_cast<INT64>(T.tlength) >
    ...
    if (Tformat == LIBRAW_INTERNAL_THUMBNAIL_JPEG)
    {
      ...
      if (T.thumb)
        free(T.thumb);
      T.thumb = static_cast<char *>(malloc(T.tlength));
      ID.input->read(T.thumb, 1, T.tlength);
      auto tthumb = (unsigned char *)T.thumb;
    ...
    if (Tformat == LIBRAW_INTERNAL_THUMBNAIL_LAYER)
    {
      ...
      if (T.thumb)
        free(T.thumb);
      T.thumb = static_cast<char *>(calloc(colors, tlength));
      auto tbuf = static_cast<unsigned char *>(calloc(colors, tlength));
    ...
    if (Tformat == LIBRAW_INTERNAL_THUMBNAIL_ROLLEI)
    {
      int i;
      THUMB_SIZE_CHECKWH(T.twidth, T.theight);
      int tlength = T.twidth * T.theight;
      if (T.thumb)
        free(T.thumb);
      T.tcolors = 3;
      T.thumb = static_cast<char *>(calloc(T.tcolors, tlength));
      auto tbuf = static_cast<unsigned short *>(calloc(2, tlength));
    ...
    if (Tformat == LIBRAW_INTERNAL_THUMBNAIL_PPM)
    {
      if (t_bytesps > 1)
      ...
      if (T.tlength &&
          static_cast<int>(T.tlength) < t_length) // try to find tiff ifd with needed offset
      {
            ...
            tiff_ifd[pifd].strip_byte_counts_count)
        {
          ...
          if (total_size != static_cast<unsigned>(t_length)) // recalculate colors
          {
    ...
    if (Tformat == LIBRAW_INTERNAL_THUMBNAIL_PPM16)
    {
      if (t_bytesps > 2)
      ...
      auto t_thumb = static_cast<ushort *>(calloc(i_length, 1));
      ...
      {
        T.thumb = static_cast<char *>(malloc(o_length));
