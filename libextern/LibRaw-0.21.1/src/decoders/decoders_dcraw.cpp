...
#define reset tls->getbits.reset
  ...
  if (nbits == 0 || vbits < 0)
    return 0;
  while (!reset && vbits < nbits && (c = fgetc(ifp)) != static_cast<unsigned>(EOF) &&
         !(reset = zero_after_ff && c == 0xff && fgetc(ifp)))
  {
    bitbuf = (bitbuf << 8) + static_cast<uchar>(c);
    vbits += 8;
  }
...
ushort *LibRaw::make_decoder_ref(const uchar **source)
{
  ...
  huff = static_cast<ushort *>(calloc(1 + (1 << max), sizeof *huff));
  huff[0] = max;
...
int LibRaw::canon_has_lowbits()
{
  ...
  for (i = 540; i < static_cast<int>(sizeof test - 1); i++)
    if (test[i] == 0xff)
    {
      if (test[i + 1])
        return 1;
...
int LibRaw::ljpeg_start(struct jhead *jh, int info_only)
{
  ...
  jh->row = static_cast<ushort *>(calloc(jh->wide * jh->clrs, 16));
  ...
  try
  {
    for (jrow = 0; jrow < jh.high; jrow++)
    {
      ...
      for (jcol = 0; jcol < jwide; jcol++)
      {
        ...
        if (row > raw_height)
          throw LIBRAW_EXCEPTION_IO_CORRUPT;
        if (static_cast<unsigned>(row) < raw_height)
          RAW(row, col) = val;
        if (++col >= raw_width)
          col = (row++, 0);
      }
  ...
  try
  {
    ...
    if (!(imgdata.rawparams.specials & LIBRAW_RAWSPECIAL_SRAW_NO_RGB))
      for (; rp < ip[0]; rp += 4)
      {
        ...
        FORC3
          rp[c] = CLIP15(pix[c] * sraw_mul[c] >> 10);
      }
  }
  catch (...)
  {
...
void LibRaw::ljpeg_idct(struct jhead *jh)
{
  ...
  FORC(8)
    work[0][0][c] *= static_cast<float>(M_SQRT1_2);
  FORC(8)
    work[0][c][0] *= static_cast<float>(M_SQRT1_2);
  for (i = 0; i < 8; i++)
    for (j = 0; j < 8; j++)
      FORC(8)
    ...
    for (min = row = 0; row < height; row++)
    {
      ...
      for (col = 0; col < raw_width; col++)
      {
        ...
        if (col < 2)
          ...
          hpred[col & 1] += diff;
        if (static_cast<ushort>(hpred[col & 1] + min) >= max)
          derror();
  ...
  for (row = 0; row < raw_height; row++)
  {
    ...
    for (col = 0; col < raw_width; col++)
    {
      if (!(b = col & 1))
      {
        bitbuf = 0;
        FORC(6)
          bitbuf |= static_cast<UINT64>(fgetc(ifp)) << c * 8;
        FORC(4)
  ...
  else
  {
    for (row = 0; row < raw_height; row++)
    {
      ...
      for (col = 0; col < raw_width; col++)
      {
        ...
        if (i % 3 == 2)
          sh = 4 >> (3 - pana_data(2, nullptr));
        if (nonz[i & 1])
        {
          if ((j = pana_data(8, nullptr)))
          {
        ...
        else if ((nonz[i & 1] = pana_data(8, nullptr)) || i > 11)
          pred[i & 1] = nonz[i & 1] << 4 | pana_data(4, nullptr);
...
void LibRaw::olympus_load_raw()
{
  ...
  for (row = 0; row < height; row++)
  {
    ...
    for (col = 0; col < raw_width; col++)
    {
      carry = acarry[col & 1];
      i = 2 * (carry[2] < 3);
      for (nbits = 2 + i; static_cast<ushort>(carry[0]) >> (nbits + i); nbits++);
  ...
  for (rb = 0; rb < 2; rb++)
    for (row = 2 + rb; row < height + 2; row += 2)
    {
      ...
      for (col = 3 - (row & 1); col < width + 2; col += 2)
      {
        ...
        {
          val = ABS(pixel[(row - 2) * 644 + col] - pixel[row * 644 + col - 2]) + ABS(
                    pixel[(row - 2) * 644 + col] - pixel[(row - 2) * 644 + col - 2]) +
...
void LibRaw::sony_arw2_load_raw()
{
  ...
  data = static_cast<uchar *>(malloc(raw_width + 1));
  try
  {
    for (row = 0; row < height; row++)
    {
  ...
  for (row = 0; row < raw_height; row++)
  {
    ...
    for (col = 0; col < raw_width; col += 16)
    {
      ...
      for (c = 0; c < 16; c += 2)
      {
        ...
        unsigned isrc = (dir
                           ? RAWINDEX(row + (~c | -2), col + c)
                           : col
                           ? RAWINDEX(row, col + (c | -2))
                           : 0);
        if (idest < maxpixels &&
            isrc <
            maxpixels) // less than zero is handled by unsigned conversion
          RAW(row, col + c) = (i > 0 ? (static_cast<signed>(ph1_bits(i)) << (32 - i) >> (32 - i)) : 0) +
