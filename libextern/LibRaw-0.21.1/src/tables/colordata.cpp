  ...
  static const struct
  {
    ...
    { LIBRAW_CAMERAMAKER_RaspberryPi, "ov5647", 16, 0x3ff,
      { 12782,-4059,-379,-478,9066,1413,1340,1513,5176 } }, /* DJC */
          ...
          {LIBRAW_CAMERAMAKER_Minolta, "DG-5D", 0, 0xffb,
           // same CamID: "ALPHA 5D", "DYNAX 5D", "MAXXUM 5D", "Alpha Sweet Digital"
          ...
          {LIBRAW_CAMERAMAKER_Olympus, "E-M10 Mark IV", 0, 0,
           {9476, -3182, -765, -2613, 10958, 1893, -449, 1315, 5268}},
          {LIBRAW_CAMERAMAKER_Olympus, "E-M10", 0, 0,
           // Same CMs: E-M10, E-M10 Mark II, E-M10 Mark III; "CLAUSS piX 5oo"
           {8380, -2630, -639, -2887, 10725, 2496, -627, 1427, 5438}},
          ...
          {LIBRAW_CAMERAMAKER_Olympus, "STYLUS 1", 0, 0,
           // Olympus "STYLUS 1 and STYLUS 1s have the same CamID, cameras are slightly different
          ...
          {LIBRAW_CAMERAMAKER_Sony, "DSC-RX100M", -800, 0,
           // same CMs: DSC-RX100M2, DSC-RX100M3, DSC-RX100M4, DSC-RX100M5
           {6596, -2079, -562, -4782, 13016, 1933, -970, 1581, 5181}},
    ...
    if (table[i].m_idx == make_idx)
    {
      ...
      if (!l || !strncasecmp(t_model, table[i].prefix, l))
      {
        if (!dng_version)
        {
          ...
          {
            black = static_cast<ushort>(table[i].t_black);
          ...
          if (table[i].t_maximum)
            maximum = static_cast<ushort>(table[i].t_maximum);
        }
        if (table[i].trans[0])
        {
...
void LibRaw::simple_coeff(int index)
{
  ...
  for (raw_color = i = 0; i < 3; i++)
    FORCC
      rgb_cam[i][c] = table[index][i * MIN(colors, 4) + c];
}
