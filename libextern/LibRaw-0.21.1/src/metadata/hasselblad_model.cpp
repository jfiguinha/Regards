...
{
  for (int i = 0; i < static_cast<int>(sizeof HassyRawFormat / sizeof *HassyRawFormat); i++)
    if (static_cast<unsigned>(HassyRawFormat[i].idx) == idx)
...
void LibRaw::parseHassyModel()
{
  ...
  if (imHassy.format == LIBRAW_HF_AdobeDNG)
  {
    // Adobe DNG, use LocalizedCameraModel
    ...
    if ((ps = strrchr(imgdata.color.LocalizedCameraModel, '-')))
      ...
      c = static_cast<int>(strlen(imgdata.color.LocalizedCameraModel));
  ...
  else
  {
    if ((ps = strrchr(imgdata.color.UniqueCameraModel, '/')))
    {
      ...
      c = static_cast<int>(strlen(imgdata.color.UniqueCameraModel));
    int cc = MIN(c, (int)sizeof(tmp_model)-1);
  ...
  FORC(static_cast<int>(sizeof Hasselblad_Ctrl / sizeof *Hasselblad_Ctrl))
  {
  ...
  if (!imHassy.HostBody[0])
  {
    ...
    if (ps)
    {
      // check if model contains both host body and sensor version, resolution, MS info
  ...
  {
    FORC(static_cast<int>(sizeof Hasselblad_SensorEnclosures / sizeof *Hasselblad_SensorEnclosures))
    {
      if (strcasestr(model, Hasselblad_SensorEnclosures[c]))
      {
        if (add_MP_toName)
          strcpy(model, Hasselblad_SensorEnclosures[c]);
        ilm.CameraMount = LIBRAW_MOUNT_DigitalBack;
        break;
      }
    }
...
#define cpynorm(str)                \
           ...
           (imHassy.CoatingCode == 5))
  {
    ...
    cpynorm("100-17-Coated5");
           ...
           ((raw_width == 4080) && (raw_height == 4080)))
  {
    // Phocus crop
    strcpy(imHassy.Sensor, "-16");
    cpynorm("16-Uncoated");
    if (!imHassy.SensorCode)
      imHassy.SensorCode = 4;
  ...
  else if ((raw_width == 5568) && (raw_height == 3648))
  {
    strcpy(imHassy.Sensor, "-20c");
           ...
           ((raw_width == 4080) && (raw_height == 5440)))
  {
    // Phocus crop
    strcpy(imHassy.Sensor, "-22");
    cpynorm("22-Uncoated");
    if (!imHassy.SensorCode)
      imHassy.SensorCode = 6;

  }
  else if (((raw_width == 6542) && (raw_height == 4916)) ||
           ((raw_width == 6504) && (raw_height == 4880)) || // Adobe crop
           ((raw_width == 6496) && (raw_height == 4872)))
  {
    // Phocus crop
    strcpy(imHassy.Sensor, "-31");
    cpynorm("31-Uncoated");
    if (!imHassy.SensorCode)
      imHassy.SensorCode = 8;
           ...
           ((nPix >= 7212 * 5412) && (nPix < 7304 * 5478)))
  {
    ...
    if (!strncmp(model, "H3D", 3))
    {
      ...
      cpynorm("39-Uncoated");
           ...
           ((raw_width == 7304) && (raw_height == 5478)))
  {
    // Phocus crop
    strcpy(imHassy.Sensor, "-40");
    ...
    else
    {
      ...
      if (!imHassy.CoatingCode)
        imHassy.CoatingCode = 5;
           ...
           ((raw_width == 8176) && (raw_height == 6132)))
  {
    // Phocus crop
    strcpy(imHassy.Sensor, "-50");
    ...
    else
    {
      ...
      if (!strncmp(model, "H3D", 3))
      {
        ...
        add_MP_toName = 0;
           ...
           ((raw_width == 8272) && (raw_height == 6200)))
  {
    // Phocus crop
    cpynorm("50-15-Coated5");
    if (!imHassy.SensorCode)
      imHassy.SensorCode = 15;
    if (!imHassy.CoatingCode)
      imHassy.CoatingCode = 5;
    strcpy(imHassy.Sensor, "-50c");
    if ((raw_width == 8384) ||
        ...
        !strncmp(imHassy.CaptureSequenceInitiator, "CFV II", 6))
    {
      ...
      else
      {
        strcpy(model, "X1D-50c");
           ...
           ((raw_width == 8956) && (raw_height == 6708)))
  {
    // Phocus crop
    strcpy(imHassy.Sensor, "-60");
    cpynorm("60-Coated");
    if (!imHassy.SensorCode)
      imHassy.SensorCode = 12;
    if (!imHassy.CoatingCode)
      imHassy.CoatingCode = 4;
           ...
           ((nPix >= 10320 * 7752) && (nPix < 10520 * 8000)))
  {
    ...
    cpynorm("80-Coated");
           ...
           ((raw_width == 11600) && (raw_height == 8700)))
  {
    // Phocus crop
    strcpy(imHassy.Sensor, "-100c");
    cpynorm("100-17-Coated5");
    if (!imHassy.SensorCode)
      imHassy.SensorCode = 17;
    if (!imHassy.CoatingCode)
      imHassy.CoatingCode = 5;
