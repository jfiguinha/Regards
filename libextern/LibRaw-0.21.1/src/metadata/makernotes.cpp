  ...
  while (entries--)
  {
    ...
    else if (tag == 0x0120)
    {
      ...
      if ((len >= tblsz) && (len % 3 == 0) && len / 3 <= tblsz)
      {
        for (i = 0; i < (len / 3); i++)
        {
          icWBC[wb_table1[i]][0] = static_cast<int>(getreal(type) * 10000.0);
          icWBC[wb_table1[i]][1] = icWBC[wb_table1[i]][3] = static_cast<int>(getreal(type) * 10000.0);
          icWBC[wb_table1[i]][2] = static_cast<int>(getreal(type) * 10000.0);
        }
      }
    }
  ...
  while (entries--)
  {
    ...
    if (!strncmp(make, "Canon", 5))
    {
      if (tag == 0x000d && len < 256000)
      {
        ...
        {
          CanonCameraInfo = static_cast<uchar *>(malloc(MAX(16, len)));
          fread(CanonCameraInfo, len, 1, ifp);
        }
        else
        {
          CanonCameraInfo = static_cast<uchar *>(malloc(MAX(16, len * 4)));
