...
#define plibs imgdata.shootinginfo.InternalBodySerial
  ...
  if (strnlen(plibs, len) == 13)
  {
    ...
    plibs[3] = plibs[14] = ' ';
    plibs[8] = plibs[11] = '/';
    if ((static_cast<short>((plibs[3] - '0')) * 10 + static_cast<short>((plibs[4] - '0'))) < 70)
    {
      memcpy(plibs + 4, "20", 2);
    }
    else
...
void LibRaw::parseLeicaMakernote(int base, int uptag, unsigned MakernoteTagType)
{
  ...
  else
  {
    ...
    LeicaMakernoteSignature = (static_cast<uchar>(buf[6]) << 8) | static_cast<uchar>(buf[7]);
  ...
  while (entries--)
  {
             ...
             (LeicaMakernoteSignature == 0x1000))   // "X-U (Typ 113)"
    {
      if (tag == 0x040d)
      {
        ci = fgetc(ifp);
        cj = fgetc(ifp);
        imgdata.shootinginfo.ExposureMode = (static_cast<ushort>(ci) << 8) | cj;
      }
    }
