...
{
  if ((in == static_cast<ushort>(0xffe0)) || (in == static_cast<ushort>(0x7fff)))
    return 0.0f;
  return libraw_powf64l(2.f, static_cast<float>(in) / 64.f);
}
...
static float _CanonConvertEV(short in)
{
  ...
  else if (Frac == 0x14)
  {
    ...
    Frac_f = static_cast<float>(Frac);

  return (static_cast<float>(Sign) * (static_cast<float>(EV) + Frac_f)) / 32.0f;
}
                                    ...
                                    unsigned maxlen, unsigned type, unsigned dng_writer)
{
  ...
  if (tagtypeIs(LIBRAW_EXIFTAG_TYPE_LONG))
  {
    if ((maxlen == 94) || (maxlen == 138) || (maxlen == 148) ||
      ...
      imCommon.CameraTemperature = static_cast<float>(sget4(CameraInfo + ((maxlen - 3) << 2)));
    else if (maxlen == 72)
      imCommon.CameraTemperature = static_cast<float>(sget4(CameraInfo + ((maxlen - 1) << 2)));
    else if ((maxlen == 85) || (maxlen == 93))
      imCommon.CameraTemperature = static_cast<float>(sget4(CameraInfo + ((maxlen - 2) << 2)));
    else if ((maxlen == 96) || (maxlen == 104))
      imCommon.CameraTemperature = static_cast<float>(sget4(CameraInfo + ((maxlen - 4) << 2)));
  }
  ...
  if (iMakernotesFlip && (CameraInfo[iMakernotesFlip] < 3))
  {
    ...
    // printf ("==>> iMakernotesFlip: 0x%x, flip: %d\n", iMakernotesFlip, imCanon.MakernotesFlip);
           ...
           (mgck == 0xaaaa) && (dng_writer == nonDNG))
  {
    // CameraOrientation
    int c, i;
    for (i = 2; (sget2(CameraInfo + i) != 0xbbbb) && i < static_cast<int>(maxlen); i++);
    i += 2;
    while (i < static_cast<int>(maxlen - 5))
    {
  ...
  if (iHTP)
  {
    ...
    {
      imCommon.ExposureCalibrationShift -= static_cast<float>(imCanon.HighlightTonePriority);
    }
  }
  if (iALO)
  {
  ...
  {
    if (iCanonLens + 64 >= static_cast<int>(maxlen)) // broken;
      return;
...
void LibRaw::Canon_CameraSettings(unsigned len)
{
  ...
  if (ilm.FocalUnits > 1)
  {
    ilm.MaxFocal /= static_cast<float>(ilm.FocalUnits);
    ilm.MinFocal /= static_cast<float>(ilm.FocalUnits);
  }
...
void LibRaw::Canon_WBpresets(int skip1, int skip2)
{
  int c;
  FORC4
    imgdata.color.WB_Coeffs[LIBRAW_WBI_Daylight][RGGB_2_RGBG(c)] = get2();

  if (skip1)
    fseek(ifp, skip1, SEEK_CUR);
  FORC4
    imgdata.color.WB_Coeffs[LIBRAW_WBI_Shade][RGGB_2_RGBG(c)] = get2();

  if (skip1)
    fseek(ifp, skip1, SEEK_CUR);
  FORC4
    imgdata.color.WB_Coeffs[LIBRAW_WBI_Cloudy][RGGB_2_RGBG(c)] = get2();

  if (skip1)
    fseek(ifp, skip1, SEEK_CUR);
  FORC4
    imgdata.color.WB_Coeffs[LIBRAW_WBI_Tungsten][RGGB_2_RGBG(c)] = get2();

  if (skip1)
    fseek(ifp, skip1, SEEK_CUR);
  FORC4
    imgdata.color.WB_Coeffs[LIBRAW_WBI_FL_W][RGGB_2_RGBG(c)] = get2();

  if (skip2)
    fseek(ifp, skip2, SEEK_CUR);
  FORC4
    imgdata.color.WB_Coeffs[LIBRAW_WBI_Flash][RGGB_2_RGBG(c)] = get2();
...
void LibRaw::Canon_WBCTpresets(short WBCTversion)
{
  ...
  else if (WBCTversion == 2)
  {
    ...
    else if (imCanon.ColorDataSubVer == 0xfffd)
    {
      ...
      {
        fseek(ifp, 2, SEEK_CUR);
        norm = static_cast<signed short>(get2());
        norm = 512.0f + norm / 8.0f;
        icWBCCTC[i][2] = icWBCCTC[i][4] =
                         1.0f;
        icWBCCTC[i][1] = static_cast<float>(get2());
        if (norm > 0.001f)
          icWBCCTC[i][1] /= norm;
        icWBCCTC[i][3] = static_cast<float>(get2());
        if (norm > 0.001f)
          icWBCCTC[i][3] /= norm;
        icWBCCTC[i][0] = get2();
      }
...
#define CR3_ColorData(offset)                                \
  ...
  else if (tag == 0x0004)
  {
    ...
    if (dng_writer == nonDNG)
    {
      ...
      {
        imCommon.real_ISO = floorf(
            100.f * libraw_powf64l(2.f, static_cast<float>(imCanon.ISOgain[0] + imCanon.ISOgain[1]) / 32.f - 5.f));
      ...
      {
        shutter = libraw_powf64l(2.f, static_cast<float>(static_cast<short>(i)) / -32.0f);
      }
    ...
    tempAp = get2();
    if (tempAp != 0)
      imCommon.CameraTemperature = static_cast<float>(tempAp - 128);
    tempAp = get2();
    if (tempAp != -1)
      imCommon.FlashGN = static_cast<float>(tempAp) / 32;
    get2();

    imCommon.FlashEC = _CanonConvertEV(static_cast<signed short>(get2()));
  ...
  else if ((tag == 0x0029) && (dng_writer == nonDNG))
  {
    // PowerShot G9
    int Got_AsShotWB = 0;
    ...
    for (unsigned linenum = 0; linenum < Canon_G9_linenums_2_StdWBi.size(); linenum++)
    {
      ...
      {
        FORC4
          icWBC[Canon_G9_linenums_2_StdWBi[linenum]][GRBG_2_RGBG(c)] = get4();
        if (Canon_wbi2std[imCanon.wbi] == Canon_G9_linenums_2_StdWBi[linenum])
        {
          FORC4
            cam_mul[c] = static_cast<float>(icWBC[Canon_G9_linenums_2_StdWBi[linenum]][c]);
          Got_AsShotWB = 1;
        }
      }
  ...
  else if ((tag == 0x0081) && (dng_writer == nonDNG))
  {
    // -1D, -1Ds
    data_offset = get4();
  ...
  else if ((tag == 0x00a4) && (dng_writer == nonDNG))
  {
    // -1D, -1Ds
  ...
  else if (tag == 0x00a9)
  {
    ...
    FORC4
      imgdata.color.WB_Coeffs[LIBRAW_WBI_Auto][RGGB_2_RGBG(c)] = get2();
    Canon_WBpresets(0, 0);
  ...
  else if (tag == 0x4001 && len > 500)
  {
    ...
    case 653:
      ...
      FORC4
        icWBC[LIBRAW_WBI_Custom3][RGGB_2_RGBG(c)] = get2();
    ...
    case 5120: // G10, G11, G12, G15, G16
      ...
      // M5, M100, M6
      {
        ...
        fseek(ifp, 8, SEEK_CUR);
        FORC4
          icWBC[LIBRAW_WBI_Measured][RGGB_2_RGBG(c)] =
              get2();
        fseek(ifp, 8, SEEK_CUR);
        FORC4
          icWBC[LIBRAW_WBI_Other][RGGB_2_RGBG(c)] = get2();
        fseek(ifp, 8, SEEK_CUR);
        Canon_WBpresets(8, 24);
        fseek(ifp, 168, SEEK_CUR);
        FORC4
          icWBC[LIBRAW_WBI_FL_WW][RGGB_2_RGBG(c)] = get2();
        fseek(ifp, 24, SEEK_CUR);
  ...
  else if (tag == 0x4013)
  {
    ...
    float a = static_cast<float>(get4());
    float b = static_cast<float>(get4());
    if (fabsf(b) > 0.001f)
