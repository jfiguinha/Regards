...
#define numel (sizeof(code) / sizeof(code[0]))
  int i;
  for (i = 0; i < static_cast<int>(numel); i++)
  {
    if (code[i] == c)
    {
      iso_speed = value[i];
...
void LibRaw::PentaxLensInfo(unsigned long long id, unsigned len) // tag 0x0207
{
  ushort iLensData = 0;
  uchar *table_buf;
  table_buf = static_cast<uchar *>(malloc(MAX(len, 128)));
         ...
         (table_buf[20] == 0xff)))))
  {
    ...
    if (ilm.LensID == LIBRAW_LENS_NOT_SET)
      ilm.LensID = (static_cast<unsigned>(table_buf[0]) << 8) + table_buf[1];
  }
  else
    switch (len)
    {
    case 90: // LensInfo3
      iLensData = 13;
      if (ilm.LensID == LIBRAW_LENS_NOT_SET)
        ilm.LensID = (static_cast<unsigned>((table_buf[1] & 0x0f) + table_buf[3]) << 8) +
                     table_buf[4];
      break;
    case 91: // LensInfo4
      iLensData = 12;
    ...
    case 128:
      iLensData = 15;
      if (ilm.LensID == LIBRAW_LENS_NOT_SET)
        ilm.LensID = (static_cast<unsigned>((table_buf[1] & 0x0f) + table_buf[4]) << 8) +
                     table_buf[5];
      break;
    ...
    default:
      if (id >= 0x12b9cULL) // LensInfo2
      {
        iLensData = 4;
        if (ilm.LensID == LIBRAW_LENS_NOT_SET)
          ilm.LensID = (static_cast<unsigned>((table_buf[0] & 0x0f) + table_buf[2]) << 8) +
                       table_buf[3];
      }
    }
  if (iLensData)
  {
    ...
    if (table_buf[iLensData + 10] & 0xf0)
      ilm.MaxAp4CurFocal = libraw_powf64l(
          2.0f, static_cast<float>((table_buf[iLensData + 10] & 0xf0) >> 4) / 4.0f);
    if (table_buf[iLensData + 10] & 0x0f)
      ilm.MinAp4CurFocal = libraw_powf64l(
          2.0f, static_cast<float>((table_buf[iLensData + 10] & 0x0f) + 10) / 4.0f);
    ...
    if (iLensData != 12)
    {
      ...
      if (table_buf[iLensData] & 0x70)
        ilm.LensFStops =
            static_cast<float>(((table_buf[iLensData] & 0x70) >> 4) ^ 0x07) / 2.0f +
            5.0f;

      ilm.MinFocusDistance = static_cast<float>(table_buf[iLensData + 3] & 0xf8);
      ilm.FocusRangeIndex = static_cast<float>(table_buf[iLensData + 3] & 0x07);

      if ((table_buf[iLensData + 14] > 1) && (fabs(ilm.MaxAp4CurFocal) < 0.7f))
        ilm.MaxAp4CurFocal = libraw_powf64l(
            2.0f, static_cast<float>((table_buf[iLensData + 14] & 0x7f) - 1) / 32.0f);
    }
    ...
    {
      ilm.MaxAp4CurFocal = libraw_powf64l(
          2.0f, static_cast<float>((table_buf[iLensData + 15] & 0x7f) - 1) / 32.0f);
    }
  }
  free(table_buf);
}
                                   ...
                                   unsigned len, unsigned dng_writer)
{
  ...
  else if (tag == 0x000f)
  {
    ...
    {
      imPentax.AFPointsInFocus = static_cast<unsigned>(get2());
  ...
  {
    ilm.CurAp = static_cast<float>(get2()) / 10.0f;
  }
  else if (tag == 0x0014)
  {
    PentaxISO(get2());
  ...
  {
    ilm.CurFocal = static_cast<float>(get4()) / 100.0f;
  }
  else if (tag == 0x0034)
  {
    uchar uc;
  ...
  {
    unsigned a = static_cast<unsigned>(fgetc(ifp)) << 8;
    ilm.LensID = a | fgetc(ifp);
  }
  else if (tag == 0x0047)
  {
    imCommon.CameraTemperature = static_cast<float>(fgetc(ifp));
  }
  else if (tag == 0x004d)
  {
    if (tagtypeIs(LIBRAW_EXIFTAG_TYPE_SLONG))
      ...
      imCommon.FlashEC = static_cast<float>(static_cast<signed short>(fgetc(ifp))) / 6.0f;
  }
  else if (tag == 0x005c)
  {
    fgetc(ifp);
    imgdata.shootinginfo.ImageStabilization = static_cast<short>(fgetc(ifp));
  }
  else if (tag == 0x0072)
  {
  ...
  {
    short a = static_cast<short>(fgetc(ifp));
    switch (a)
    {
    case 0:
  ...
  else if ((tag == 0x0203) && (dng_writer == nonDNG))
  {
    for (int i = 0; i < 3; i++)
      FORC3
        cmatrix[i][c] = static_cast<short>(get2()) / 8192.0;
  }
  else if (tag == 0x0205)
  {
  ...
  {
    FORC4
      icWBC[Pentax_wb_list1[tag - 0x020d]][RGGB_2_RGBG(c)] = get2();
  }
  ...
  else if (tag == 0x021f)
  {
        ...
        (unique_id != PentaxID_K_1_Mark_II))
    {
      ...
      imPentax.AFPointsInFocus = static_cast<unsigned>(fgetc(ifp));
      if (!imPentax.AFPointsInFocus)
        imPentax.AFPointsInFocus = 0xffffffff;
      else
  ...
  {
    int nWB = get2();
    if (nWB <= static_cast<int>(sizeof(icWBCCTC) / sizeof(icWBCCTC[0])))
      FORC(nWB)
      {
        icWBCCTC[c][0] = static_cast<unsigned>(0xcfc6) - get2();
  ...
  else if (tag == 0x022d)
  {
    int wb_ind;
    getc(ifp);
    for (int wb_cnt = 0; wb_cnt < static_cast<int>(Pentax_wb_list2.size()); wb_cnt++)
    {
      wb_ind = getc(ifp);
      if (wb_ind >= 0 && wb_ind < static_cast<int>(Pentax_wb_list2.size()))
        FORC4
          icWBC[Pentax_wb_list2[wb_ind]][RGGB_2_RGBG(c)] = get2();
    }
  }
