...
void LibRaw::getOlympus_CameraType2()
{
  ...
  {
    if (strcmp(model, "SP510UZ"))
      OlyID = OlyID_SP_510UZ;
    else
      OlyID = 0x0ULL;
  }
  unique_id = OlyID;
...
void LibRaw::getOlympus_SensorTemperature(unsigned len)
{
  if (OlyID != 0x0ULL)
  {
    ...
    if ((OlyID == OlyID_E_1) ||
        (OlyID == OlyID_E_M5) ||
        (len != 1))
      imCommon.SensorTemperature = static_cast<float>(temp);
    else if ((temp != -32768) && (temp != 0))
    {
      if (temp > 199)
        imCommon.SensorTemperature = 86.474958f - 0.120228f * static_cast<float>(temp);
      else
        imCommon.SensorTemperature = static_cast<float>(temp);
    }
  }
                                    ...
                                    unsigned dng_writer)
{
  ...
  case 0x0201:
  {
    ...
    fread(bits, 1, 4, ifp);
    ilm.LensID = static_cast<unsigned long long>(bits[0]) << 16 |
                 static_cast<unsigned long long>(bits[2]) << 8 | static_cast<unsigned long long>(bits[3]);
  ...
  case 0x0206:
    ...
    break;
  case 0x0207:
    ilm.MinFocal = static_cast<float>(get2());
    break;
  case 0x0208:
    ilm.MaxFocal = static_cast<float>(get2());
                                         ...
                                         unsigned len, unsigned dng_writer)
{
  ...
  case 0x0600:
    ...
    for (c = 1; c < static_cast<int>(len) && c < 5; c++)
    {
                                          ...
                                          unsigned len, unsigned dng_writer)
{
  ...
  else if (tag == 0x011f)
  {
    ...
    FORC4
      if (icWBC[LIBRAW_WBI_Custom1 + c][0])
           ...
           strcmp(software, "v757-71"))
  {
    for (i = 0; i < 3; i++)
    {
      ...
      {
        FORC3
          cmatrix[i][c] = static_cast<short>(get2()) / 256.0;
      }
      else
      {
        FORC3
          imgdata.color.ccm[i][c] = static_cast<short>(get2()) / 256.0;
      }
    }
  }
  ...
  else if ((tag == 0x0805) && (len == 2))
  {
    ...
    if ((dng_writer == nonDNG) && (OlyID != OlyID_XZ_1))
      FORC4
        imgdata.color.linear_max[c] = imOly.SensorCalibration[0];
  }
  else if (tag == 0x1112)
  {
    sorder = order;
  ...
  else if (tag == 0x1306)
  {
    ...
    {
      if (c < 61)
        imCommon.CameraTemperature = static_cast<float>(c);
      else
        imCommon.CameraTemperature = static_cast<float>(c - 32) / 1.8f;
...
void LibRaw::parseOlympusMakernotes(int base, unsigned tag, unsigned type, unsigned len, unsigned dng_writer)
{
  ...
  else
  {
    ...
    case 0x0200:
      FORC3
        if ((imOly.SpecialMode[c] = get4()) >= 0xff)
          imOly.SpecialMode[c] = 0xffffffff;
      break;
    case 0x0207:
      getOlympus_CameraType2();
      break;
    ...
    case 0x1002:
      ...
      break;
    case 0x1007:
      imCommon.SensorTemperature = static_cast<float>(get2());
      break;
    case 0x1008:
      imCommon.LensTemperature = static_cast<float>(get2());
      break;
    case 0x100b:
    ...
    case 0x1038:
      ...
      break;
    case 0x103b:
      if (imOly.FocusStepInfinity == 0xffff)
    ...
    case 0x20500305:
      a = get4();
    /*b = */
      get4(); // b is not used, so removed
      if (a >= 0x7f000000)
        imOly.FocusDistance = -1.0; // infinity
      else
        imOly.FocusDistance = static_cast<double>(a) / 1000.0; // convert to meters
      break;
    case 0x20500308:
      imOly.AFPoint = get2();
      break;
