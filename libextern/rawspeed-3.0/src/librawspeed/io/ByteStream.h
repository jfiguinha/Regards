...
protected:
  size_type pos = 0;
  // position of stream in bytes (this is next byte to deliver)

public:
  ByteStream() = default;

  explicit ByteStream(const DataBuffer& buffer)
    : DataBuffer(buffer) {
  }
  ...
  [[nodiscard]] size_type getRemainSize() const {
    assert(size >= pos);
    (void)check(0);
    return size - pos;
  }
  ...
  [[nodiscard]] ByteStream peekStream(size_type nmemb,
                                      size_type size_) const {
    if (size_ && nmemb > std::numeric_limits<size_type>::max() / size_)
      ThrowIOE("Integer overflow when calculating stream length");
  ...
  bool hasPatternAt(const char* pattern, size_type size_,
  ...
  bool hasPrefix(const char* prefix, size_type size_) const {
    return hasPatternAt(prefix, size_, 0);
  }

  bool skipPrefix(const char* prefix, size_type size_) {
