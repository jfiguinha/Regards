...
public:
  ...
  void release();
  int decodefile(rawspeed3_ret_t* resultp, const void* data, size_t datasize,
                 bool allowunknown);
...
DllDef int rawspeed3_decodefile(rawspeed3_handle_t handle,
                                ...
                                bool allowunknown) {
  if (!handle || !resultp || !data || datasize > 2UL * 1024UL * 1024UL *
      1024UL) {
    if (resultp)
...
rawspeed3_handle_data::rawspeed3_handle_data(const char* cameradefs,
                                             bool is_file)
  : rawParser(nullptr) {
  cameraMeta = std::make_unique<rawspeed::CameraMetaData>(
      _rawspeed3_data_xml, true);
}
