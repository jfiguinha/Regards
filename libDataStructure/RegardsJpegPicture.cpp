...
CRegardsJpegPicture::CRegardsJpegPicture(uint8_t * buffer, const int &width, const int &height)
{
	isValid = true;
	m_lSize = 0;
	this->data = CompressRegardsBitmapToJpeg(buffer, width, height, m_lSize);
	m_iWidth = width;
	m_iHeight = height;

}

uint8_t * CRegardsJpegPicture::CompressRegardsBitmapToJpeg(uint8_t * buffer, int width, int height, unsigned long & outputsize)
{
