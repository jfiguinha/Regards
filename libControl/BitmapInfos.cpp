...
{
	if (fileGeolocalisation->HasGps() && gpsInfosUpdate)
	{
...
void CBitmapInfos::SetFilename(const wxString &libelle)
{
	if (filename != libelle)
	{
		...
		int insertValue = sqlPhotos.GetCriteriaInsert(libelle);
		if (insertValue > 0)
		{
			//muInfosUpdate.lock();
			//gpsInfosUpdate = false;
			//muInfosUpdate.unlock();

			filename = libelle;
			CriteriaVector criteriaList;
			sqlPhotos.GetPhotoCriteria(&criteriaList, libelle);
		...
		else
		{
			...
			filename = libelle;
			//fileGeolocalisation->SetFile(libelle);
