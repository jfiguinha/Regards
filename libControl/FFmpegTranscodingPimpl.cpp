...
double CFFmpegTranscodingPimpl::GetTotalTime()
{
...
int CFFmpegTranscodingPimpl::OpenFile(const wxString & input)
{
	int ret;
	...
	while (!pictureFind)
	{
		...
		if (st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO)
			...
			while (ret >= 0)
			{
				...
				int numBytes = avpicture_get_size(AV_PIX_FMT_BGRA, sw_frame->width, sw_frame->height);
				if (numBytes != bitmap->GetBitmapSize())
				{
					bitmap->SetBitmap(sw_frame->width, sw_frame->height);
				}

				uint8_t * convertedFrameBuffer = bitmap->GetPtBitmap();
				int linesize = sw_frame->width * 4;
