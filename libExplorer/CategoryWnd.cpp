...
CCategoryWnd::CCategoryWnd(CListPicture * listPicture, CThemeTree * theme, CTreeElementControlInterface * interfaceControl)
{
...
void CCategoryWnd::InitCatalogCategorie(tree<CTreeData *>::iterator parent, int numParent)
{
	...
	for (PhotoCategorieVector::iterator fit = categorieVector.begin(); fit != categorieVector.end(); fit++)
	{
		...
		if (fit->GetId() == 2)
		{
			LoadFolder(treeData->GetNumCatalog(), child, idElement);
		}
		else
		{
...
void CCategoryWnd::UpdateSQLSearchCriteria()
{
	...
	for (PositionElementVector::iterator fit = vectorPosElementDynamic.begin(); fit != vectorPosElementDynamic.end(); fit++)
	{
		...
		if (value->GetType() == ELEMENT_CHECKBOX)
		{
			...
			{
				if (treeData->GetNumCategorie() == 2 && treeData->GetIdElement() != -1)
				{
					listFolder.push_back(treeData->GetIdElement());
				}
			}
			else
			{
	...
	sqlFindPhotos.SearchPhotos(&newListPhoto);

	if (newListPhoto.size() != listPhoto.size())
	{
		if (listPicture != nullptr)
			listPicture->RefreshList();
	...
	else
	{
		...
		std::sort(listPhoto.begin(), listPhoto.end());     //  5 10 15 20 25
		std::sort(newListPhoto.begin(), newListPhoto.end());
		std::vector<int>::iterator it = std::set_difference(listPhoto.begin(), listPhoto.end(), newListPhoto.begin(), newListPhoto.end(), diff.begin());
...
void CCategoryWnd::SaveStateTriangle()
{
	...
	CExplorerParam * config = (CExplorerParam *)CExplorerParamInit::getInstance();
	if (config != nullptr)
...
void  CCategoryWnd::ClickOnElement(CPositionElement * element, wxWindow * window, const int &x, const int &y, const int& posLargeur, const int &posHauteur)
{
	...
	{
		CTreeDataCategory * treeData = (CTreeDataCategory *)element->GetTreeData();
		CMainWindow * mainWindow = (CMainWindow *)window->FindWindowById(MAINEXPLORERWINDOWID);
		if (mainWindow != nullptr)
			mainWindow->RemoveFolder(treeData->GetIdElement());
		//deleteElement->ClickElement(window, x, y);
		//UpdateElement();
		//eventControl->Update();
	}
}
