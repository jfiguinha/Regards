...
bool CxImageBMP::Encode(CxFile* hFile)
{
	...
	if (GetNumColors() == 0 && AlphaIsValid())
	{
		...
		hFile->Write(&hdr, min((int32_t)14, (int32_t)sizeof(BITMAPFILEHEADER)), 1);
...
bool CxImageBMP::Decode(CxFile* hFile)
{
	...
	cx_try
	{
		if (hFile->Read(&bf, min((uint32_t)14, (uint32_t)sizeof(bf)), 1) == 0)
			cx_throw("Not a BMP");
		...
		if (pRgb)
		{
			if (bIsOldBmp)
			{
				...
				hFile->Read(pRgb,DibNumColors(&bmpHeader) * sizeof(RGBTRIPLE), 1);
			...
			{
				hFile->Read(pRgb,DibNumColors(&bmpHeader) * sizeof(RGBQUAD), 1);
		...
		case 32:
			...
			if (dwCompression == BI_BITFIELDS || dwCompression == BI_RGB)
			{
				...
				auto buff32 = static_cast<uint8_t*>(malloc(imagesize));
				if (buff32)
				{
		...
		case 16:
			{
				...
				if (dwCompression == BI_BITFIELDS)
				{
					hFile->Read(buffer, 12, 1);
				}
				else
				{
					buffer[0] = 0x7C00;
					buffer[1] = 0x3E0;
					buffer[2] = 0x1F; //RGB555
				}
				...
				// transform into RGB
				Bitfield2RGB(info.pImage, buffer[0], buffer[1], buffer[2], 16);
				break;
			}
		case 8:
		case 4:
					...
					for (BOOL bContinue = TRUE; bContinue && hFile->Read(&status_byte, sizeof(uint8_t), 1);)
					{
						...
						case RLE_COMMAND:
							...
							default:
								...
								{
									if (sline + bits < info.pImage + head.biSizeImage)
									{
										if (low_nibble)
										{
											if (i & 1)
			...
			case BI_RLE8:
				{
					...
					for (BOOL bContinue = TRUE; bContinue && hFile->Read(&status_byte, sizeof(uint8_t), 1);)
					{
						...
						case RLE_COMMAND:
							...
							default:
								hFile->Read(iter.GetRow(scanline) + bits, sizeof(uint8_t) * status_byte, 1);
	...
	case sizeof(BITMAPCOREHEADER):
		{
			...
			pdib->biWidth = static_cast<uint32_t>(bc.bcWidth);
			pdib->biHeight = static_cast<uint32_t>(bc.bcHeight);
			...
			fh->Seek(static_cast<int32_t>(sizeof(BITMAPCOREHEADER) - sizeof(BITMAPINFOHEADER)), SEEK_CUR);
		}
		break;
	default:
		//give a last chance
		if (pdib->biSize > (sizeof(BITMAPINFOHEADER)) &&
			(pdib->biSizeImage >= static_cast<uint32_t>(pdib->biHeight * ((((pdib->biBitCount * pdib->biWidth) + 31) /
				32) * 4))) &&
			(pdib->biPlanes == 1) && (pdib->biClrUsed == 0))
		{
			if (pdib->biCompression == BI_RGB)
				fh->Seek(static_cast<int32_t>(pdib->biSize - sizeof(BITMAPINFOHEADER)),SEEK_CUR);
			break;
		}
		return false;
	}
