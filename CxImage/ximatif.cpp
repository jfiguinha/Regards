			...
			for (y = 0; y < nrow; y++) {
				...
				if (samplesperpixel==1) { 
					if (BIG_palette)
						...
						memcpy(info.pImage+info.dwEffWidth*(height-ys-nrow+y),bits+offset,std::min((unsigned)line, info.dwEffWidth));
					else
					...
					while (ii</*line*/width){		// * VK
						...
						c.rgbRed  =(uint8_t)std::max(0,std::min(255,(int32_t)(cr*255)));
						c.rgbGreen=(uint8_t)std::max(0,std::min(255,(int32_t)(cg*255)));
						c.rgbBlue =(uint8_t)std::max(0,std::min(255,(int32_t)(cb*255)));
