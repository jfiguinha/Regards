...
static int opt_abort_on(void* optctx, const char* opt, const char* arg)
{
	static const AVOption opts[] = {
		...
		{
			"empty_output_stream", NULL, 0, AV_OPT_TYPE_CONST, {.i64 = ABORT_ON_FLAG_EMPTY_OUTPUT_STREAM},
			.unit = "flags"
		},
		{NULL},
	};
...
static int open_input_file(OptionsContext* o, const char* filename)
{
	...
	f->ts_offset = o->input_ts_offset - (copy_ts
		                                     ? (start_at_zero && ic->start_time != AV_NOPTS_VALUE ? ic->start_time : 0)
		                                     : timestamp);
...
static int open_output_file(OptionsContext* o, const char* filename)
{
	...
	if (!o->nb_stream_maps)
	{
		...
		if (!o->video_disable && av_guess_codec(oc->oformat, NULL, filename, NULL, AVMEDIA_TYPE_VIDEO) !=
			AV_CODEC_ID_NONE)
		{
		...
		/* audio: most channels */
		/* audio: most channels */
		if (!o->audio_disable && av_guess_codec(oc->oformat, NULL, filename, NULL, AVMEDIA_TYPE_AUDIO) !=
			AV_CODEC_ID_NONE)
		{
			int best_score = 0, idx = -1;
			for (j = 0; j < nb_input_files; j++)
			{
				...
				if (file_best_idx >= 0)
				{
					file_best_score -= 5000000 * !!(input_streams[file_best_idx]->st->disposition &
		...
		if (!o->subtitle_disable && (avcodec_find_encoder(oc->oformat->subtitle_codec) || subtitle_codec_name))
		{
			for (i = 0; i < nb_input_streams; i++)
				if (input_streams[i]->st->codecpar->codec_type == AVMEDIA_TYPE_SUBTITLE)
				{
					...
					const AVCodecDescriptor* output_descriptor = NULL;
