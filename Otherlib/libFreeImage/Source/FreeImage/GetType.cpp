...
FreeImage_GetFileType(const char *filename, int size) {
	...
	if (handle != NULL) {
		FREE_IMAGE_FORMAT format = FreeImage_GetFileTypeFromHandle(&io, handle, size);
...
FreeImage_GetFileTypeFromMemory(FIMEMORY *stream, int size) {
	...
	if (stream != NULL) {
		return FreeImage_GetFileTypeFromHandle(&io, stream, size);
	}
...
FreeImage_Validate(FREE_IMAGE_FORMAT fif, const char *filename) {
	...
	if (handle != NULL) {
		BOOL bIsValidFIF = FreeImage_ValidateFromHandle(fif, &io, handle);
		fclose(handle);
		return bIsValidFIF;
	}
...
FreeImage_ValidateFromMemory(FREE_IMAGE_FORMAT fif, FIMEMORY *stream) {
	...
	if (stream != NULL) {
		BOOL bIsValidFIF = FreeImage_ValidateFromHandle(fif, &io, stream);
		return bIsValidFIF;
	}
