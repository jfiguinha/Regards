...
bool wxTwainMSW::Startup()
{
    ...
    if (!Ok())
    {
		wxString resourcePath = CFileUtility::GetResourcesFolderPath() + "\\dll\\TWAINDSM.dll";

        m_hTwainDLL = ::LoadLibrary(resourcePath);
        if (m_hTwainDLL)
        {
            ...
            wxLogDebug("Unable to load twain lib '%s'", resourcePath);
        if (Ok())
