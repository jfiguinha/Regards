...
std::vector<uint8_t> ImageGrid::write() const
{
	...
	data[1] = flags;
	data[2] = static_cast<uint8_t>(m_rows - 1);
	data[3] = static_cast<uint8_t>(m_columns - 1);

	if (field_size == 32)
	{
		data[4] = static_cast<uint8_t>((m_output_width >> 24) & 0xFF);
		data[5] = static_cast<uint8_t>((m_output_width >> 16) & 0xFF);
		data[6] = static_cast<uint8_t>((m_output_width >> 8) & 0xFF);
		data[7] = static_cast<uint8_t>((m_output_width) & 0xFF);

		data[8] = static_cast<uint8_t>((m_output_height >> 24) & 0xFF);
		data[9] = static_cast<uint8_t>((m_output_height >> 16) & 0xFF);
		data[10] = static_cast<uint8_t>((m_output_height >> 8) & 0xFF);
		data[11] = static_cast<uint8_t>((m_output_height) & 0xFF);
	}
	else
	{
		data[4] = static_cast<uint8_t>((m_output_width >> 8) & 0xFF);
		data[5] = static_cast<uint8_t>((m_output_width) & 0xFF);

		data[6] = static_cast<uint8_t>((m_output_height >> 8) & 0xFF);
		data[7] = static_cast<uint8_t>((m_output_height) & 0xFF);
	}
...
Error ImageOverlay::parse(size_t num_images, const std::vector<uint8_t>& data)
{
	...
	if (m_version != 0)
	{
		...
		sstr << "Overlay image data version " << static_cast<int>(m_version) << " is not implemented yet";
...
std::string ImageOverlay::dump() const
{
	...
	sstr << "version: " << static_cast<int>(m_version) << "\n"
			...
			for (const Box_iref::Reference& ref : references)
			{
				...
				else if (type == fourcc("auxl"))
				{
					...
					if (auxC_property->get_aux_type() == "urn:mpeg:avc:2015:auxid:1" || // HEIF (avc)
						...
						auxC_property->get_aux_type() == "urn:mpeg:mpegB:cicp:systems:auxiliary:alpha")
					{
						// MIAF
					...
					if (auxC_property->get_aux_type() == "urn:mpeg:hevc:2015:auxid:2" || // HEIF
						auxC_property->get_aux_type() == "urn:mpeg:mpegB:cicp:systems:auxiliary:depth")
					{
						// AVIF
                                     ...
                                     const struct heif_decoding_options* options) const
{
	...
	heif_chroma target_chroma = (out_chroma == heif_chroma_undefined ? img->get_chroma_format() : out_chroma);
                                          ...
                                          const std::vector<uint8_t>& grid_data) const
{
	...
	if (static_cast<int>(image_references.size()) != grid.get_rows() * grid.get_columns())
	{
		std::stringstream sstr;
                                        ...
                                        std::shared_ptr<HeifPixelImage>& img) const
{
	...
	if (static_cast<int>(image_references.size()) != 1)
	{
		return Error(heif_error_Invalid_input,
		             heif_suberror_Missing_grid_images,
	...
	*/
	...
	for (size_t i = 0; i < image_references.size(); i++)
	{
		...
		err = decode_image_planar(image_references[i], overlay_img,
		                          heif_colorspace_RGB, nullptr, false);
		// TODO: always RGB? Probably yes, because of RGB background color.
		if (err != Error::Ok)
		{
			return err;
		}
...
create_alpha_image_from_image_alpha_channel(const std::shared_ptr<HeifPixelImage> image)
{
	...
	auto alpha_image = std::make_shared<HeifPixelImage>();
...
void HeifContext::Image::set_preencoded_hevc_image(const std::vector<uint8_t>& data)
{
	...
	for (;;)
	{
		...
		if (ptr == static_cast<int>(data.size()))
		{
			start_code_start = static_cast<int>(data.size());
			dump_nal = true;
			eof = true;
		}
                                        ...
                                        std::shared_ptr<Image>& out_image)
{
		...
		out_image->get_height() != encoded_height)
	{
		...
		else
		{
			...
			// TODO: using out_image->get_width/height() would be shorter.
...
Error HeifContext::add_exif_metadata(std::shared_ptr<Image> master_image, const void* data, int size)
{
	...
	uint32_t offset = 0;
	auto tiffmagic1 = "MM\0*";
	auto tiffmagic2 = "II*\0";
	while (offset + 4 < static_cast<unsigned int>(size))
	{
	...
	data_array[0] = static_cast<uint8_t>((offset >> 24) & 0xFF);
	data_array[1] = static_cast<uint8_t>((offset >> 16) & 0xFF);
	data_array[2] = static_cast<uint8_t>((offset >> 8) & 0xFF);
	data_array[3] = static_cast<uint8_t>((offset) & 0xFF);
	memcpy(data_array.data() + 4, data, size);


	return add_generic_metadata(master_image,
	                            data_array.data(), static_cast<int>(data_array.size()),
	                            "Exif", nullptr);
}
