                         ...
                         int stride)
{
	...
	else
	{
		...
		auto _dest = static_cast<uint8_t*>(dest);
	...
	struct heif_context* context)
{
	...
	auto ids = static_cast<heif_item_id*>(malloc(count * sizeof(heif_item_id)));
	if (!ids)
	{
...
{
	return static_cast<int>((v / 2.0) + 0.5);
}
	...
	case heif_colorspace_YCbCr:
		{
			...
			auto dest = const_cast<char*>(data.data());
