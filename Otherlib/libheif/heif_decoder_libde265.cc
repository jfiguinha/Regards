                                                              ...
                                                              struct heif_image** image)
{
	...
	auto yuv_img = std::make_shared<HeifPixelImage>();
	yuv_img->create(de265_get_image_width(de265img, 0),
	                ...
	                static_cast<heif_chroma>(de265_get_chroma_format(de265img)));
...
{
	auto decoder = new libde265_decoder();
	struct heif_error err = {heif_error_Ok, heif_suberror_Unspecified, kSuccess};
...
{
	auto decoder = static_cast<struct libde265_decoder*>(decoder_raw);
...
static struct heif_error libde265_v1_push_data(void* decoder_raw, const void* data, size_t size)
{
	...
	auto cdata = static_cast<const uint8_t*>(data);
	...
	*out_img = nullptr;
	do
	{
		...
		if (image)
		{
			...
			nclx->set_matrix_coefficients(static_cast<uint16_t>(de265_get_image_matrix_coefficients(image)));
			nclx->set_colour_primaries(static_cast<uint16_t>(de265_get_image_colour_primaries(image)));
			nclx->set_transfer_characteristics(static_cast<uint16_t>(de265_get_image_transfer_characteristics(image)));
#endif
