...
Fraction::Fraction(uint32_t num, uint32_t den)
{
	...
	*this = Fraction(static_cast<int32_t>(num), static_cast<int32_t>(den));
}
...
std::string BoxHeader::get_type_string() const
{
	if (m_type == fourcc("uuid"))
	{
		...
		for (int i = 0; i < 16; i++)
		{
			...
			sstr << static_cast<int>(m_uuid_type[i]);
		}
...
Error BoxHeader::parse(BitstreamRange& range)
{
...
size_t BoxHeader::reserve_box_header_space(StreamWriter& writer) const
{
...
Error BoxHeader::prepend_header(StreamWriter& writer, size_t box_start) const
{
	...
	else
	{
		...
		writer.write32(static_cast<uint32_t>(box_size));
	}
...
std::string BoxHeader::dump(Indent& indent) const
{
	...
	if (m_is_full_box)
	{
		sstr << indent << "version: " << static_cast<int>(m_version) << "\n"
...
Error Box::read(BitstreamRange& range, std::shared_ptr<Box>* result)
{
	...
	if (hdr.get_box_size() > static_cast<uint64_t>(std::numeric_limits<int64_t>::max()))
	{
		return Error(heif_error_Invalid_input,
		             heif_suberror_Invalid_box_size);
	}
...
Error Box_pitm::write(StreamWriter& writer) const
{
	...
	if (get_version() == 0)
	{
		...
		writer.write16(static_cast<uint16_t>(m_item_ID));
	}
	else
	{
...
Error Box_iloc::parse(BitstreamRange& range)
{
	...
	*/
	...
	for (int i = 0; i < item_count; i++)
	{
		...
		{
			item.base_offset = static_cast<uint64_t>(range.read32()) << 32;
			item.base_offset |= range.read32();
		}
		...
		for (int e = 0; e < extent_count; e++)
		{
			...
			if (get_version() > 1 && index_size > 0)
			{
				...
				{
					extent.index = static_cast<uint64_t>(range.read32()) << 32;
					extent.index |= range.read32();
				}
			}
			...
			{
				extent.offset = static_cast<uint64_t>(range.read32()) << 32;
				extent.offset |= range.read32();
			}
			...
			{
				extent.length = static_cast<uint64_t>(range.read32()) << 32;
				extent.length |= range.read32();
			}
...
std::string Box_iloc::dump(Indent& indent) const
{
	...
	for (const Item& item : m_items)
	{
		sstr << indent << "item ID: " << item.item_ID << "\n"
			<< indent << "  construction method: " << static_cast<int>(item.construction_method) << "\n"
                          ...
                          std::vector<uint8_t>* dest) const
{
	...
	for (const auto& extent : item.extents)
	{
		if (item.construction_method == 0)
		{
			...
			if (status == StreamReader::timeout)
			{
                            ...
                            uint8_t construction_method)
{
	...
	if (construction_method == 1)
	{
		...
		m_idat_offset += static_cast<int>(data.size());
	}
...
void Box_iloc::derive_box_version()
{
	...
	set_version(static_cast<uint8_t>(min_version));
}
...
Error Box_iloc::write(StreamWriter& writer) const
{
	...
	if (sum_idat_size > 0)
	{
		writer.write32(static_cast<uint32_t>(sum_idat_size + 8));
...
Error Box_iloc::write_mdat_after_iloc(StreamWriter& writer)
{
	...
	writer.write32(static_cast<uint32_t>(sum_mdat_size + 8));
...
void Box_iloc::patch_iloc_header(StreamWriter& writer) const
{
	...
	writer.write8(static_cast<uint8_t>((m_offset_size << 4) | (m_length_size)));
	writer.write8(static_cast<uint8_t>((m_base_offset_size << 4) | (m_index_size)));

	if (get_version() < 2)
	{
		writer.write16(static_cast<uint16_t>(m_items.size()));
	}
	else
	{
		writer.write32(static_cast<uint32_t>(m_items.size()));
	}
	...
	for (const auto& item : m_items)
	{
		if (get_version() < 2)
		{
			writer.write16(static_cast<uint16_t>(item.item_ID));
		}
		else
		{
		...
		writer.write16(static_cast<uint16_t>(item.extents.size()));
...
Error color_profile_nclx::get_nclx_color_profile(struct heif_color_profile_nclx** out_data) const
{
	*out_data = alloc_nclx_color_profile();
	...
	nclx->version = 1;
	nclx->color_primaries = static_cast<enum heif_color_primaries>(get_colour_primaries());
	nclx->transfer_characteristics = static_cast<enum heif_transfer_characteristics>(get_transfer_characteristics());
	nclx->matrix_coefficients = static_cast<enum heif_matrix_coefficients>(get_matrix_coefficients());
...
{
	auto profile = static_cast<heif_color_profile_nclx*>(malloc(sizeof(struct heif_color_profile_nclx)));
...
std::string Box_pixi::dump(Indent& indent) const
{
	...
	{
		if (i > 0) sstr << ",";
		sstr << static_cast<int>(m_bits_per_channel[i]);
	}
...
Error Box_pixi::write(StreamWriter& writer) const
{
	...
	writer.write8(static_cast<uint8_t>(m_bits_per_channel.size()));
	for (size_t i = 0; i < m_bits_per_channel.size(); i++)
	{
...
void Box_ipma::derive_box_version()
{
	...
	set_version(static_cast<uint8_t>(version));
...
Error Box_ipma::write(StreamWriter& writer) const
{
	...
	writer.write32(static_cast<uint32_t>(entry_cnt));

	for (const Entry& entry : m_entries)
	{
		if (get_version() < 1)
		{
			writer.write16(static_cast<uint16_t>(entry.item_ID));
		}
		else
		{
		...
		writer.write8(static_cast<uint8_t>(assoc_cnt));

		for (const PropertyAssociation& association : entry.associations)
		{
			if (get_flags() & 1)
			{
				writer.write16(static_cast<uint16_t>((association.essential ? 0x8000 : 0) |
					(association.property_index & 0x7FFF)));
			}
			else
			{
				writer.write8(static_cast<uint8_t>((association.essential ? 0x80 : 0) |
...
std::string Box_auxC::dump(Indent& indent) const
{
	...
	for (uint8_t subtype : m_aux_subtypes)
	{
		sstr << std::hex << std::setw(2) << std::setfill('0') << static_cast<int>(subtype) << " ";
	}
                   ...
                   uint32_t image_width, uint32_t image_height)
{
	...
	m_horizontal_offset = Fraction(-static_cast<int32_t>(image_width - clap_width), 2);
	m_vertical_offset = Fraction(-static_cast<int32_t>(image_height - clap_height), 2);
}
...
Error Box_iref::write(StreamWriter& writer) const
{
	...
	for (const auto& ref : m_references)
	{
		uint32_t box_size = static_cast<uint32_t>(4 + 4 + 2 + id_size * (1 + ref.to_item_ID.size()));
		...
		writer.write16(static_cast<uint16_t>(ref.to_item_ID.size()));
...
std::string Box_hvcC::dump(Indent& indent) const
{
	...
	sstr << indent << "configuration_version: " << static_cast<int>(c.configuration_version) << "\n"
		<< indent << "general_profile_space: " << static_cast<int>(c.general_profile_space) << "\n"
		<< indent << "general_tier_flag: " << c.general_tier_flag << "\n"
		<< indent << "general_profile_idc: " << static_cast<int>(c.general_profile_idc) << "\n";
	...
	sstr << indent << "general_level_idc: " << static_cast<int>(c.general_level_idc) << "\n"
		<< indent << "min_spatial_segmentation_idc: " << c.min_spatial_segmentation_idc << "\n"
		<< indent << "parallelism_type: " << static_cast<int>(c.parallelism_type) << "\n"
		<< indent << "chroma_format: " << static_cast<int>(c.chroma_format) << "\n"
		<< indent << "bit_depth_luma: " << static_cast<int>(c.bit_depth_luma) << "\n"
		<< indent << "bit_depth_chroma: " << static_cast<int>(c.bit_depth_chroma) << "\n"
		<< indent << "avg_frame_rate: " << c.avg_frame_rate << "\n"
		<< indent << "constant_frame_rate: " << static_cast<int>(c.constant_frame_rate) << "\n"
		<< indent << "num_temporal_layers: " << static_cast<int>(c.num_temporal_layers) << "\n"
		<< indent << "temporal_id_nested: " << static_cast<int>(c.temporal_id_nested) << "\n"
		<< indent << "length_size: " << static_cast<int>(m_length_size) << "\n";
	...
	for (const auto& array : m_nal_array)
	{
		...
		indent++;
		sstr << indent << "array_completeness: " << static_cast<int>(array.m_array_completeness) << "\n"
			<< indent << "NAL_unit_type: " << static_cast<int>(array.m_NAL_unit_type) << "\n";
		...
		for (const auto& unit : array.m_nal_units)
		{
			...
			for (uint8_t b : unit)
			{
				sstr << std::setfill('0') << std::setw(2) << std::hex << static_cast<int>(b) << " ";
			}
			sstr << "\n";
			sstr << std::dec;
		}
...
void Box_hvcC::append_nal_data(const std::vector<uint8_t>& nal)
{
	...
	array.m_NAL_unit_type = static_cast<uint8_t>(nal[0] >> 1);
...
Error Box_hvcC::write(StreamWriter& writer) const
{
	...
	writer.write8(static_cast<uint8_t>(((c.general_profile_space & 3) << 6) |
	...
	writer.write8(static_cast<uint8_t>((c.bit_depth_luma - 8) | 0xF8));
	writer.write8(static_cast<uint8_t>((c.bit_depth_chroma - 8) | 0xF8));
	writer.write16(c.avg_frame_rate);

	writer.write8(static_cast<uint8_t>(((c.constant_frame_rate & 0x03) << 6) |
	...
	{
		writer.write8(static_cast<uint8_t>(((array.m_array_completeness & 1) << 6) |
...
Error Box_av1C::write(StreamWriter& writer) const
{
	...
	writer.write8(static_cast<uint8_t>(((c.seq_profile & 0x7) << 5) |
		(c.seq_level_idx_0 & 0x1f)));

	writer.write8(static_cast<uint8_t>((c.seq_tier_0 ? 0x80 : 0) |
...
std::string Box_av1C::dump(Indent& indent) const
{
	...
	sstr << indent << "version: " << static_cast<int>(c.version) << "\n"
		<< indent << "seq_profile: " << static_cast<int>(c.seq_profile) << "\n"
		<< indent << "seq_level_idx_0: " << static_cast<int>(c.seq_level_idx_0) << "\n"
		<< indent << "high_bitdepth: " << static_cast<int>(c.high_bitdepth) << "\n"
		<< indent << "twelve_bit: " << static_cast<int>(c.twelve_bit) << "\n"
		<< indent << "chroma_subsampling_x: " << static_cast<int>(c.chroma_subsampling_x) << "\n"
		<< indent << "chroma_subsampling_y: " << static_cast<int>(c.chroma_subsampling_y) << "\n"
		<< indent << "chroma_sample_position: " << static_cast<int>(c.chroma_sample_position) << "\n"
	...
	for (size_t i = 0; i < m_config_OBUs.size(); i++)
	{
		sstr << " " << std::hex << std::setfill('0') << std::setw(2)
			<< static_cast<int>(m_config_OBUs[i]);
	}
	sstr << std::dec << "\n";
                          ...
                          std::vector<uint8_t>& out_data) const
{
	...
	if (start > static_cast<uint64_t>(m_data_start_pos) + get_box_size())
	{
		return Error(heif_error_Invalid_input,
		             heif_suberror_End_of_data);
	}
	if (length > get_box_size() || start + length > get_box_size())
	{
		return Error(heif_error_Invalid_input,
		             heif_suberror_End_of_data);
	}

	StreamReader::grow_status status = istr->
		wait_for_file_size(static_cast<int64_t>(m_data_start_pos) + start + length);
	if (status == StreamReader::size_beyond_eof ||
	...
	bool success;
	success = istr->seek(m_data_start_pos + static_cast<std::streampos>(start));
	assert(success);
	(void)success;
...
Error Box_dref::parse(BitstreamRange& range)
{
	...
	*/
	...
	if (static_cast<int>(m_children.size()) != nEntities)
	{
		// TODO return Error(
	}
