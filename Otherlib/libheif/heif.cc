...
static struct heif_error error_invalid_parameter_value = {
	heif_error_Usage_error,
...
static struct heif_error error_null_parameter = {
	heif_error_Usage_error,
...
heif_brand heif_fourcc_to_brand_enum(const char* fourcc)
{
	...
	if (strcmp(brand, "heix") == 0)
	{
		return heif_heix;
	}
	if (strcmp(brand, "hevc") == 0)
	{
		return heif_hevc;
	}
	if (strcmp(brand, "hevx") == 0)
	{
		return heif_hevx;
	}
	if (strcmp(brand, "heim") == 0)
	{
		return heif_heim;
	}
	if (strcmp(brand, "heis") == 0)
	{
		return heif_heis;
	}
	if (strcmp(brand, "hevm") == 0)
	{
		return heif_hevm;
	}
	if (strcmp(brand, "hevs") == 0)
	{
		return heif_hevs;
	}
	...
	if (strcmp(brand, "avif") == 0)
	{
		return heif_avif;
	}
	if (strcmp(brand, "avis") == 0)
	{
		return heif_avis;
	}
	return heif_unknown_brand;
...
void heif_brand_to_fourcc(heif_brand2 brand, char* out_fourcc)
{
	if (out_fourcc)
	{
		out_fourcc[0] = static_cast<char>((brand >> 24) & 0xFF);
		out_fourcc[1] = static_cast<char>((brand >> 16) & 0xFF);
		out_fourcc[2] = static_cast<char>((brand >> 8) & 0xFF);
		out_fourcc[3] = static_cast<char>((brand >> 0) & 0xFF);
	}
}
...
{
	if (data == nullptr || len <= 0 || brand_fourcc == nullptr || !brand_fourcc[0] || !brand_fourcc[1] || !brand_fourcc[
		2] || !brand_fourcc[3])
	{
		return -1;
	}
...
struct heif_error heif_list_compatible_brands(const uint8_t* data, int len, heif_brand2** out_brands, int* out_size)
{
	...
	*out_brands = static_cast<heif_brand2*>(malloc(sizeof(heif_brand2) * brands.size()));
	*out_size = static_cast<int>(brands.size());
...
const char* heif_get_file_mime_type(const uint8_t* data, int len)
{
	...
	if (mainBrand == heif_msf1)
	{
		return "image/heif-sequence";
	}
	if (mainBrand == heif_avif)
	{
		return "image/avif";
	}
	if (mainBrand == heif_avis)
	{
		return "image/avif-sequence";
	}
	if (is_jpeg(data, len) == TriBool::Yes)
	{
		return "image/jpeg";
	}
	if (is_png(data, len) == TriBool::Yes)
	{
		return "image/png";
	}
	return "";
...
{
	return static_cast<int>(ctx->context->get_top_level_images().size());
}
                                                 ...
                                                 int count)
{
	...
	int n = std::min(count, static_cast<int>(imgs.size()));
	for (int i = 0; i < n; i++)
	{
...
{
	return static_cast<int>(handle->image->get_thumbnails().size());
}
                                                ...
                                                heif_item_id* ids, int count)
{
	...
	int n = std::min(count, static_cast<int>(thumbnails.size()));
...
{
	return static_cast<int>(handle->image->get_aux_images(include_alpha_image).size());
}
                                                      ...
                                                      heif_item_id* ids, int count)
{
	...
	int n = std::min(count, static_cast<int>(auxImages.size()));
                                                       ...
                                                       const char** out_type)
{
	...
	auto buf = static_cast<char*>(malloc(auxType.length() + 1));
                                                   ...
                                                   const size_t profile_size)
{
	...
	data.insert(data.end(),
	            static_cast<const uint8_t*>(profile_data),
	            static_cast<const uint8_t*>(profile_data) + profile_size);
...
heif_color_profile_type heif_image_handle_get_color_profile_type(const struct heif_image_handle* handle)
{
	...
	if (profile_icc)
	{
		return static_cast<heif_color_profile_type>(profile_icc->get_type());
	}

	auto profile_nclx = handle->image->get_color_profile_nclx();
	if (profile_nclx)
	{
		return static_cast<heif_color_profile_type>(profile_nclx->get_type());
	}
...
enum heif_color_profile_type heif_image_get_color_profile_type(const struct heif_image* image)
{
	...
	return static_cast<heif_color_profile_type>(profile->get_type());
}
...
struct heif_error heif_register_decoder(heif_context* heif, const heif_decoder_plugin* decoder_plugin)
{
	...
	if (decoder_plugin->plugin_api_version != 1)
	{
...
struct heif_error heif_register_encoder_plugin(const heif_encoder_plugin* encoder_plugin)
{
	...
	if (encoder_plugin->plugin_api_version != 1)
	{
		return error_unsupported_plugin_version;
	}
...
{
	auto filename = static_cast<const char*>(userdata);
...
{
	auto plugin = get_decoder(format);
                                                              ...
                                                              const int** out_integer_array)
{
	     ...
	     params++)
	{
		if (strcmp((*params)->name, parameter_name) == 0)
		{
			return heif_encoder_parameter_get_valid_integer_values(*params, have_minimum, have_maximum, minimum,
			                                                       maximum,
...
static void copy_options(heif_encoding_options& options, const heif_encoding_options& input_options)
{
	...
	case 4:
		...
		options.macOS_compatibility_workaround_no_nclx_profile = input_options.
			macOS_compatibility_workaround_no_nclx_profile;
	// fallthrough
	case 3:
		options.save_two_colr_boxes_when_ICC_and_nclx_available = input_options.
	...
	else
	{
		...
		if (options.output_nclx_profile == nullptr)
		{
			...
			if (input_nclx)
			{
				...
				nclx.version = 1;
				nclx.color_primaries = static_cast<enum heif_color_primaries>(input_nclx->get_colour_primaries());
				nclx.transfer_characteristics = static_cast<enum heif_transfer_characteristics>(input_nclx->
					get_transfer_characteristics());
				nclx.matrix_coefficients = static_cast<enum heif_matrix_coefficients>(input_nclx->
