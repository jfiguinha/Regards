...
static std::shared_ptr<SEIMessage> read_depth_representation_info(BitReader& reader)
{
	...
	msg->has_z_near = static_cast<uint8_t>(reader.get_bits(1));
	msg->has_z_far = static_cast<uint8_t>(reader.get_bits(1));
	msg->has_d_min = static_cast<uint8_t>(reader.get_bits(1));
	msg->has_d_max = static_cast<uint8_t>(reader.get_bits(1));
	...
	msg->depth_representation_type = static_cast<enum heif_depth_representation_type>(rep_type);
                                         ...
                                         std::vector<std::shared_ptr<SEIMessage>>& msgs)
{
	...
	uint32_t len = static_cast<uint32_t>(reader.get_bits(32));
	...
	while (reader.get_current_byte_index() < static_cast<int>(len))
	{
		int currPos = reader.get_current_byte_index();
		BitReader sei_reader(data.data() + currPos, static_cast<int>(data.size()) - currPos);

		uint32_t nal_size = static_cast<uint32_t>(sei_reader.get_bits(32));
		(void)nal_size;

		uint8_t nal_type = static_cast<uint8_t>(sei_reader.get_bits(8) >> 1);
			...
			nal_type == 40)
		{
			...
			uint8_t payload_id = static_cast<uint8_t>(sei_reader.get_bits(8));
			uint8_t payload_size = static_cast<uint8_t>(sei_reader.get_bits(8));
			(void)payload_size;
                                             ...
                                             int* width, int* height)
{
	...
	config->temporal_id_nested = static_cast<uint8_t>(reader.get_bits(1));

	// --- profile_tier_level ---

	config->general_profile_space = static_cast<uint8_t>(reader.get_bits(2));
	config->general_tier_flag = static_cast<uint8_t>(reader.get_bits(1));
	config->general_profile_idc = static_cast<uint8_t>(reader.get_bits(5));
	...
	config->general_level_idc = static_cast<uint8_t>(reader.get_bits(8));
	...
	config->chroma_format = static_cast<uint8_t>(value);
	...
	reader.get_uvlc(&value);
	config->bit_depth_luma = static_cast<uint8_t>(value + 8);

	reader.get_uvlc(&value);
	config->bit_depth_chroma = static_cast<uint8_t>(value + 8);
