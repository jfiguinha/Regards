...
Error heif::fill_av1C_configuration(Box_av1C::configuration* inout_config, std::shared_ptr<HeifPixelImage> image)
{
	...
	inout_config->chroma_subsampling_x = static_cast<uint8_t>(chroma_h_subsampling(chroma) >> 1);
	inout_config->chroma_subsampling_y = static_cast<uint8_t>(chroma_v_subsampling(chroma) >> 1);
...
bool heif::fill_av1C_configuration_from_stream(Box_av1C::configuration* out_config, const uint8_t* data, int dataSize)
{
	...
	while (reader.get_bits_remaining() > 0)
	{
		...
		if (header_info.has_size)
		{
			if (header_info.size > static_cast<uint64_t>(std::numeric_limits<int>::max()))
			{
				return false;
			}

			reader.skip_bytes(static_cast<int>(header_info.size));
		}
		else
		{
			return false;
	...
	out_config->seq_profile = static_cast<uint8_t>(reader.get_bits(3));
	...
	{
		out_config->seq_level_idx_0 = static_cast<uint8_t>(reader.get_bits(5));
	...
	else
	{
		...
		{
			reader.skip_bits(12);
			auto level = static_cast<uint8_t>(reader.get_bits(5));
			if (i == 0)
			{
			...
			if (level > 7)
			{
				auto tier = static_cast<uint8_t>(reader.get_bits(1));
				if (i == 0)
				{
			...
			if (initial_display_delay_present_flag)
			{
				...
				{
					auto delay = static_cast<uint8_t>(reader.get_bits(4));
					if (i == 0)
					{
	...
	out_config->high_bitdepth = static_cast<uint8_t>(reader.get_bits(1));
	if (out_config->seq_profile == 2 && out_config->high_bitdepth)
	{
		out_config->twelve_bit = static_cast<uint8_t>(reader.get_bits(1));
	}
	else
	{
	...
	{
		out_config->monochrome = static_cast<uint8_t>(reader.get_bits(1));
	}
	...
	else
	{
		...
		else
		{
			...
			{
				out_config->chroma_subsampling_x = static_cast<uint8_t>(reader.get_bits(1));
				if (out_config->chroma_subsampling_x)
				{
					out_config->chroma_subsampling_y = static_cast<uint8_t>(reader.get_bits(1));
				}
				else
				{
					out_config->chroma_subsampling_y = 0;
		...
		{
			out_config->chroma_sample_position = static_cast<uint8_t>(reader.get_bits(2));
		}
	}
