...
{
	auto encoder = new encoder_struct_x265();
...
{
	auto encoder = static_cast<struct encoder_struct_x265*>(encoder_raw);
...
static struct heif_error x265_set_parameter_lossless(void* encoder_raw, int enable)
{
	...
	encoder->add_param(heif_encoder_parameter_name_lossless, static_cast<bool>(enable));
...
static struct heif_error x265_set_parameter_integer(void* encoder_raw, const char* name, int value)
{
	...
	if (strcmp(name, kParam_TU_intra_depth) == 0)
	{
		if (value < 1 || value > 4)
		{
			return heif_error_invalid_parameter_value;
	...
	if (strcmp(name, kParam_complexity) == 0)
	{
...
static struct heif_error x265_set_parameter_string(void* encoder_raw, const char* name, const char* value)
{
	...
	if (strcmp(name, kParam_chroma) == 0)
	{
		...
		if (strcmp(value, "422") == 0)
		{
			encoder->chroma = heif_chroma_422;
			return heif_error_ok;
		}
		if (strcmp(value, "444") == 0)
		{
