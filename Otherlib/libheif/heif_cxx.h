...
namespace heif
{
	...
	{
		auto err = Error(heif_context_read_from_file(m_context.get(), filename.c_str(), nullptr));
		if (err)
		{
			throw err;
		}
	...
	{
		auto err = Error(heif_context_read_from_memory(m_context.get(), mem, size, nullptr));
		if (err)
		{
			throw err;
		}
	...
	{
		auto err = Error(heif_context_read_from_memory_without_copy(m_context.get(), mem, size, nullptr));
		if (err)
		{
			throw err;
		}
	...
	{
		auto reader = static_cast<Context::Reader*>(userdata);
	...
	{
		auto err = Error(heif_context_read_from_reader(m_context.get(), &heif_reader_trampoline,
		                                               &reader, nullptr));
		if (err)
		{
			throw err;
	...
	{
		heif_item_id id;
		auto err = Error(heif_context_get_primary_image_ID(m_context.get(), &id));
		if (err)
		{
			throw err;
		}
	...
	inline ImageHandle Context::get_primary_image_handle() const
	{
		...
		auto err = Error(heif_context_get_primary_image_handle(m_context.get(), &handle));
		if (err)
		{
			throw err;
		}
	...
	inline ImageHandle Context::get_image_handle(heif_item_id id) const
	{
		...
		auto err = Error(heif_context_get_image_handle(m_context.get(), id, &handle));
		if (err)
		{
			throw err;
		}
  ...
  inline Context Context::wrap_without_releasing(heif_context* ctx) {
	...
	inline struct heif_error heif_writer_trampoline_write(struct heif_context* ctx,
	                                                      ...
	                                                      void* userdata)
	{
		auto writer = static_cast<Context::Writer*>(userdata);
	...
	{
		auto err = Error(heif_context_write(m_context.get(), &heif_writer_trampoline, &writer));
		if (err)
		{
			throw err;
		}
	...
	{
		auto err = Error(heif_context_write_to_file(m_context.get(), filename.c_str()));
		if (err)
		{
			throw err;
		}
	...
	{
		heif_image_handle* handle;
		auto err = Error(heif_image_handle_get_thumbnail(m_image_handle.get(), id, &handle));
		if (err)
		{
			throw err;
		}
	...
	{
		heif_image* out_img;
		auto err = Error(heif_decode_image(m_image_handle.get(),
		                                   &out_img,
		                                   colorspace,
		                                   chroma,
	...
	inline std::vector<uint8_t> ImageHandle::get_metadata(heif_item_id metadata_id) const
	{
		...
		auto err = Error(heif_image_handle_get_metadata(m_image_handle.get(),
		                                                metadata_id,
		                                                data.data()));
		if (err)
		{
	...
	{
		heif_image* image;
		auto err = Error(heif_image_create(width, height, colorspace, chroma, &image));
		if (err)
		{
			m_image.reset();
			throw err;
	...
	inline void Image::add_plane(enum heif_channel channel,
	                             int width, int height, int bit_depth)
	{
		auto err = Error(heif_image_add_plane(m_image.get(), channel, width, height, bit_depth));
		if (err)
		{
			throw err;
		}
	...
	{
		auto err = Error(heif_image_set_nclx_color_profile(m_image.get(), nclx.mProfile));
		if (err)
		{
			throw err;
		}
	...
	inline ColorProfile_nclx Image::get_nclx_color_profile() const
	{
		...
		auto err = Error(heif_image_get_nclx_color_profile(m_image.get(), &nclx));
		if (err)
		{
			throw err;
		}
	                                         ...
	                                         const std::vector<uint8_t>& data)
	{
		...
		case heif_color_profile_type_rICC:
			...
			break;
		default:
			throw Error(heif_error_Usage_error,
			            heif_suberror_Unspecified,
			            "invalid raw color profile type");
			break;
		}

		auto err = Error(heif_image_set_raw_color_profile(m_image.get(), profile_type,
		                                                  data.data(), data.size()));
		if (err)
		{
			throw err;
	...
	{
		heif_image* img;
		auto err = Error(heif_image_scale_image(m_image.get(), &img, width, height,
	...
	inline Encoder EncoderDescriptor::get_encoder() const
	{
		...
		auto err = Error(heif_context_get_encoder(nullptr, m_descriptor, &encoder));
		if (err)
		{
			throw err;
		}
	...
	{
		heif_encoder* encoder;
		auto err = Error(heif_context_get_encoder_for_format(nullptr, format, &encoder));
		if (err)
		{
			throw err;
		}
	...
	inline bool EncoderParameter::get_valid_integer_range(int& out_minimum, int& out_maximum)
	{
		...
		auto err = Error(heif_encoder_parameter_get_valid_integer_range(m_parameter,
	...
	inline std::vector<std::string> EncoderParameter::get_valid_string_values() const
	{
		...
		auto err = Error(heif_encoder_parameter_get_valid_string_values(m_parameter,
		                                                                &stringarray));
		if (err)
		{
			throw err;
	...
	{
		auto err = Error(heif_encoder_set_lossy_quality(m_encoder.get(), quality));
		if (err)
		{
			throw err;
		}
	...
	{
		auto err = Error(heif_encoder_set_lossless(m_encoder.get(), enable_lossless));
		if (err)
		{
			throw err;
		}
	...
	{
		auto err = Error(heif_encoder_set_parameter_integer(m_encoder.get(), parameter_name.c_str(), value));
		if (err)
		{
			throw err;
		}
	...
	{
		int value;
		auto err = Error(heif_encoder_get_parameter_integer(m_encoder.get(), parameter_name.c_str(), &value));
		if (err)
		{
			throw err;
		}
	...
	{
		auto err = Error(heif_encoder_set_parameter_boolean(m_encoder.get(), parameter_name.c_str(), value));
		if (err)
		{
			throw err;
		}
	...
	{
		int value;
		auto err = Error(heif_encoder_get_parameter_boolean(m_encoder.get(), parameter_name.c_str(), &value));
		if (err)
		{
			throw err;
		}
	...
	{
		auto err = Error(heif_encoder_set_parameter_string(m_encoder.get(), parameter_name.c_str(), value.c_str()));
		if (err)
		{
			throw err;
		}
	...
	inline std::string Encoder::get_string_parameter(std::string parameter_name) const
	{
		...
		char value[max_size];
		auto err = Error(heif_encoder_get_parameter_string(m_encoder.get(), parameter_name.c_str(),
		                                                   value, max_size));
		if (err)
		{
			throw err;
	...
	{
		auto err = Error(heif_encoder_set_parameter(m_encoder.get(), parameter_name.c_str(),
	...
	inline std::string Encoder::get_parameter(std::string parameter_name) const
	{
		const int max_size = 250;
		char value[max_size];
		auto err = Error(heif_encoder_get_parameter(m_encoder.get(), parameter_name.c_str(),
		                                            value, max_size));
		if (err)
		{
			throw err;
	...
	{
		auto err = Error(heif_context_set_primary_image(m_context.get(),
	                                         ...
	                                         const EncodingOptions& options)
	{
		...
		auto err = Error(heif_context_encode_image(m_context.get(),
		                                           img.m_image.get(),
	                                             ...
	                                             int bbox_size)
	{
		...
		auto err = Error(heif_context_encode_thumbnail(m_context.get(),
		                                               image.m_image.get(),
	...
	inline void Context::assign_thumbnail(const ImageHandle& thumbnail_image,
	                                      const ImageHandle& master_image)
	{
		auto err = Error(heif_context_assign_thumbnail(m_context.get(),
	...
	inline void Context::add_exif_metadata(const ImageHandle& master_image,
	                                       const void* data, int size)
	{
		auto err = Error(heif_context_add_exif_metadata(m_context.get(),
		                                                master_image.get_raw_image_handle(),
		                                                data, size));
		if (err)
		{
	...
	inline void Context::add_XMP_metadata(const ImageHandle& master_image,
	                                      const void* data, int size)
	{
		auto err = Error(heif_context_add_XMP_metadata(m_context.get(),
		                                               master_image.get_raw_image_handle(),
		                                               data, size));
		if (err)
		{
