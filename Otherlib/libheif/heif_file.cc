...
Error HeifFile::read_from_file(const char* input_filename)
{
	...
	auto input_stream_istr = std::unique_ptr<std::istream>(
		std::make_unique<std::ifstream>(convert_utf8_path_to_utf16(input_filename).c_str(), std::ios_base::binary));
#else
...
{
	auto input_stream = std::make_shared<StreamReader_memory>(static_cast<const uint8_t*>(data), size, copy);
...
heif_chroma HeifFile::get_image_chroma_from_configuration(heif_item_id imageID) const
{
	...
	if (hvcC_box)
	{
		return static_cast<heif_chroma>(hvcC_box->get_configuration().chroma_format);
	}
...
Error HeifFile::get_compressed_image_data(heif_item_id ID, std::vector<uint8_t>* data) const
{
  ...
  std::lock_guard<std::mutex> guard(m_read_mutex);
	...
	auto error = Error(heif_error_Unsupported_feature,
...
void HeifFile::add_ispe_property(heif_item_id id, uint32_t width, uint32_t height)
{
	...
	m_ipma_box->add_property_for_item_ID(id, Box_ipma::PropertyAssociation{false, static_cast<uint16_t>(index + 1)});
}
                                 ...
                                 uint32_t image_width, uint32_t image_height)
{
	...
	m_ipma_box->add_property_for_item_ID(id, Box_ipma::PropertyAssociation{true, static_cast<uint16_t>(index + 1)});
}
...
void HeifFile::append_iloc_data_with_4byte_size(heif_item_id id, const uint8_t* data, size_t size)
{
	...
	nal[0] = static_cast<uint8_t>((size >> 24) & 0xFF);
	nal[1] = static_cast<uint8_t>((size >> 16) & 0xFF);
	nal[2] = static_cast<uint8_t>((size >> 8) & 0xFF);
	nal[3] = static_cast<uint8_t>((size >> 0) & 0xFF);
