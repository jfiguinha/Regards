...
wxMBConv* wxPdfFontData::ms_winEncoding = nullptr;
...
public:
	...
	*/
	bool IsIncluded(wxUint32 unicode) const override
	{
	...
	if (FindFile(fontCollectionFileName, fullFontCollectionFileName))
	{
		...
		else
		{
			wxLogWarning(wxString(wxS("wxPdfFontManagerBase::RegisterFontCollection: ")) +
				wxString::Format(
					_("Font collection file '%s' has not the file extension '.ttc'."), fontCollectionFileName.c_str()));
		}
	}
	else
	{
		wxLogError(wxString(wxS("wxPdfFontManagerBase::RegisterFontCollection: ")) +
			wxString::Format(
				_("Font collection file '%s' does not exist or is not readable."), fontCollectionFileName.c_str()));
	}
	return count;
}
#endif
	...
	if (familyIter == m_fontFamilyMap.end())
	{
		...
		else
		{
			wxLogError(wxString(wxS("wxPdfFontManagerBase::RegisterFontCJK: ")) +
				wxString::Format(
					_("CJK Font file '%s' for CJK family '%s' does not exist or is not readable."),
...
wxPdfFontManagerBase::RegisterSystemFonts()
{
	...
	auto fontRegKey = new wxRegKey(strFont);
...
wxPdfFontManagerBase::GetFont(const wxString& fontName, int fontStyle) const
{
	...
	if (familyIter != m_fontFamilyMap.end())
	{
		...
		size_t j;
		for (j = 0; j < n && fontData == nullptr; ++j)
		{
...
wxPdfFontManagerBase::RegisterEncoding(const wxPdfEncoding& encoding)
{
	...
	if (m_encodingMap->find(encodingName) == m_encodingMap->end())
	{
		...
		wxCriticalSectionLocker locker(gs_csFontManager);
#endif
		auto addedEncoding = new wxPdfEncoding(encoding);
	...
	if (fontData != nullptr)
	{
		...
		if (fontType.IsSameAs(wxS("TrueType")) || fontType.IsSameAs(wxS("Type1")))
		{
			if (RegisterEncoding(encoding))
			{
				...
				wxPdfEncoding* baseEncoding = (beIter != m_encodingMap->end()) ? beIter->second : nullptr;
		...
		else if (fontType.IsSameAs(wxS("Type0")))
		{
			...
			wxPdfEncodingChecker* encodingChecker = (ecIter != m_encodingCheckerMap->end()) ? ecIter->second : nullptr;
...
wxPdfFontManagerBase::GetEncoding(const wxString& encodingName)
{
	...
	wxCriticalSectionLocker locker(gs_csFontManager);
#endif
	wxPdfEncoding* foundEncoding = nullptr;
...
{
	int j;
	for (j = 0; gs_encodingTableData[j].m_encodingName != nullptr; ++j)
	{
		wxString encodingName(gs_encodingTableData[j].m_encodingName);
		wxPdfEncodingChecker* encodingChecker;
		if (gs_encodingTableData[j].m_encodingTable != nullptr)
		{
...
wxPdfFontManagerBase::RegisterEncoding(const wxString& encoding)
{
	...
	if (m_encodingMap->find(encodingName) == m_encodingMap->end())
	{
		auto addedEncoding = new wxPdfEncoding();
...
#endif
				{
					...
					else
					{
						...
						wxLogError(wxString(wxS("wxPdfFontManagerBase::LoadFontFromXML: ")) +
							wxString::Format(
								_("Unknown font type '%s' in font file '%s'."), fontType.c_str(),
								fontFileName.c_str()));
					}
					if (fontData != nullptr)
					{
						...
						if (!fontData->LoadFontMetrics(root))
						{
							wxLogError(wxString(wxS("wxPdfFontManagerBase::LoadFontFromXML: ")) +
								wxString::Format(
									_("Loading of font metrics failed for font file '%s'."), fontFileName.c_str()));
							delete fontData;
							fontData = nullptr;
						}
					}
...
wxPdfFontManagerBase::AddFont(wxPdfFontData* fontData, wxPdfFont& font)
{
	...
	if (fontIter == m_fontNameMap.end())
	{
		...
		auto fontEntry = new wxPdfFontListEntry(fontData);
...
wxPdfFontManager* wxPdfFontManager::ms_fontManager = nullptr;
...
void wxPdfDocumentModule::OnExit()
{
	...
	wxPdfFontManager::ms_fontManager = nullptr;
}
