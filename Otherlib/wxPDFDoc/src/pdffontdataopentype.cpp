...
{
	wxUnusedVar(encoding);
	bool doSubsetting = usedGlyphs != nullptr && subsetGlyphs != nullptr;
	wxString t;
	...
	for (ch = s.begin(); ch != s.end(); ++ch)
	{
		...
		if (charIter != m_gn->end())
		{
			...
			if (doSubsetting)
			{
				...
				{
					subsetGlyph = static_cast<wxUint32>(usedGlyphs->GetCount());
	...
	if (fontStream != nullptr)
	{
		if (usedGlyphs != nullptr)
		{
			...
			else
			{
				...
				auto buffer = new char[m_cffLength];
