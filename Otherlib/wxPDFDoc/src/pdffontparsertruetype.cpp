...
ReadUShort(BYTE* p)
{
	return (static_cast<USHORT>(p[0]) << 8) + static_cast<USHORT>(p[1]);
}

static DWORD
ReadDWord(BYTE* p)
{
	return (static_cast<LONG>(p[0]) << 24) + (static_cast<LONG>(p[1]) << 16) + (static_cast<LONG>(p[2]) << 8) +
		static_cast<LONG>(p[3]);
}

static DWORD ReadTag(BYTE* p)
{
	return (static_cast<LONG>(p[3]) << 24) + (static_cast<LONG>(p[2]) << 16) + (static_cast<LONG>(p[1]) << 8) +
		static_cast<LONG>(p[0]);
}

static void
WriteDWord(BYTE* p, DWORD dw)
{
	p[0] = static_cast<BYTE>((dw >> 24) & 0xFF);
	p[1] = static_cast<BYTE>((dw >> 16) & 0xFF);
	p[2] = static_cast<BYTE>((dw >> 8) & 0xFF);
	p[3] = static_cast<BYTE>((dw) & 0xFF);
}
...
ExtractFontData(HDC hdc, DWORD& fontDataSize, BYTE*& fontData)
{
	...
	if (GetFontData(hdc, TTC_FILE, 0, nullptr, 0) != GDI_ERROR)
	{
		...
		auto fontHeader = new BYTE[headerSize];
		if (!fontHeader)
		{
			wxLogError(wxString(wxS("ExtractFontData: ")) +
...
wxPdfFontParserTrueType::CalculateChecksum(const char* b, size_t length)
{
	...
	for (k = 0; k < len; ++k)
	{
		d3 += static_cast<int>(b[ptr++]) & 0xff;
		d2 += static_cast<int>(b[ptr++]) & 0xff;
		d1 += static_cast<int>(b[ptr++]) & 0xff;
		d0 += static_cast<int>(b[ptr++]) & 0xff;
	}
		...
		if (fileName.GetExt().Lower().IsSameAs(wxS("ttc")))
		{
			if (fontIndex >= 0)
			{
				...
				if (mainTag == wxS("ttcf"))
				{
					...
					else
					{
						wxLogError(wxString(wxS("wxPdfFontParserTrueType::IdentifyFont: ")) +
							wxString::Format(
								_("Font index %d out of range for font file '%s'."), fontIndex, fontFileName.c_str()));
						ok = false;
					}
				}
				else
				{
					wxLogError(wxString(wxS("wxPdfFontParserTrueType::IdentifyFont: '")) +
						wxString::Format(
							_("Font file '%s' not a valid TrueType collection (TTC) file."), fontFileName.c_str()));
					ok = false;
				}
			}
			else
	...
	if (fontStream != nullptr)
	{
		...
		if (ok)
		{
			...
			else
			{
				wxLogError(wxString(wxS("wxPdfFontParserTrueType::IdentifyFont: ")) +
					wxString::Format(
						_("Reading of font directory failed for font file '%s'."), font.GetFaceName().c_str()));
			}
		}
		delete fontStream;
	}
...
wxPdfFontParserTrueType::IdentifyFont()
{
	...
	if (ReadTableDirectory())
	{
		if (CheckTables())
		{
			...
			if (m_cff)
			{
				auto otfFontData = new wxPdfFontDataOpenTypeUnicode();
		...
		if (m_inFont != nullptr)
		{
				...
				if (mainTag == wxS("ttcf"))
				{
					...
					else
					{
						wxLogError(wxString(wxS("wxPdfFontParserTrueType::LoadFontData: ")) +
							wxString::Format(
								_("Font index %d out of range for font file '%s'."), fontIndex, m_fileName.c_str()));
					}
				}
				else
				{
					...
					if (!ok)
					{
						wxLogError(wxString(wxS("wxPdfFontParserTrueType::LoadFontData: '")) +
							wxString::Format(
								_("Font file '%s' not a valid TrueType collection (TTC) file."), m_fileName.c_str()));
					}
				}
			}
			else
			...
			if (ok)
			{
				if (ReadTableDirectory())
				{
					if (CheckTables())
					{
						...
						else
						{
							wxLogError(wxString(wxS("wxPdfFontParserTrueType::LoadFontData: ")) +
								wxString::Format(
									_("Wrong font data type '%s' for font file '%s'."), fontData->GetType().c_str(),
									m_fileName.c_str()));
						}
					}
					else
				...
				else
				{
					wxLogError(wxString(wxS("wxPdfFontParserTrueType::LoadFontData: ")) +
						wxString::Format(
							_("Reading of font directory failed for font file '%s'."), m_fileName.c_str()));
				}
			}
			if (fontFile != nullptr)
			{
...
wxPdfFontParserTrueType::PrepareFontData(wxPdfFontData* fontData)
{
	...
	if (ok)
	{
		auto widths = new wxPdfGlyphWidthMap();
		auto glyphs = new wxPdfChar2GlyphMap();

		wxPdfCMap* cMap;
		if (m_cmapExt != nullptr)
			cMap = m_cmapExt;
		else if (!m_fontSpecific && m_cmap31 != nullptr)
			cMap = m_cmap31;
		else if (m_fontSpecific && m_cmap10 != nullptr)
			cMap = m_cmap10;
		else if (m_cmap31 != nullptr)
			cMap = m_cmap31;
		else
			cMap = m_cmap10;
		wxPdfCMap::iterator cMapIter;
	...
	if (!m_isMacCoreText)
	{
		...
		if (id == 0x00010000 || id == 0x4F54544F || id == 0x74727565)
		{
			...
			for (k = 0; k < num_tables; ++k)
			{
				...
				auto tableLocation = new wxPdfTableDirectoryEntry();
		...
		else
		{
			if (!m_fileName.IsEmpty())
			{
				wxLogError(wxString(wxS("wxPdfFontParserTrueType::ReadTableDirectory: '")) +
					wxString::Format(
						_("Font file '%s' not a valid TrueType (TTF) or OpenType (OTF) file."), m_fileName.c_str()));
			}
			ok = false;
		}
	}
...
wxPdfFontParserTrueType::CheckTables()
{
	...
	int tableCount = 0;
	while (ok && tableCount < maxTableCount && checkTableNames[tableCount] != nullptr)
	{
...
wxPdfFontParserTrueType::ReadMaps()
{
	...
	if (entry != m_tableDirectory->end())
	{
		...
		{
			os_2.m_sTypoDescender = static_cast<short>(-os_2.m_sTypoDescender);
		}
		...
		{
			os_2.m_sxHeight = 0;
			os_2.m_sCapHeight = static_cast<int>(0.7 * head.m_unitsPerEm);
		}
		ReleaseTable();
		m_fd.SetOpenTypeMetrics(
			hhea.m_ascender * 1000 / head.m_unitsPerEm,
			hhea.m_descender * 1000 / head.m_unitsPerEm,
			hhea.m_lineGap * 1000 / head.m_unitsPerEm,
			os_2.m_sTypoAscender * 1000 / head.m_unitsPerEm,
			realOS2TypoDescender * 1000 / head.m_unitsPerEm,
			os_2.m_sTypoLineGap * 1000 / head.m_unitsPerEm,
			os_2.m_usWinAscent * 1000 / head.m_unitsPerEm,
			os_2.m_usWinDescent * 1000 / head.m_unitsPerEm
		);
	}
	else
	{
	...
	if (entry == m_tableDirectory->end())
	{
		...
		double caretSlopeRun = hhea.m_caretSlopeRun;
		double caretSlopeRise = hhea.m_caretSlopeRise;
	...
	else
	{
		...
		italicAngle = static_cast<double>(mantissa) + static_cast<double>(fraction) / 16384.0;
	...
	m_fd.SetAscent(os_2.m_sTypoAscender * 1000 / head.m_unitsPerEm);
	m_fd.SetDescent(os_2.m_sTypoDescender * 1000 / head.m_unitsPerEm);
	m_fd.SetCapHeight(os_2.m_sCapHeight * 1000 / head.m_unitsPerEm);
	m_fd.SetXHeight(os_2.m_sxHeight * 1000 / head.m_unitsPerEm);
	m_fd.SetItalicAngle(static_cast<int>(italicAngle));
	m_fd.SetStemV(80);
	m_fd.SetUnderlinePosition(underlinePosition * 1000 / head.m_unitsPerEm);
	m_fd.SetUnderlineThickness(underlineThickness * 1000 / head.m_unitsPerEm);
  ...
  int      m_stemV;                   ///< StemV
	...
	wxString fbb = wxString::Format(wxS("[%d %d %d %d]"),
	                                head.m_xMin * 1000 / head.m_unitsPerEm,
	                                head.m_yMin * 1000 / head.m_unitsPerEm,
	                                head.m_xMax * 1000 / head.m_unitsPerEm,
	                                head.m_yMax * 1000 / head.m_unitsPerEm);
	...
	bool ok = (m_cmap10 != nullptr) || (m_cmap31 != nullptr) || (m_cmapExt != nullptr);
	if (!ok)
	{
...
{
	auto h = new wxPdfCMap();
	SkipBytes(4);
	int k;
	for (k = 0; k < 256; k++)
	{
		auto r = new wxPdfCMapEntry();
		r->m_glyph = static_cast<int>(ReadByte());
...
wxPdfFontParserTrueType::ReadFormat4()
{
	...
	auto endCount = new int[segCount];
	auto startCount = new int[segCount];
	auto idDelta = new int[segCount];
	auto idRO = new int[segCount];
	auto glyphId = new int[glyphIdCount];
		...
		for (k = 0; k < nTables; ++k)
		{
			...
			if ((coverage & 0xfff7) == 0x0001)
			{
				...
				for (j = 0; j < nPairs; ++j)
				{
					u1 = ReadUShort();
					u2 = ReadUShort();
					int value = (static_cast<int>(ReadShort()) * 1000) / unitsPerEm;
					if (u1 != u1prev)
					{
						u1prev = u1;
...
wxPdfFontParserTrueType::GetGlyphWidth(unsigned int glyph)
{
	...
	{
		glyph = static_cast<unsigned int>(m_glyphWidths.GetCount()) - 1;
	}
