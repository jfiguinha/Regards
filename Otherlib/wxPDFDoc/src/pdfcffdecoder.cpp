...
wxPdfCffDecoder::wxPdfCffDecoder()
{
	...
	m_globalSubrIndex = nullptr;
	m_hGlobalSubrsUsed = nullptr;
	m_lGlobalSubrsUsed = nullptr;
                                          ...
                                          int& achar)
{
	...
	stream->SeekI(begin);
	ReadCommand(stream);
	wxPdfCffFontObject* element = nullptr;
                           ...
                           wxPdfCffIndexArray& localSubrIndex)
{
  ...
  wxLogDebug(wxS("ReadAsubr %d %d %d %d"), begin, end, globalBias, localBias);
	...
	while (stream->TellI() < end)
	{
		...
		wxPdfCffFontObject* topElement = nullptr;
		if (m_argCount > 0)
		{
...
wxPdfCffDecoder::ReadCommand(wxInputStream* stream)
{
	...
	while (!gotKey)
	{
		...
		{
			m_args[m_argCount].m_type = 0;
			m_args[m_argCount].m_intValue = b0 - 139;
			m_argCount++;
			continue;
		}
		if (b0 >= 247 && b0 <= 250) // The byte read and the next byte constetute a short int
		{
			...
			short item = static_cast<short>((b0 - 247) * 256 + b1 + 108);
			m_args[m_argCount].m_type = 0;
		...
		{
			unsigned char b1 = ReadByte(stream);
			short item = static_cast<short>(-(b0 - 251) * 256 - b1 - 108);
			m_args[m_argCount].m_type = 0;
			m_args[m_argCount].m_intValue = item;
			m_argCount++;
			continue;
