...
wxFileSystem* wxPdfImage::ms_fileSystem = nullptr;
...
wxPdfImage::wxPdfImage(wxPdfDocument* document, int index, const wxString& filename, const wxString& type)
{
	...
	m_imageFile = GetFileSystem()->OpenFile(fileURL);
...
{
	if (m_pal != nullptr) delete [] m_pal;
	if (m_trns != nullptr) delete [] m_trns;
...
wxPdfImage::ConvertWxImage(const wxImage& image, bool jpegFormat)
{
  ...
  if (jpegFormat)
  {
	...
	if (wxImage::FindHandler(bitmapType) == nullptr)
	{
#if wxUSE_LIBJPEG
		if (jpegFormat)
		{
...
wxPdfImage::ParsePNG(wxInputStream* imageStream)
{
	...
	imageStream->Read(buffer, 4);
	m_parms = wxString::Format(
		wxS("/DecodeParms <</Predictor 15 /Colors %d /BitsPerComponent %d /Columns %d>>"), (ct == 2 ? 3 : 1),
		static_cast<int>(bpc), w);
	...
	if (colspace == wxS("Indexed") && m_pal == nullptr)
	{
		if (m_pal != nullptr) delete [] m_pal;
		if (m_trns != nullptr) delete [] m_trns;
		if (m_data != nullptr) delete [] m_data;
		...
		default:
			{
				...
				off_t pos = static_cast<unsigned int>(ReadUShortBE(imageStream));
				pos = pos - 2;
				if (pos)
				{
...
AddGdiObject(wxArrayPtrVoid& gdiObjects, void* obj)
{
	...
	for (idx = 0; idx < n; idx++)
	{
		if (gdiObjects[idx] == nullptr) break;
	}
	if (idx < n)
	{
		gdiObjects[idx] = obj;
...
wxPdfImage::ParseWMF(wxInputStream* imageStream)
{
	...
	while (!imageStream->Eof() && !endRecord)
	{
		...
		case 0x01f0: // DeleteObject
			{
				...
				delete static_cast<GdiObject*>(gdiObjects[idx]);
				gdiObjects[idx] = nullptr;
			}
			break;
		...
		case 0x012d: // SelectObject
			{
				idx = ReadUShortLE(imageStream);
				obj = static_cast<GdiObject*>(gdiObjects[idx]);
	...
	{
		if (gdiObjects[i] != nullptr)
		{
			delete static_cast<GdiObject*>(gdiObjects[i]);
		}
	}
	m_x = wo[0];
	m_y = wo[1];
	...
	m_dataSize = static_cast<unsigned int>(data.Length());
	m_data = new char[m_dataSize];
	memcpy(m_data, wcb, m_dataSize);
	return isValid;
}
