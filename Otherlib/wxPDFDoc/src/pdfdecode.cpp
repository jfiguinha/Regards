...
wxPdfParser::FlateDecode(wxMemoryOutputStream* osIn)
{
	...
	auto osOut = new wxMemoryOutputStream();
	osOut->Write(zin);
	osOut->Close();
	return osOut;
}
...
wxPdfParser::ASCIIHexDecode(wxMemoryOutputStream* osIn)
{
	...
	for (k = 0; k < inLength; ++k)
	{
		...
		{
			osOut->PutC(static_cast<char>(((n1 << 4) + n) & 0xff));
		}
		first = !first;
	}
	if (!first)
	{
		osOut->PutC(static_cast<char>((n1 << 4) & 0xff));
	}
	osOut->Close();
	return osOut;
}
...
wxPdfParser::ASCII85Decode(wxMemoryOutputStream* osIn)
{
	...
	for (k = 0; k < inLength; k++)
	{
		...
		if (state == 5)
		{
			...
			osOut->PutC(static_cast<char>((r >> 24) & 0xff));
			osOut->PutC(static_cast<char>((r >> 16) & 0xff));
			osOut->PutC(static_cast<char>((r >> 8) & 0xff));
			osOut->PutC(static_cast<char>(r & 0xff));
		}
	}
	int r = 0;
	if (state == 1)
...
{
	if (dicPar == nullptr || dicPar->GetType() != OBJTYPE_DICTIONARY)
	{
		return osIn;
	}

	auto dic = static_cast<wxPdfDictionary*>(dicPar);
	wxPdfObject* obj = ResolveObject(dic->Get(wxS("Predictor")));
	if (obj == nullptr || obj->GetType() != OBJTYPE_NUMBER)
	{
		return osIn;
	}
	int predictor = static_cast<wxPdfNumber*>(obj)->GetInt();
	if (predictor < 10)
	{
		return osIn;
	}
	...
	if (obj != nullptr && obj->GetType() == OBJTYPE_NUMBER)
	{
		width = static_cast<wxPdfNumber*>(obj)->GetInt();
	}
	int colours = 1;
	obj = ResolveObject(dic->Get(wxS("Colors")));
	if (obj != nullptr && obj->GetType() == OBJTYPE_NUMBER)
	{
		colours = static_cast<wxPdfNumber*>(obj)->GetInt();
	}
	int bpc = 8;
	obj = ResolveObject(dic->Get(wxS("BitsPerComponent")));
	if (obj != nullptr && obj->GetType() == OBJTYPE_NUMBER)
	{
		bpc = static_cast<wxPdfNumber*>(obj)->GetInt();
	}
	...
	auto curr = new char[bytesPerRow];
	auto prior = new char[bytesPerRow];
	...
	while (true)
	{
		...
		if (dataStream.LastRead() != static_cast<size_t>(bytesPerRow))
		{
			break;
		}
...
wxPdfLzwDecoder::GetNextCode()
{
	...
	if (static_cast<size_t>(m_bytePointer) < m_dataSize)
	{
...
wxPdfLzwDecoder::WriteString(int code)
{
	...
	for (j = 0; j < length; j++)
	{
		m_dataOut->PutC(static_cast<char>(m_stringTable[code][j]));
	}
}
