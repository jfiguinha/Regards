...
{
	auto layer = new wxPdfLayer(wxEmptyString);
...
wxPdfLayer::SetCreatorInfo(const wxString& creator, const wxString& subtype)
{
	...
	if (usage->Get(wxS("CreatorInfo")) == nullptr)
	{
...
{
	wxPdfDictionary* usage = AllocateUsage();
	if (usage->Get(wxS("Language")) == nullptr)
	{
		auto dic = new wxPdfDictionary();
...
{
	wxPdfDictionary* usage = AllocateUsage();
	if (usage->Get(wxS("Export")) == nullptr)
	{
		auto dic = new wxPdfDictionary();
...
{
	wxPdfDictionary* usage = AllocateUsage();
	if (usage->Get(wxS("Print")) == nullptr)
	{
		auto dic = new wxPdfDictionary();
		dic->Put(wxS("Subtype"), new wxPdfName(subtype));
