...
int wxPdfRijndael::blockEncrypt(const UINT8* input, int inputLen, UINT8* outBuffer)
{
	...
	if (input == nullptr || inputLen <= 0)return 0;
		...
		for (i = numBlocks; i > 0; i--)
		{
			for (k = 0; k < 128; k++)
			{
				...
				*((UINT32*)(block + 12)) = *((UINT32*)iv[3]);
				...
				iv[0][0] = static_cast<UINT8>((iv[0][0] << 1) | (iv[0][1] >> 7));
				iv[0][1] = static_cast<UINT8>((iv[0][1] << 1) | (iv[0][2] >> 7));
				iv[0][2] = static_cast<UINT8>((iv[0][2] << 1) | (iv[0][3] >> 7));
				iv[0][3] = static_cast<UINT8>((iv[0][3] << 1) | (iv[1][0] >> 7));
				iv[1][0] = static_cast<UINT8>((iv[1][0] << 1) | (iv[1][1] >> 7));
				iv[1][1] = static_cast<UINT8>((iv[1][1] << 1) | (iv[1][2] >> 7));
				iv[1][2] = static_cast<UINT8>((iv[1][2] << 1) | (iv[1][3] >> 7));
				iv[1][3] = static_cast<UINT8>((iv[1][3] << 1) | (iv[2][0] >> 7));
				iv[2][0] = static_cast<UINT8>((iv[2][0] << 1) | (iv[2][1] >> 7));
				iv[2][1] = static_cast<UINT8>((iv[2][1] << 1) | (iv[2][2] >> 7));
				iv[2][2] = static_cast<UINT8>((iv[2][2] << 1) | (iv[2][3] >> 7));
				iv[2][3] = static_cast<UINT8>((iv[2][3] << 1) | (iv[3][0] >> 7));
				iv[3][0] = static_cast<UINT8>((iv[3][0] << 1) | (iv[3][1] >> 7));
				iv[3][1] = static_cast<UINT8>((iv[3][1] << 1) | (iv[3][2] >> 7));
				iv[3][2] = static_cast<UINT8>((iv[3][2] << 1) | (iv[3][3] >> 7));
				iv[3][3] = static_cast<UINT8>((iv[3][3] << 1) | ((outBuffer[k / 8] >> (7 - (k & 7))) & 1));
			}
		}
		break;
	default:
...
int wxPdfRijndael::padEncrypt(const UINT8* input, int inputOctets, UINT8* outBuffer)
{
	...
	//      assert(padLen > 0 && padLen <= 16); // DO SOMETHING HERE ?
		...
		{
			block[i] = static_cast<UINT8>(input[i] ^ iv[i]);
		}
		...
		for (i = numBlocks; i > 0; i--)
		{
			for (k = 0; k < 128; k++)
			{
				...
				*((UINT32*)(block + 12)) = *((UINT32*)iv[3]);
				...
				iv[3][1] = static_cast<UINT8>((iv[3][1] << 1) | (iv[3][2] >> 7));
				iv[3][2] = static_cast<UINT8>((iv[3][2] << 1) | (iv[3][3] >> 7));
				iv[3][3] = static_cast<UINT8>((iv[3][3] << 1) | ((input[k / 8] >> (7 - (k & 7))) & 1));
				outBuffer[k / 8] ^= (block[0] & 0x80) >> (k & 7);
			}
		}
		break;
