...
{
	const wxPdfChar2GlyphMap* convMap = nullptr;
	if (encoding != nullptr)
	{
		convMap = encoding->GetEncodingMap();
	}
	if (convMap == nullptr && m_encoding != nullptr)
	{
...
{
	wxString t;
	if (m_encodingChecker != nullptr)
	{
		if (m_encodingChecker->IsIncluded(replace))
		{
			replace = wxS('?');
		}
		wxString::const_iterator ch = s.begin();
		...
		{
			if (m_encodingChecker->IsIncluded(*ch))
			{
				t.Append(*ch);
			}
			else
...
wxPdfFontData::WriteToUnicode(wxPdfGlyphList& glyphs, wxMemoryOutputStream& toUnicode, bool simple)
{
	...
	unsigned int size = 0;
	unsigned int k;
	unsigned int numGlyphs = static_cast<unsigned int>(glyphs.GetCount());
