...
wxPdfFontSubsetTrueType::CreateNewTables()
{
	...
	for (k = 0; k < m_locaTableSize; k++)
	{
		...
		if (glyphIndex < usedGlyphCount && static_cast<size_t>((*m_usedGlyphs)[glyphIndex]) == k)
		{
			glyphIndex++;
			m_newLocaTable[k] = newGlyphOffset;
...
wxPdfFontSubsetTrueType::WriteSubsetFont()
{
	...
	int tableCount = 0;
	while (tableNames[tableCount] != nullptr)
	{
		tableCount++;
	}
	...
	for (k = 0; k < tableCount; ++k)
	{
		...
		if (entry != m_tableDirectory->end())
		{
			...
			if (name == wxS("glyf"))
			{
				...
				tableLength = static_cast<int>(m_newGlyfTableRealSize);
			}
			else if (name == wxS("loca"))
			{
				WriteInt(CalculateChecksum(m_newLocaTableStream, m_newLocaTableStreamSize));
				tableLength = static_cast<int>(m_locaTableRealSize);
			}
			else
			{
...
{
	buffer[0] = static_cast<char>((n >> 8) & 0xff);
	buffer[1] = static_cast<char>((n) & 0xff);
}
...
{
	buffer[0] = static_cast<char>((n >> 24) & 0xff);
	buffer[1] = static_cast<char>((n >> 16) & 0xff);
	buffer[2] = static_cast<char>((n >> 8) & 0xff);
	buffer[3] = static_cast<char>((n) & 0xff);
}
