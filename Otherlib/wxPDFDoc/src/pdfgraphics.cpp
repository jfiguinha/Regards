...
wxPdfDocument::SetAlpha(double lineAlpha, double fillAlpha, wxPdfBlendMode blendMode)
{
	...
	int id = static_cast<int>(blendMode) * 100000000 + static_cast<int>(lineAlpha * 1000) * 10000 + static_cast<int>(
		fillAlpha * 1000);
	...
	{
		n = static_cast<int>((*m_extGStates).size()) + 1;
...
{
	if (alphaState > 0 && static_cast<size_t>(alphaState) <= (*m_extGStates).size())
	{
...
{
	m_subpath = static_cast<int>(m_x.GetCount());
...
wxPdfShape::GetSegment(int iterType, int iterPoints, double coords[]) const
{
	...
	if (iterType >= 0 && static_cast<size_t>(iterType) < m_types.GetCount())
	{
		int pointCount = (m_types[iterType] == wxPDF_SEG_CURVETO) ? 2 : 0;
		if (iterPoints >= 0 && static_cast<size_t>(iterPoints + pointCount) < m_x.GetCount())
		{
			segType = static_cast<wxPdfSegmentType>(m_types[iterType]);
			switch (segType)
			{
...
wxPdfFlatPath::FetchSegment()
{
	...
	if (static_cast<size_t>(m_iterType) >= m_shape->GetSegmentCount())
	{
		m_done = true;
		return;
	}
...
wxPdfDocument::ShapedText(const wxPdfShape& shape, const wxString& text, wxPdfShapedTextMode mode)
{
	...
	unsigned int length = static_cast<unsigned int>(voText.Length());
				...
				if (distance >= next)
				{
					...
					while (currentChar < length && distance >= next)
					{
						...
						nextAdvance = currentChar < length - 1
							              ? DoGetStringWidth(voText.Mid(currentChar + 1, 1)) * 0.5
							              : (repeat)
							              ? DoGetStringWidth(voText.Mid(0, 1)) * 0.5
							              : 0;
						SetXY(x, y);
						StartTransform();
						Rotate(angle);
...
{
	unsigned int np = (x.GetCount() < y.GetCount())
		                  ? static_cast<unsigned int>(x.GetCount())
		                  : static_cast<unsigned int>(y.GetCount());
...
wxPdfDocument::EndPath(int style)
{
	...
	switch (style)
	{
	case wxPDF_STYLE_FILL: op = (m_fillRule == wxODDEVEN_RULE) ? wxS("f*") : wxS("f");
		break;
	case wxPDF_STYLE_FILLDRAW: op = (m_fillRule == wxODDEVEN_RULE) ? wxS("B*") : wxS("B");
		break;
...
wxPdfDocument::ClosePath(int style)
{
	...
	switch (style)
	{
	case wxPDF_STYLE_DRAW: op = wxS("S");
		break;
	case wxPDF_STYLE_FILL: op = (m_fillRule == wxODDEVEN_RULE) ? wxS("f*") : wxS("f");
		break;
	case wxPDF_STYLE_FILLDRAW: op = (m_fillRule == wxODDEVEN_RULE) ? wxS("B*") : wxS("B");
                              ...
                              wxPdfLinearGradientType gradientType)
{
	...
	if (ColourSpaceOk(col1, col2))
	{
		...
		n = static_cast<int>((*m_gradients).size()) + 1;
