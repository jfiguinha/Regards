...
wxPdfFont::wxPdfFont(wxPdfFontData* fontData, int fontStyle)
	: m_embed(false), m_subset(false), m_fontStyle(fontStyle),
	  m_fontData(fontData), m_encoding(nullptr)
{
...
wxPdfFont::wxPdfFont(const wxPdfFont& font)
	: m_embed(font.m_embed), m_subset(font.m_subset), m_fontStyle(font.m_fontStyle),
	  m_fontData(font.m_fontData), m_encoding(nullptr)
{
	if (m_fontData != nullptr)
	{
		m_fontData->IncrementRefCount();
...
{
	bool canShow = false;
	if (m_fontData != nullptr && wxPdfFontManager::GetFontManager()->InitializeFontData(*this))
	{
...
wxPdfFont::GetSupportedUnicodeCharacters(wxPdfArrayUint32& unicodeCharacters) const
{
	...
	if (m_fontData != nullptr && wxPdfFontManager::GetFontManager()->InitializeFontData(*this))
	{
		...
		if (ctgMap == nullptr && m_encoding != nullptr)
		{
...
{
	return (m_fontData != nullptr) ? m_fontData->EmbedRequired() : false;
}

bool
wxPdfFont::EmbedSupported() const
{
	return (m_fontData != nullptr) ? m_fontData->EmbedSupported() : false;
}
...
wxPdfFont::SetEncoding(const wxString& encodingName)
{
	...
	if (m_fontData != nullptr)
	{
		if (m_fontData->GetType().IsSameAs(wxS("Type1")) &&
			encoding != nullptr && encoding->IsOk())
		{
			ok = wxPdfFontManager::GetFontManager()->InitializeFontData(*this);
			if (ok)
			{
				if (!static_cast<wxPdfFontDataType1*>(m_fontData)->GetEncodingType().IsEmpty())
				{
					m_encoding = encoding;
					ok = true;
				}
				else
				{
					wxLogDebug(wxString(wxS("wxPdfFont::SetEncoding: ")) +
						wxString(_(
							"Setting a user defined encoding is only supported for dynamically loaded Type1 fonts.")));
				}
			}
			else
			{
