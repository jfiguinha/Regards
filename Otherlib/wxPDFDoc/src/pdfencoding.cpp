...
wxPdfEncoding::GlyphName2Unicode(const wxString& glyphName, wxUint32& unicode)
{
	...
	bool found = false;
	int first = 0;
	int last = static_cast<int>(gs_glyphName2UnicodeTableSize) - 1;
	int mid;
...
wxPdfEncoding::Unicode2GlyphName(wxUint32 unicode, wxString& glyphName)
{
	...
	bool found = false;
	int first = 0;
	int last = static_cast<int>(gs_unicode2GlyphNameTableSize) - 1;
	int mid;
	while (!found && first <= last)
	{
		mid = (first + last) / 2;
...
wxPdfEncoding::GetKnownEncodings()
{
	...
	size_t j = 0;
	while (gs_encodingData[j].m_encodingName != nullptr)
	{
...
wxPdfEncoding::wxPdfEncoding()
{
	...
	m_encodingMap = nullptr;
}

wxPdfEncoding::~wxPdfEncoding()
{
	if (m_encodingMap != nullptr)
	{
...
wxPdfEncoding::SetEncoding(const wxString& encoding)
{
	...
	bool found = false;
	size_t j = 0;
	while (!found && gs_encodingData[j].m_encodingName != nullptr)
	{
	...
	if (found)
	{
		...
		for (k = 0; k < 128; ++k)
		{
			if (hasFullSize)
			{
				unicodeChar = static_cast<wxUint32>(gs_encodingData[j].m_encodingMap[k]);
				m_cmap[k] = unicodeChar;
				m_cmapBase[k] = unicodeChar;
			}
			else
			{
				unicodeChar = static_cast<wxUint32>(k);
				m_cmap[k] = unicodeChar;
				m_cmapBase[k] = unicodeChar;
			}
		...
		for (k = 128; k < 256; ++k)
		{
			...
			unicodeChar = static_cast<wxUint32>(gs_encodingData[j].m_encodingMap[k - delta]);
			m_cmap[k] = unicodeChar;
			m_cmapBase[k] = static_cast<wxUint32>(gs_encodingData[j].m_encodingBase[k - delta]);
