...
wxPdfCoonsPatchMesh::~wxPdfCoonsPatchMesh()
{
	...
	if (n > 0)
	{
		...
		for (j = 0; j < n; j++)
		{
			delete static_cast<wxPdfCoonsPatch*>(m_patches[j]);
		}
	}
}
...
wxPdfCoonsPatchMesh::AddPatch(int edgeFlag, wxPdfColour colours[], double x[], double y[])
{
	...
	auto patch = new wxPdfCoonsPatch(edgeFlag, colours, x, y);
	m_patches.Add(patch);
	m_ok = true;
	return true;
}
	...
	: wxPdfGradient(wxPDF_GRADIENT_COONS)
{
	...
	for (j = 0; j < n; j++)
	{
		auto patch = static_cast<wxPdfCoonsPatch*>((*patches)[j]);
		...
		for (k = 0; k < nc; k++)
		{
			...
			coord = static_cast<int>(((x[k] - minCoord) / (maxCoord - minCoord)) * bpcd);
			if (coord < 0) coord = 0;
			...
			ch = coord & 0xFF;
			m_buffer.Write(&ch, 1);
			coord = static_cast<int>(((y[k] - minCoord) / (maxCoord - minCoord)) * bpcd);
			if (coord < 0) coord = 0;
			if (coord > bpcd) coord = bpcd;
			ch = (coord >> 8) & 0xFF;
			m_buffer.Write(&ch, 1);
		...
		for (k = 0; k < nc; k++)
		{
			...
			{
				ch = static_cast<int>(wxPdfUtility::String2Double(tkz.GetNextToken()) * 255) & 0xFF;
				m_buffer.Write(&ch, 1);
			}
		}
	}
