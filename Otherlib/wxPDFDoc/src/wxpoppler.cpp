...
bool wxPoppler::Open(const wxString& strFileName)
{
	...
	if (!m_pPdfDocument)
	{
		std::vector<uint8_t> _compressedImage = CPictureUtility::ReadFile(strFileName);
		if (_compressedImage.size() > 0)
		{
			m_pPdfDocument = poppler::document::load_from_raw_data((char*)&_compressedImage.at(0), _compressedImage.size());
		}
...
bool wxPDFHandler::LoadFile(wxImage* poImage, wxInputStream& oIStream, bool bVerbose, int nIndex)
{
	...
	if (oPoppler.Open(static_cast<char*>(oOStream.GetOutputStreamBuffer()->GetBufferStart()),
...
bool wxPDFHandler::SaveFile(wxImage* poImage, wxOutputStream& oOStream, bool bVerbose)
{
	if (poImage->HasOption(wxIMAGE_OPTION_RESOLUTIONUNIT))
	{
		...
		if (nResolution)
		{
			...
			oPdfDocument.SetImageScale(
				static_cast<double>(nResolution) * (nResolutionUnit == wxIMAGE_RESOLUTION_CM ? 2.54 : 1.0) / 72.0);
