...
wxPdfFontSubsetCff::~wxPdfFontSubsetCff()
{
	if (m_fdDict.GetCount() > 0)
	{
		...
		{
			if (m_fdDict[j] != nullptr)
			{
				DestructDictionary(static_cast<wxPdfCffDictionary*>(m_fdDict[j]));
			}
		}
		m_fdDict.Clear();
	}
	if (m_fdPrivateDict.GetCount() > 0)
	{
		...
		{
			if (m_fdPrivateDict[j] != nullptr)
			{
				DestructDictionary(static_cast<wxPdfCffDictionary*>(m_fdPrivateDict[j]));
			}
		}
	...
	if (m_fdLocalSubrIndex.GetCount() > 0)
	{
		...
		{
			if (m_fdLocalSubrIndex[j] != nullptr)
			{
				delete static_cast<wxPdfCffIndexArray*>(m_fdLocalSubrIndex[j]);
			}
		}
...
{
	m_inFont = inFont;
	m_numGlyphsUsed = static_cast<int>(usedGlyphs->size());
...
wxPdfFontSubsetCff::ReadFontDict(wxPdfCffDictionary* dict, int dictOffset, int dictSize)
{
	...
	while (TellI() < end)
	{
		...
		while (argSize > 0);
		op = ReadOperator();
		auto dictElement = new wxPdfCffDictElement(op, m_inFont, argStart, argTotal);
		(*dict)[op] = dictElement;
	}
	return ok;
}
...
{
	wxPdfCffDictElement* dictElement = nullptr;
...
wxPdfFontSubsetCff::SetDictElementArgument(wxPdfCffDictionary* dict, int key, wxMemoryOutputStream& buffer)
{
	...
	{
		auto argument = new wxPdfCffIndexElement(buffer);
...
wxPdfFontSubsetCff::ReadCidFontDict()
{
	...
	m_numFontDicts = static_cast<int>(index.GetCount());
	...
	for (j = 0; ok && (j < m_numFontDicts); j++)
	{
		...
		ok = ReadFontDict(static_cast<wxPdfCffDictionary*>(m_fdDict[j]), element.GetOffset(), element.GetLength());
		if (ok)
		{
			wxPdfCffDictElement* privateOp = FindDictElement(static_cast<wxPdfCffDictionary*>(m_fdDict[j]), PRIVATE_OP);
			ok = (privateOp == nullptr);
			if (ok)
			{
				...
				ok = ReadPrivateDict(static_cast<wxPdfCffDictionary*>(m_fdPrivateDict[j]),
				                     static_cast<wxPdfCffIndexArray*>(m_fdLocalSubrIndex[j]), offset, size);
				if (ok)
				{
					wxMemoryOutputStream buffer;
					EncodeIntegerMax(0, buffer);
					EncodeIntegerMax(0, buffer);
					SetDictElementArgument(static_cast<wxPdfCffDictionary*>(m_fdDict[j]), PRIVATE_OP, buffer);
				}
			}
		}
	}
...
wxPdfFontSubsetCff::ReadTopDict()
{
	...
	m_numGlyphs = static_cast<int>(m_charstringsIndex->GetCount());

	if (m_isCid)
	{
		wxPdfCffDictElement* fdselectOp = FindDictElement(m_topDict, FDSELECT_OP);
		ok = (fdselectOp != nullptr);
		if (ok)
		{
...
{
	return static_cast<int>(m_inFont->TellI());
}
...
{
	return static_cast<int>(m_outFont->TellO());
}

int
wxPdfFontSubsetCff::GetSizeI()
{
	return static_cast<int>(m_inFont->GetSize());
}
...
wxPdfFontSubsetCff::ReadOffset(int offSize)
{
	...
	{
		offset *= 256;
		offset += static_cast<int>(ReadByte());
	}
	return offset;
}
...
wxPdfFontSubsetCff::EncodeIntegerMax(int value, wxMemoryOutputStream& buffer)
{
	char locBuffer[5];
	locBuffer[0] = 29;
	locBuffer[1] = static_cast<char>((value >> 24) & 0xff);
	locBuffer[2] = static_cast<char>((value >> 16) & 0xff);
	locBuffer[3] = static_cast<char>((value >> 8) & 0xff);
	locBuffer[4] = static_cast<char>((value) & 0xff);
...
wxPdfFontSubsetCff::EncodeInteger(int value, wxMemoryOutputStream& buffer)
{
	...
	{
		value -= 108;
		locBuffer[0] = static_cast<char>(((value >> 8) + 247) & 0xff);
		locBuffer[1] = static_cast<char>(((value)) & 0xff);
		count = 2;
	}
	else if (value >= -1131 && value <= -108)
	{
		value = -value - 108;
		locBuffer[0] = static_cast<char>(((value >> 8) + 251) & 0xff);
		locBuffer[1] = static_cast<char>(((value)) & 0xff);
		count = 2;
	}
	...
	{
		locBuffer[0] = 28;
		locBuffer[1] = static_cast<char>((value >> 8) & 0xff);
		locBuffer[2] = static_cast<char>((value) & 0xff);
		count = 3;
	}
	else
	{
...
{
	int sid1, sid2;
	sid1 = NUM_STD_STRINGS + static_cast<int>(m_stringsSubsetIndex->GetCount());
	auto registry = new wxPdfCffIndexElement("Adobe");
	m_stringsSubsetIndex->Add(registry);

	sid2 = NUM_STD_STRINGS + static_cast<int>(m_stringsSubsetIndex->GetCount());
	auto ordering = new wxPdfCffIndexElement("Identity");
...
{
	int numGlyphsUsed = static_cast<int>(m_usedGlyphs.GetCount());
	int usedGlyph;
...
{
	m_numFontDicts = 1;
	auto fdDict = new wxPdfCffDictionary();
	m_fdDict.Add(fdDict);
...
wxPdfFontSubsetCff::SubsetDictString(wxPdfCffDictionary* dict, int op)
{
	...
	if (element != nullptr)
	{
		...
		{
			int sidNew = NUM_STD_STRINGS + static_cast<int>(m_stringsSubsetIndex->GetCount());
...
wxPdfFontSubsetCff::SubsetStrings()
{
	...
	if (m_isCid)
	{
		...
		{
			SubsetDictStrings(static_cast<wxPdfCffDictionary*>(m_fdDict[m_fdSubsetMap[j]]));
			SubsetDictStrings(static_cast<wxPdfCffDictionary*>(m_fdPrivateDict[m_fdSubsetMap[j]]));
		}
	}
	else
	{
...
wxPdfFontSubsetCff::WriteInteger(int value, int size, wxMemoryOutputStream* buffer)
{
	...
	{
	case 4:
		locBuffer[i] = static_cast<char>((value >> 24) & 0xff);
		i++;
	case 3:
		locBuffer[i] = static_cast<char>((value >> 16) & 0xff);
		i++;
	case 2:
		locBuffer[i] = static_cast<char>((value >> 8) & 0xff);
		i++;
	case 1:
		locBuffer[i] = static_cast<char>((value) & 0xff);
		i++;
	default:
		break;
	}
...
{
	int numElements = static_cast<int>(index->GetCount());
...
wxPdfFontSubsetCff::WriteName()
{
	...
	wxMemoryOutputStream buffer;
	int len = static_cast<int>(m_fontName.Length());
	int j;
	for (j = 0; j < len; j++)
	{
		char ch = static_cast<char>(m_fontName[j]) & 0xff;
		buffer.Write(&ch, 1);
	}
	wxPdfCffIndexArray index;
...
wxPdfFontSubsetCff::WriteCidFontDict()
{
	...
	for (j = 0; j < m_numSubsetFontDicts; j++)
	{
		WriteDict(static_cast<wxPdfCffDictionary*>(m_fdDict[m_fdSubsetMap[j]]));
		offsetDict = TellO();
		offset += offsetSize;
...
wxPdfFontSubsetCff::WriteCidPrivateDictAndLocalSub()
{
	if (m_isCid)
	{
		...
		for (j = 0; j < m_numSubsetFontDicts; j++)
		{
			WritePrivateDict(j, static_cast<wxPdfCffDictionary*>(m_fdDict[m_fdSubsetMap[j]]),
			                 static_cast<wxPdfCffDictionary*>(m_fdPrivateDict[m_fdSubsetMap[j]]));
		}
		for (j = 0; j < m_numSubsetFontDicts; j++)
		{
			WriteLocalSub(j, static_cast<wxPdfCffDictionary*>(m_fdPrivateDict[m_fdSubsetMap[j]]),
			              static_cast<wxPdfCffIndexArray*>(m_fdLocalSubrIndex[m_fdSubsetMap[j]]));
		}
	}
	else
	{
		WritePrivateDict(0, static_cast<wxPdfCffDictionary*>(m_fdDict[0]), m_privateDict);
...
{
	int nGlobalSubrs = static_cast<int>(m_globalSubrIndex->GetCount());
	...
	if (m_isCid)
	{
		auto fdDictUsed = new bool[m_numFontDicts];
		int j;
		for (j = 0; j < m_numFontDicts; j++)
		{
			fdDictUsed[j] = false;
		...
		for (j = 0; j < m_numFontDicts; j++)
		{
			if (fdDictUsed[j])
			{
				...
				FindSubrsUsed(j, *static_cast<wxPdfCffIndexArray*>(m_fdLocalSubrIndex[j]), hSubrsUsed, lSubrsUsed);
				// Builds the New Local Subrs index
				SubsetSubrs(*static_cast<wxPdfCffIndexArray*>(m_fdLocalSubrIndex[j]), hSubrsUsed);
			}
		}
		delete [] fdDictUsed;
	}
...
wxPdfFontSubsetCff::SubsetSubrs(wxPdfCffIndexArray& subrIndex, wxPdfSortedArrayInt& subrsUsed)
{
	...
	{
		size_t j;
		auto isSubrUsed = new bool[nSubrs];
		int subr;
		for (j = 0; j < nSubrs; j++)
		{
			isSubrUsed[j] = false;
                                  ...
                                  wxPdfSortedArrayInt& hSubrsUsed, wxArrayInt& lSubrsUsed)
{
	...
	int nSubrs = static_cast<int>(localSubrIndex.GetCount());
...
{
	int nGlobalSubrs = static_cast<int>(m_globalSubrIndex->GetCount());
	int nLocalSubrs = static_cast<int>(m_localSubrIndex->GetCount());
	int localBias = 0;
