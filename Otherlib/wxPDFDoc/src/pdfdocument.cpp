...
{
	int width = static_cast<int>(pageWidth * (m_k * 254. / 72.));
	int height = static_cast<int>(pageHeight * (m_k * 254. / 72.));
...
wxPdfDocument::~wxPdfDocument()
{
	...
	{
		auto bookmark = static_cast<wxPdfBookmark*>(m_outlines[j]);
		delete bookmark;
	}
	...
	{
		if (formAnnotation->second != nullptr)
		{
	...
	{
		if (templateIter->second != nullptr)
		{
...
wxPdfDocument::MultiCell(double w, double h, const wxString& txt, int border, int align, int fill, int maxline)
{
	...
	m_wsApply = (align == wxPDF_ALIGN_JUSTIFY) && ((fontType == wxS("TrueTypeUnicode")) || (fontType == wxS(
		"OpenTypeUnicode")));
                       ...
                       int halign, int valign, int border, int fill)
{
	...
	int maxrows = static_cast<int>(floor(h / hrow));
...
wxPdfDocument::ImageMask(const wxString& file, const wxString& type)
{
	...
	{
		// First use of image, get info
		n = static_cast<int>((*m_images).size()) + 1;
...
wxPdfDocument::SaveAsFile(const wxString& name)
{
	...
	if (ok)
	{
		...
		{
			// Save to local file
			wxMemoryInputStream tmp(*static_cast<wxMemoryOutputStream*>(m_buffer));
			outfile.Write(tmp);
		}
		outfile.Close();
	}
...
wxPdfDocument::CloseAndGetBuffer()
{
	...
	return *static_cast<wxMemoryOutputStream*>(m_buffer);
}
...
wxPdfDocument::AttachFile(const wxString& fileName, const wxString& attachName, const wxString& description)
{
	...
	if (ok)
	{
		auto attachment = new wxArrayString();
		...
		int index = static_cast<int>(m_attachments->size() + 1);
...
wxPdfDocument::AddSpotColour(const wxString& name, double cyan, double magenta, double yellow, double black)
{
	...
	{
		int i = static_cast<int>((*m_spotColours).size()) + 1;
...
wxPdfDocument::AddPattern(const wxString& patternName, const wxImage& image, double width, double height)
{
	...
	if (patternIter == m_patterns->end())
	{
		if (image.IsOk() && width > 0 && height > 0)
		{
			...
			int i = static_cast<int>(m_patterns->size()) + 1;
