...
public:
	...
	///
	/// \return                     Returns true on success and false otherwise.
	virtual bool Open(char* pcData, int nLength);
...
public:
		...
		: m_nPageCount(0)
	{
		...
		m_extension = wxT("pdf");
		m_type = static_cast<wxBitmapType>(wxBITMAP_TYPE_PDF);
...
public:
	...
	///                     from zero).
	bool LoadFile(wxImage* poImage, wxInputStream& oIStream, bool bVerbose = true, int nIndex = 0) override;
	...
	bool SaveFile(wxImage* poImage, wxOutputStream& oOStream, bool bVerbose = true) override;
...
protected:
	...
	///
	/// \param oIStream     Opened input stream for reading image data.
	bool DoCanRead(wxInputStream& oIStream) override;
...
#  if wxCHECK_VERSION(2, 9, 0)
	...
	/// \param oIStream     Opened input stream for reading image data
	///                     (ignored).
	int DoGetImageCount(wxInputStream& WXUNUSED(oIStream) ) override { return m_nPageCount; }
#  else  // wxCHECK_VERSION( 2, 9, 0 )
#  endif // wxCHECK_VERSION( 2, 9, 0 )
#endif // wxUSE_STREAMS
