...
typedef struct avifDecoderItem
{
	...
	avifRWData mergedExtents;
	// if populated, is a single contiguous block of this item's extents (unused when extents.count == 1)
	...
	avifBool hasUnsupportedEssentialProperty;
...
{
	avifSampleTable* sampleTable = avifAlloc(sizeof(avifSampleTable));
...
{
	avifCodecDecodeInput* decodeInput = avifAlloc(sizeof(avifCodecDecodeInput));
...
static avifBool avifCodecDecodeInputGetSamples(avifCodecDecodeInput* decodeInput, avifSampleTable* sampleTable,
                                               avifROData* rawInput)
{
	...
	for (uint32_t chunkIndex = 0; chunkIndex < sampleTable->chunks.count; ++chunkIndex)
	{
		...
		for (int sampleToChunkIndex = sampleTable->sampleToChunks.count - 1; sampleToChunkIndex >= 0; --
		     sampleToChunkIndex)
		{
			avifSampleTableSampleToChunk* sampleToChunk = &sampleTable->sampleToChunks.sampleToChunk[
				sampleToChunkIndex];
		...
		for (uint32_t sampleIndex = 0; sampleIndex < sampleCount; ++sampleIndex)
		{
			...
			avifDecodeSample* sample = avifArrayPushPtr(&decodeInput->samples);
			...
			if ((sampleOffset + sampleSize) > rawInput->size)
			{
				return AVIF_FALSE;
			}
	...
	{
		uint32_t frameIndex = sampleTable->syncSamples.syncSample[syncSampleIndex].sampleNumber - 1;
		// sampleNumber is 1-based
...
{
	avifMeta* meta = avifAlloc(sizeof(avifMeta));
...
static avifDecoderItem* avifMetaFindItem(avifMeta* meta, uint32_t itemID)
{
	...
	avifDecoderItem* item = avifArrayPushPtr(&meta->items);
...
typedef struct avifDecoderData
{
	...
	const avifSampleTable* sourceSampleTable;
...
{
	avifDecoderData* data = avifAlloc(sizeof(avifDecoderData));
...
{
	avifTile* tile = avifArrayPushPtr(&data->tiles);
...
{
	avifTrack* track = avifArrayPushPtr(&data->tracks);
...
static const uint8_t* avifDecoderDataCalcItemPtr(avifDecoderData* data, avifDecoderItem* item)
{
	...
	if (item->extents.count == 1)
	{
		...
		if (offsetSize > offsetBuffer->size)
		{
			return NULL;
		}
...
static avifBool avifDecoderDataGenerateImageGridTiles(avifDecoderData* data, avifImageGrid* grid,
                                                      avifDecoderItem* gridItem, avifBool alpha)
{
	...
	for (uint32_t i = 0; i < gridItem->meta->items.count; ++i)
	{
		...
		if (item->dimgForID == gridItem->id)
		{
			...
			avifDecodeSample* sample = avifArrayPushPtr(&tile->input->samples);
			...
			if (firstTile)
			{
				...
				avifProperty* dstProp = avifArrayPushPtr(&gridItem->properties);
                                             ...
                                             avifBool alpha)
{
	...
	avifBool firstTileUVPresent = (firstTile->image->yuvPlanes[AVIF_CHAN_U] && firstTile->image->yuvPlanes[
		AVIF_CHAN_V]);
	...
	if (!alpha)
	{
		if ((firstTile->image->yuvFormat == AVIF_PIXEL_FORMAT_YUV422) || (firstTile->image->yuvFormat ==
	...
	if ((dstImage->width != grid->outputWidth) || (dstImage->height != grid->outputHeight) ||
		(dstImage->depth != firstTile->image->depth) || (!alpha && (dstImage->yuvFormat != firstTile->image->
			yuvFormat)))
	{
		if (alpha)
		{
		...
		for (unsigned int colIndex = 0; colIndex < grid->columns; ++colIndex, ++tileIndex)
		{
			...
			if (alpha)
			{
				...
				for (unsigned int j = 0; j < heightToCopy; ++j)
				{
					...
					uint8_t* dst = &dstImage->alphaPlane[(yaColOffset * pixelBytes) + ((yaRowOffset + j) * dstImage->
						alphaRowBytes)];
			...
			else
			{
				...
				for (unsigned int j = 0; j < heightToCopy; ++j)
				{
					...
					uint8_t* dst =
						&dstImage->yuvPlanes[AVIF_CHAN_Y][(yaColOffset * pixelBytes) + ((yaRowOffset + j) * dstImage->
							yuvRowBytes[AVIF_CHAN_Y])];
					memcpy(dst, src, yaRowBytes);
				}
				...
				for (unsigned int j = 0; j < heightToCopy; ++j)
				{
					...
					uint8_t* dstU =
						&dstImage->yuvPlanes[AVIF_CHAN_U][(uvColOffset * pixelBytes) + ((uvRowOffset + j) * dstImage->
							yuvRowBytes[AVIF_CHAN_U])];
					memcpy(dstU, srcU, uvRowBytes);

					uint8_t* srcV = &tile->image->yuvPlanes[AVIF_CHAN_V][j * tile->image->yuvRowBytes[AVIF_CHAN_V]];
					uint8_t* dstV =
						&dstImage->yuvPlanes[AVIF_CHAN_V][(uvColOffset * pixelBytes) + ((uvRowOffset + j) * dstImage->
							yuvRowBytes[AVIF_CHAN_V])];
...
static avifBool avifDecoderDataFindMetadata(avifDecoderData* data, avifMeta* meta, avifImage* image, uint32_t colorId)
{
	...
	for (uint32_t itemIndex = 0; itemIndex < meta->items.count; ++itemIndex)
	{
		...
		if (!memcmp(item->type, "Exif", 4))
		{
			...
			CHECK(avifROStreamReadU32(&exifBoxStream, &exifTiffHeaderOffset));
		...
		else if (!memcmp(item->type, "mime", 4) && !memcmp(item->contentType.contentType, xmpContentType,
		                                                   xmpContentTypeSize))
		{
...
static avifBool avifParseItemLocationBox(avifMeta* meta, const uint8_t* raw, size_t rawLen)
{
	...
	for (uint32_t i = 0; i < itemCount; ++i)
	{
		...
		for (int extentIter = 0; extentIter < extentCount; ++extentIter)
		{
			...
			avifDecoderItemExtent* extent = avifArrayPushPtr(&item->extents);
...
static avifBool avifParseImageGridBox(avifImageGrid* grid, const uint8_t* raw, size_t rawLen)
{
	...
	if ((grid->outputWidth == 0) || (grid->outputHeight == 0) || (grid->outputWidth > (AVIF_MAX_IMAGE_SIZE / grid->
		outputHeight)))
	{
		return AVIF_FALSE;
	}
	...
	for (uint32_t entryIndex = 0; entryIndex < entryCount; ++entryIndex)
	{
		...
		for (uint8_t associationIndex = 0; associationIndex < associationCount; ++associationIndex)
		{
			...
			static const char* supportedTypes[] = {
				"ispe", "auxC", "colr", "av1C", "pasp", "clap", "irot", "imir", "pixi"
			};
			...
			if (supportedType)
			{
				avifProperty* dstProp = avifArrayPushPtr(&item->properties);
				memcpy(dstProp, srcProp, sizeof(avifProperty));
			}
			else
			{
...
static avifBool avifParseChunkOffsetBox(avifSampleTable* sampleTable, avifBool largeOffsets, const uint8_t* raw,
                                        size_t rawLen)
{
	BEGIN_STREAM(s, raw, rawLen);
	...
	for (uint32_t i = 0; i < entryCount; ++i)
	{
		...
		avifSampleTableChunk* chunk = avifArrayPushPtr(&sampleTable->chunks);
		chunk->offset = offset;
	}
	return AVIF_TRUE;
}
...
static avifBool avifParseSampleToChunkBox(avifSampleTable* sampleTable, const uint8_t* raw, size_t rawLen)
{
	...
	for (uint32_t i = 0; i < entryCount; ++i)
	{
		avifSampleTableSampleToChunk* sampleToChunk = avifArrayPushPtr(&sampleTable->sampleToChunks);
		...
		CHECK(avifROStreamReadU32(&s, &sampleToChunk->sampleDescriptionIndex));
...
static avifBool avifParseSampleSizeBox(avifSampleTable* sampleTable, const uint8_t* raw, size_t rawLen)
{
	...
	else
	{
		...
		{
			avifSampleTableSampleSize* sampleSize = avifArrayPushPtr(&sampleTable->sampleSizes);
...
static avifBool avifParseSyncSampleBox(avifSampleTable* sampleTable, const uint8_t* raw, size_t rawLen)
{
	...
	for (uint32_t i = 0; i < entryCount; ++i)
	{
		...
		avifSyncSample* syncSample = avifArrayPushPtr(&sampleTable->syncSamples);
...
static avifBool avifParseTimeToSampleBox(avifSampleTable* sampleTable, const uint8_t* raw, size_t rawLen)
{
	...
	for (uint32_t i = 0; i < entryCount; ++i)
	{
		avifSampleTableTimeToSample* timeToSample = avifArrayPushPtr(&sampleTable->timeToSamples);
...
static avifBool avifParseSampleDescriptionBox(avifSampleTable* sampleTable, const uint8_t* raw, size_t rawLen)
{
	...
	for (uint32_t i = 0; i < entryCount; ++i)
	{
		...
		avifSampleDescription* description = avifArrayPushPtr(&sampleTable->sampleDescriptions);
		...
		if (!memcmp(description->format, "av01", 4) && (remainingBytes > VISUALSAMPLEENTRY_SIZE))
		{
			CHECK(avifParseItemPropertyContainerBox(
				&description->properties, avifROStreamCurrent(&s) + VISUALSAMPLEENTRY_SIZE, remainingBytes -
				VISUALSAMPLEENTRY_SIZE));
		}
...
{
	avifDecoder* decoder = avifAlloc(sizeof(avifDecoder));
...
avifResult avifDecoderReset(avifDecoder* decoder)
{
	...
	else
	{
		...
		else
		{
			...
			avifTile* colorTile = avifDecoderDataCreateTile(data);
			avifDecodeSample* colorSample = avifArrayPushPtr(&colorTile->input->samples);
		...
		if (alphaOBUItem)
		{
			...
			else
			{
				...
				avifTile* alphaTile = avifDecoderDataCreateTile(data);
				avifDecodeSample* alphaSample = avifArrayPushPtr(&alphaTile->input->samples);
...
avifResult avifDecoderNextImage(avifDecoder* decoder)
{
	...
	{
		if (!avifDecoderDataFillImageGrid(
			decoder->data, &decoder->data->alphaGrid, decoder->image, decoder->data->colorTileCount,
			decoder->data->alphaTileCount, AVIF_TRUE))
		{
			return AVIF_RESULT_INVALID_IMAGE_GRID;
		}
	}
...
avifResult avifDecoderNthImage(avifDecoder* decoder, uint32_t frameIndex)
{
	...
	decoder->imageIndex = ((int)avifDecoderNearestKeyframe(decoder, frameIndex)) - 1;
	// prepare to read nearest keyframe
