...
avifBool avifPrepareReformatState(const avifImage* image, const avifRGBImage* rgb, avifReformatState* state)
{
	...
	if ((image->matrixCoefficients == 3 /* CICP reserved */) || (image->matrixCoefficients ==
	...
	{
		// Note the >= catching "future" CICP values here too
		return AVIF_FALSE;
	}
...
{
	if ((chan != AVIF_CHAN_Y) && (mode != AVIF_REFORMAT_MODE_IDENTITY))
	{
		// Don't adjust U or V when data is raw RGB
		v += 0.5f;
	}
	...
	if (range == AVIF_RANGE_LIMITED)
	{
		if ((chan == AVIF_CHAN_Y) || (mode == AVIF_REFORMAT_MODE_IDENTITY))
		{
			// use Y range for all channels when data is raw RGB
		...
		for (uint32_t outerI = 0; outerI < image->width; outerI += 2)
		{
			...
			for (int bJ = 0; bJ < blockH; ++bJ)
			{
				for (int bI = 0; bI < blockW; ++bI)
				{
					...
					{
						rgbPixel[0] =
							*((uint16_t*)(&rgb->pixels[state.rgbOffsetBytesR + (i * state.rgbPixelBytes) + (j * rgb->
								rowBytes)])) /
							rgbMaxChannel;
						rgbPixel[1] =
							*((uint16_t*)(&rgb->pixels[state.rgbOffsetBytesG + (i * state.rgbPixelBytes) + (j * rgb->
								rowBytes)])) /
							rgbMaxChannel;
						rgbPixel[2] =
							*((uint16_t*)(&rgb->pixels[state.rgbOffsetBytesB + (i * state.rgbPixelBytes) + (j * rgb->
								rowBytes)])) /
							rgbMaxChannel;
					}
					else
					{
						rgbPixel[0] = rgb->pixels[state.rgbOffsetBytesR + (i * state.rgbPixelBytes) + (j * rgb->
							rowBytes)] / rgbMaxChannel;
						rgbPixel[1] = rgb->pixels[state.rgbOffsetBytesG + (i * state.rgbPixelBytes) + (j * rgb->
							rowBytes)] / rgbMaxChannel;
						rgbPixel[2] = rgb->pixels[state.rgbOffsetBytesB + (i * state.rgbPixelBytes) + (j * rgb->
							rowBytes)] / rgbMaxChannel;
					}
						...
						*pY = (uint16_t)yuvToUNorm(
						...
						if (image->yuvFormat == AVIF_PIXEL_FORMAT_YUV444)
						{
							...
							*pU = (uint16_t)yuvToUNorm(
								AVIF_CHAN_U, image->yuvRange, image->depth, yuvMaxChannel, yuvBlock[bI][bJ].u,
								state.mode);
							uint16_t* pV = (uint16_t*)&yuvPlanes[AVIF_CHAN_V][(i * 2) + (j * yuvRowBytes[AVIF_CHAN_V])];
							*pV = (uint16_t)yuvToUNorm(
								AVIF_CHAN_V, image->yuvRange, image->depth, yuvMaxChannel, yuvBlock[bI][bJ].v,
								state.mode);
						}
					}
					else
...
static avifResult avifImageYUVAnyToRGBAnySlow(const avifImage* image, avifRGBImage* rgb, avifReformatState* state)
{
	...
	const uint16_t yuvMaxChannel = (1 << image->depth) - 1;
		...
		for (uint32_t i = 0; i < image->width; ++i)
		{
			...
			if (hasColor)
			{
				...
				else
				{
					...
					if ((j == 0) || ((j == (image->height - 1)) && ((j % 2) != 0)) || (image->yuvFormat ==
					...
					else
					{
						...
						unormU[1][0] = *((const uint16_t*)&uPlane[(uvJ * uRowBytes) + (uvI * yuvChannelBytes) +
							uAdjCol]);
						unormV[1][0] = *((const uint16_t*)&vPlane[(uvJ * vRowBytes) + (uvI * yuvChannelBytes) +
							vAdjCol]);
						unormU[0][1] = *((const uint16_t*)&uPlane[(uvJ * uRowBytes) + (uvI * yuvChannelBytes) +
							uAdjRow]);
						unormV[0][1] = *((const uint16_t*)&vPlane[(uvJ * vRowBytes) + (uvI * yuvChannelBytes) +
							vAdjRow]);
						unormU[1][1] = *((const uint16_t*)&uPlane[(uvJ * uRowBytes) + (uvI * yuvChannelBytes) + uAdjCol
							+ uAdjRow]);
						unormV[1][1] = *((const uint16_t*)&vPlane[(uvJ * vRowBytes) + (uvI * yuvChannelBytes) + vAdjCol
							+ vAdjRow]);
					...
					if (rgb->chromaUpsampling == AVIF_CHROMA_UPSAMPLING_BILINEAR)
					{
						...
						Cb = (unormFloatTableUV[unormU[0][0]] * (9.0f / 16.0f)) + (unormFloatTableUV[unormU[1][0]] * (
								3.0f / 16.0f)) +
							(unormFloatTableUV[unormU[0][1]] * (3.0f / 16.0f)) + (unormFloatTableUV[unormU[1][1]] * (
								1.0f / 16.0f));
						Cr = (unormFloatTableUV[unormV[0][0]] * (9.0f / 16.0f)) + (unormFloatTableUV[unormV[1][0]] * (
								3.0f / 16.0f)) +
							(unormFloatTableUV[unormV[0][1]] * (3.0f / 16.0f)) + (unormFloatTableUV[unormV[1][1]] * (
								1.0f / 16.0f));
					}
					else
					{
...
static avifResult avifImageIdentity8ToRGB8ColorFullRange(const avifImage* image, avifRGBImage* rgb,
                                                         avifReformatState* state)
{
	const uint32_t rgbPixelBytes = state->rgbPixelBytes;
	for (uint32_t j = 0; j < image->height; ++j)
...
avifResult avifImageYUVToRGB(const avifImage* image, avifRGBImage* rgb)
{
	...
	(image->yuvRowBytes[AVIF_CHAN_U] && image->yuvRowBytes[AVIF_CHAN_V] && (image->yuvFormat !=
		AVIF_PIXEL_FORMAT_YUV400));

	if (!hasColor || (image->yuvFormat == AVIF_PIXEL_FORMAT_YUV444) || (rgb->chromaUpsampling ==
