...
static avifBool dav1dFeedData(avifCodec* codec)
{
	if (!codec->internal->dav1dData.sz)
	{
		if (codec->internal->inputSampleIndex < codec->decodeInput->samples.count)
		{
			...
			if (dav1d_data_wrap(&codec->internal->dav1dData, sample->data.data, sample->data.size,
			                    avifDav1dFreeCallback, NULL) != 0)
			{
				return AVIF_FALSE;
			}
		}
...
static avifBool dav1dCodecGetNextImage(avifCodec* codec, avifImage* image)
{
	...
	if (gotPicture)
	{
		...
		codec->internal->colorRange = codec->internal->dav1dPicture.seq_hdr->color_range
			                              ? AVIF_RANGE_FULL
			                              : AVIF_RANGE_LIMITED;
