...
static avifEncoderItem* avifEncoderDataCreateItem(avifEncoderData* data, const char* type, const char* infeName,
                                                  size_t infeNameSize)
{
...
static void avifEncoderWriteColorProperties(avifRWStream* s, const avifImage* imageMetadata, struct ipmaArray* ipma,
                                            uint8_t* itemPropertyIndex)
{
	...
	else
	{
		...
		avifRWStreamWriteU8(s, (imageMetadata->yuvRange == AVIF_RANGE_FULL) ? 0x80 : 0);
...
static void avifEncoderWriteTrackMetaBox(avifEncoder* encoder, avifRWStream* s)
{
	...
	avifRWStreamWriteU16(s, metadataItemCount); // unsigned int(16) item_count;
	for (uint32_t trakItemIndex = 0; trakItemIndex < encoder->data->items.count; ++trakItemIndex)
	{
		...
		avifRWStreamWriteU32(s, item->metadataPayload.size); // unsigned int(length_size*8) extent_length;
	}
	avifRWStreamFinishBox(s, iloc);

	avifBoxMarker iinf = avifRWStreamWriteFullBox(s, "iinf", AVIF_BOX_SIZE_TBD, 0, 0);
	avifRWStreamWriteU16(s, metadataItemCount); //  unsigned int(16) entry_count;
	for (uint32_t trakItemIndex = 0; trakItemIndex < encoder->data->items.count; ++trakItemIndex)
	{
		avifEncoderItem* item = &encoder->data->items.item[trakItemIndex];
		if (memcmp(item->type, "av01", 4) == 0)
...
avifResult avifEncoderFinish(avifEncoder* encoder, avifRWData* output)
{
	...
	avifRWStreamWriteZeros(&s, 1); // unsigned int(4) base_offset_size;
	// unsigned int(4) reserved;
	avifRWStreamWriteU16(&s, encoder->data->items.count); // unsigned int(16) item_count;

	for (uint32_t itemIndex = 0; itemIndex < encoder->data->items.count; ++itemIndex)
	{
		avifEncoderItem* item = &encoder->data->items.item[itemIndex];

		uint32_t contentSize = item->metadataPayload.size;
	...
	avifBoxMarker iinf = avifRWStreamWriteFullBox(&s, "iinf", AVIF_BOX_SIZE_TBD, 0, 0);
	avifRWStreamWriteU16(&s, encoder->data->items.count); //  unsigned int(16) entry_count;
	...
	for (uint32_t itemIndex = 0; itemIndex < encoder->data->items.count; ++itemIndex)
	{
		...
		{
			avifRWStreamWriteU8(&s, imageMetadata->depth); // unsigned int (8) bits_per_channel;
		}
	...
	if (encoder->data->frames.count > 1)
	{
		...
		avifRWStreamWriteU32(&s, encoder->timescale); // unsigned int(32) timescale;
		...
		for (uint32_t itemIndex = 0; itemIndex < encoder->data->items.count; ++itemIndex)
		{
			...
			if (item->irefToID != 0)
			{
				...
				avifBoxMarker refType = avifRWStreamWriteBox(&s, item->irefType, AVIF_BOX_SIZE_TBD);
				avifRWStreamWriteU32(&s, item->irefToID);
				avifRWStreamFinishBox(&s, refType);
			...
			{
				avifEncodeSample* sample = &item->encodeOutput->samples.sample[sampleIndex];
				avifRWStreamWriteU32(&s, sample->data.size); // unsigned int(32) entry_size;
			}
			...
			for (uint32_t sampleCount = 0, frameIndex = 0; frameIndex < encoder->data->frames.count; ++frameIndex)
			{
				...
				avifRWStreamWriteU32(&s, frame->durationInTimescales); // unsigned int(32) sample_delta;
				sampleCount = 0;
				++sttsEntryCount;
			}
			...
			avifRWStreamWriteU16(&s, imageMetadata->width); // unsigned int(16) width;
			avifRWStreamWriteU16(&s, imageMetadata->height); // unsigned int(16) height;
			...
			avifRWStreamWriteU16(&s, 0xffff); // int(16) pre_defined = -1;
			writeConfigBox(&s, &item->codec->configBox);
			if (!item->alpha)
			{
	...
	for (uint32_t itemPasses = 0; itemPasses < 2; ++itemPasses)
	{
		...
		for (uint32_t itemIndex = 0; itemIndex < encoder->data->items.count; ++itemIndex)
		{
			...
			uint32_t chunkOffset = avifRWStreamOffset(&s);
			if (item->encodeOutput->samples.count > 0)
			{
				for (uint32_t sampleIndex = 0; sampleIndex < item->encodeOutput->samples.count; ++sampleIndex)
				{
