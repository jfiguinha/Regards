...
avifBoxMarker avifRWStreamWriteFullBox(avifRWStream* stream, const char* type, size_t contentSize, int version,
                                       uint32_t flags)
{
	...
	uint32_t noSize = avifHTONL(headerSize + contentSize);
...
{
	uint32_t noSize = avifNTOHL(stream->offset - marker);
