...
//----------------------------------------------------
__kernel void Convert(__global float4 *output, const __global uchar *inputY, const __global uchar *inputUV, int widthIn, int heightIn, int widthOut, int heightOut, int pitch) 
{ 
    int x = get_global_id(0);
	int y = get_global_id(1);
	int position = x + (heightIn - y - 1) * widthOut;

	output[position] = GetColorFromNV12(inputY, inputUV, x,  y, widthIn, heightIn, pitch);
} 
