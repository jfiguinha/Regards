...
uint PostFX(const __global uint *input, int x, int y, float radius, float angleDegree, int widthIn, int heightIn)
{
  float xOut = x;
  float yOut = y;
...
__kernel void Swirl(__global uint *output,const __global uint *input, int width, int height, float radius, float angleDegree)
{
