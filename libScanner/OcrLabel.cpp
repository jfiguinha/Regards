...
void COcrLabel::MouseOver(CPositionElement* element, const int& x, const int& y,
...
void COcrLabel::UpdateChildTree(tree<CTreeData*>::sibling_iterator& parent)
{
	...
	for (auto i = 0; i < parent.number_of_children(); i++)
	{
		...
		if (posElement == nullptr)
		{
			...
			CreatePositionElement(xPos, yPos, nbRow, 0, tree_element_check->GetWidth(), tree_element_check->GetHeight(),
			                      ELEMENT_CHECKBOX, tree_element_check, data);
		...
		else
		{
			...
			if (posElement == nullptr)
			{
				...
				treeElementTexte->SetVisible(isVisible);
				CreatePositionElement(xPos, yPos, nbRow, 0, treeElementTexte->GetWidth(), treeElementTexte->GetHeight(),
				                      ELEMENT_TEXTE, treeElementTexte, data, false);
			}
			else
			{
				auto tree_element_texte = static_cast<CTreeElementTexte*>(posElement->GetTreeElement());
