...
typedef bool (*ProgressAnbAbortCallBack)(uint8_t bPercentComplete);
...
------------------------------------------------------------------------------*/
{
    ...
    for (int uY = 0; uY < int(sSrc.y); uY++)
    {
        ...
        {
            // Report progress
            if (!m_Callback (uint8_t(float (uY) / float(sSrc.y) * float(50.0))))
            {
                // Operation cancelled
...
------------------------------------------------------------------------------*/
{
    ...
    {  
        ...
        {
            // Positive angle
			dShear = (float(u) + 0.5) * dTan;
        }
        else
        {
            // Negative angle
			dShear = (float(int(u) - sDst1.y) + 0.5) * dTan;
        }
        ...
        {
            // Report progress
            uint8_t bProgress = bMidImage ? 50 + uint8_t(float (u) / float(sDst1.y) * float(50.0 / 3.0)) :
                                         uint8_t(float (u) / float(sDst1.y) * float(33.33333));
    ...
    wxSize sDst2;
    sDst2.x = sDst1.x;
    sDst2.y = int (float (sSrc.x) * fabs (dSinE) + float (sSrc.y) * cos (dRadAngle)) + 1;
    ...
    {   
        // Positive angle
        dOffset = float (sSrc.x - 1) * dSinE;
    }
    else
    {
        // Negative angle
        dOffset = -dSinE * float (sSrc.x - sDst2.x);
    }
    ...
    for (long u = 0; u < sDst2.x; u++, dOffset -= dSinE) 
    {
        ...
        VertSkew (  pDst1, 
                    ...
                    u,
                    iShear,
                    dOffset - float(iShear),
                    clrBack);
        if (m_Callback)
        {
            // Report progress
            uint8_t bProgress = bMidImage ? 66 + uint8_t(float (u) / float(sDst2.y) * float(50.0 / 3.0)) :
                                         33 + uint8_t(float (u) / float(sDst2.y) * float(33.33333));
            if (!m_Callback (bProgress))
            {
                // Operation cancelled
                DestroyBitmap (pDst1, sDst1);
    ...
    (*psDst).x = int (float(sSrc.y) * fabs (dSinE) + float(sSrc.x) * cos (dRadAngle)) + 1;
    (*psDst).y = sDst2.y;
    ...
    {
        // Positive angle
        dOffset = float(sSrc.x - 1) * dSinE * -dTan;
    }
    else 
    {
        // Negative angle
    ...
    for (int u = 0; u < int((*psDst).y); u++, dOffset += dTan)
    {
        ...
        {
            // Report progress
            uint8_t bProgress = bMidImage ? 83 + uint8_t(float (u) / float((*psDst).y) * float(50.0 / 3.0)) :
                                         66 + uint8_t(float (u) / float((*psDst).y) * float(33.33333));
            if (!m_Callback (bProgress))
            {
                // Operation cancelled
                DestroyBitmap (pDst2, sDst2);
