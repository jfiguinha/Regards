...
CMeanShift::CMeanShift(float s, float r)
{
	hs = s;
	hr = r;
}
...
void CMeanShift::MSFiltering(Mat& Img)
{
	...
	for (int i = 0; i < ROWS; i++)
	{
		for (int j = 0; j < COLS; j++)
		{
			...
			PtCur.MSPOint5DSet(i, j, IMGChannels[0].at<uchar>(i, j), IMGChannels[1].at<uchar>(i, j),
			                   IMGChannels[2].at<uchar>(i, j));
			PtCur.PointLab();
			...
			do
			{
				...
				for (int hx = Top; hx < Bottom; hx++)
				{
					for (int hy = Left; hy < Right; hy++)
					{
						...
						Pt.MSPOint5DSet(hx, hy, IMGChannels[0].at<uchar>(hx, hy), IMGChannels[1].at<uchar>(hx, hy),
						                IMGChannels[2].at<uchar>(hx, hy));
						Pt.PointLab();
			...
			while ((PtCur.MSPoint5DColorDistance(PtPrev) > MS_MEAN_SHIFT_TOL_COLOR) && (PtCur.
				MSPoint5DSpatialDistance(PtPrev) > MS_MEAN_SHIFT_TOL_SPATIAL) && (step < MS_MAX_NUM_CONVERGENCE_STEPS));
...
void CMeanShift::MSSegmentation(Mat& Img)
{
	//---------------- Mean Shift Filtering -----------------------------
	...
	auto Mode = new float [ROWS * COLS * 3]; // Store the Lab color of each region
	auto MemberModeCount = new int [ROWS * COLS]; // Store the number of each region
	memset(MemberModeCount, 0, ROWS * COLS * sizeof(int)); // Initialize the MemberModeCount
	split(Img, IMGChannels);
	// Label for each point
	auto Labels = new int*[ROWS];
	for (int i = 0; i < ROWS; i++)
			...
			if (Labels[i][j] < 0)
			{
				...
				while (!NeighbourPoints.empty())
				{
					...
					for (int k = 0; k < 8; k++)
					{
						...
						{
							Point5D P;
							P.MSPOint5DSet(hx, hy, IMGChannels[0].at<uchar>(hx, hy), IMGChannels[1].at<uchar>(hx, hy),
							               IMGChannels[2].at<uchar>(hx, hy));
							P.PointLab();
