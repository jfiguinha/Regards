...
int VideoThumbnailer::GetThumbnailPos(CRegardsBitmap* image, MovieDecoder* movieDecoder, const int64& m_seekTimeInSecond, const int& thumbnailWidth, const int& thumbnailHeight, const int& rotation)
{
    try
    {
        movieDecoder->seek(m_seekTimeInSecond);
    }
    catch (exception& e)
    {
        return -1;
...
int VideoThumbnailer::GetThumbnail( CRegardsBitmap * image, MovieDecoder * movieDecoder, const int &percent, const int & thumbnailWidth, const int & thumbnailHeight, const int & rotation)
{
    int secondToSeekTo = (movieDecoder->getDuration() * percent) / 100;
    m_seekTimeInSecond = secondToSeekTo;
    return GetThumbnailPos(image, movieDecoder, m_seekTimeInSecond, thumbnailWidth, thumbnailHeight, rotation);
...
void VideoThumbnailer::generateThumbnail(const string& videoFile, CRegardsBitmap * image, const int & thumbnailWidth, const int & thumbnailHeight, int & rotation, AVFormatContext* pAvContext)
{
	...
	GetThumbnail(image, &movieDecoder, m_SeekPercentage, thumbnailWidth, thumbnailHeight, rotation);
}
