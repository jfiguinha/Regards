...
int VideoThumbnailer::GetThumbnail(const string& videoFile, CxImage * image, MovieDecoder * movieDecoder, const int &percent)
{
	...
	*/
	writeImage(videoFile, *imageWriter, videoFrame, movieDecoder->getDuration(), videoFrame.frameData, videoFrame.lineSize);
	delete imageWriter;
...
void VideoThumbnailer::writeImage(const string& videoFile, ImageWriter& imageWriter, const VideoFrame& videoFrame, int duration, std::vector<uint8_t> & rowPointers, int lineSize)
{
    ...
    imageWriter.writeFrame(rowPointers, lineSize, videoFrame.width, videoFrame.height, m_ImageQuality);
}
