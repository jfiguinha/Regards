...
int CDetectFacePCN::DetectFaceAngle(const cv::Mat& frameOpenCVDNN)
{
	...
	try
	{
		...
		{
			FaceBox facelocal = preList[i];
			angle = static_cast<int>(360.0 - facelocal.angle) % 360;
		}
	}
	catch (Exception& e)
	{
...
{
	cv::Mat aux;
	cv::resize(_img, aux, cv::Size(static_cast<int>(_img.cols / _scale), static_cast<int>(_img.rows / _scale)));
	return aux;
}

cv::Mat CDetectFacePCN::padImg(cv::Mat _img)
{
	int row = std::min(static_cast<int>(_img.rows * 0.2), 100);
	int col = std::min(static_cast<int>(_img.cols * 0.2), 100);
	cv::Mat aux;
...
std::vector<FaceBox> CDetectFacePCN::TransformBoxes(cv::Mat _img, cv::Mat _imgPad, std::vector<FaceBox>& _faces)
{
	...
	for (int i = 0; i < _faces.size(); i++)
	{
		if (_faces[i].w > 0 && _faces[i].h > 0)
			newFaceBoxes.push_back(FaceBox(_faces[i].x - col, _faces[i].y - row, _faces[i].w, _faces[i].w,
			                               _faces[i].angle, _faces[i].scale, _faces[i].score));
	}
	return newFaceBoxes;
}


std::vector<FaceBox> CDetectFacePCN::PCN_1(cv::Mat _img, cv::Mat _paddedImg, cv::dnn::Net _net, float _thresh,
                                           int _minFaceSize)
{
	...
	while (std::min(resizedImg.rows, resizedImg.cols) >= netSize)
	{
		...
		for (int i = 0; i < scoresData.size[2]; i++)
		{
			for (int j = 0; j < scoresData.size[3]; j++)
			{
				...
				int x = static_cast<int>(j * currentScale * stride - 0.5 * sn * w + sn * xn * w + 0.5 * w) + col;
				int y = static_cast<int>(i * currentScale * stride - 0.5 * sn * w + sn * yn * w + 0.5 * w) + row;
				int wh = static_cast<int>(w * sn);
		...
		currentScale = static_cast<float>(_img.rows) / resizedImg.rows;
	}
...
std::vector<FaceBox> CDetectFacePCN::PCN_2(cv::Mat _img, cv::Mat _img180, cv::dnn::Net _net, float _threshold, int _dim,
                                           std::vector<FaceBox> _faces)
{
	...
	for (int i = 0; i < _faces.size(); i++)
	{
		...
		else
		{
			...
			dataList.push_back(preprocessImg(_img180(cv::Rect(_faces[i].x, height - 1 - y2, _faces[i].w, _faces[i].h)),
			                                 _dim));
		}
	}
	...
	for (int dataNr = 0; dataNr < dataList.size(); dataNr++)
	{
		...
		int w = static_cast<int>(sn * cropW);
		int x = static_cast<int>(cropX - 0.5 * sn * cropW + cropW * sn * xn + 0.5 * cropW);
		int y = static_cast<int>(cropY - 0.5 * sn * cropW + cropW * sn * yn + 0.5 * cropW);
		...
		if (xyValid(x, y, _img) && xyValid(x + w - 1, y + w - 1, _img))
		//(Legal(x, y, img) && Legal(x + w - 1, y + w - 1, img))
		{
			float angle = 0;
			if (abs(_faces[dataNr].angle) < EPS)
			{
...
std::vector<FaceBox> CDetectFacePCN::PCN_3(cv::Mat _img, cv::Mat _img180, cv::Mat _img90, cv::Mat _imgNeg90,
                                           cv::dnn::Net _net, float _threshold, int _dim, std::vector<FaceBox> _faces)
{
	if (_faces.size() == 0)
		return _faces;
	...
	for (int i = 0; i < _faces.size(); i++)
	{
		...
		{
			dataList.push_back(
				preprocessImg(_img90(cv::Rect(_faces[i].y, _faces[i].x, _faces[i].h, _faces[i].w)), _dim));
		}
		else if (abs(_faces[i].angle + 90) < EPS)
		{
			int x = _faces[i].y;
	...
	for (int dataNr = 0; dataNr < dataList.size(); dataNr++)
	{
		...
		if (xyValid(x, y, imgTmp) && xyValid(x + w - 1, y + w - 1, imgTmp))
		{
			...
			{
				faceBoxes_3.push_back(FaceBox(x, height - 1 - (y + w - 1), w, w, 180 - angle, _faces[dataNr].scale,
				                              score));
			}
			else if (abs(_faces[dataNr].angle - 90) < EPS)
			{
...
cv::Point CDetectFacePCN::RotatePoint(int x, int y, float centerX, float centerY, float angle)
{
	...
	int rx = static_cast<int>(centerX + x * std::cos(theta) - y * std::sin(theta));
	int ry = static_cast<int>(centerY + x * std::sin(theta) + y * std::cos(theta));
