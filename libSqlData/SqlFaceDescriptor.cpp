...
{
	ExecuteInsertBlobData("INSERT INTO FACEDESCRIPTOR (NumFace, FaceDescriptor) VALUES(" + to_string(numFace) + ", ? )", 2, zBlob, nBlob);
	return 0;
}
...
{
	return (ExecuteRequestWithNoResult("DELETE FROM FACEDESCRIPTOR WHERE NumFace = " + to_string(numFace)) != -1) ? true : false;
}
