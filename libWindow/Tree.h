...
public:
	...
	public:
		leaf_iterator();
		leaf_iterator(tree_node*, tree_node* top = nullptr);
...
typename tree<T, tree_node_allocator>::fixed_depth_iterator tree<T, tree_node_allocator>::begin_fixed(
	const iterator_base& pos, unsigned int dp, bool walk_back) const
{
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::path_t tree<T, tree_node_allocator>::path_from_iterator(
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::iterator tree<T, tree_node_allocator>::iterator_from_path(
...
iter tree<T, tree_node_allocator>::next_at_same_depth(iter position) const
{
	...
	fixed_depth_iterator tmp(position.node);
...
int tree<T, tree_node_allocator>::max_depth(const iterator_base& pos) const
{
	...
	if (tmp == nullptr || tmp == head || tmp == feet) return -1;
	...
	while (true)
	{
		...
		while (tmp->first_child == 0)
		{
			...
			if (tmp->next_sibling == 0)
			{
				...
				{
					tmp = tmp->parent;
					if (tmp == nullptr) return maxdepth;
					--curdepth;
				}
				while (tmp->next_sibling == 0);
			}
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::breadth_first_queued_iterator& tree<
	T, tree_node_allocator>::breadth_first_queued_iterator::operator+=(unsigned int num)
{
	while (num > 0)
	{
		++(*this);
...
tree<T, tree_node_allocator>::fixed_depth_iterator::fixed_depth_iterator(tree_node* tn)
	: iterator_base(tn), top_node(nullptr)
{
}
...
tree<T, tree_node_allocator>::leaf_iterator::leaf_iterator()
	: iterator_base(0), top_node(nullptr)
{
}
